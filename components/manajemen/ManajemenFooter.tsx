...
import { ShieldCheck, Server, Lock, HeartHandshake, Code2, Cpu, Activity, Mail, CheckCircle2, Clock, Headphones, Printer, Info, Globe2, RefreshCw, AlertTriangle, Send } from 'lucide-react';
...
export default function ManajemenFooter() {
  ...
  // TAMBAHAN: State untuk panel / modal panggilan darurat IT (Emergency Alert)
  ...
  // TAMBAHAN: Handler pengiriman sinyal darurat IT
    ...
    <footer className={`w-full border-t backdrop-blur-xl mt-auto pt-8 pb-6 shadow-sm print:hidden relative transition-colors duration-300 ${isFooterDarkMode ? 'bg-slate-900 border-slate-800 text-slate-100' : 'bg-white/95 border-slate-200 text-slate-800'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 space-y-6">
        ...
        <div className={`flex flex-col sm:flex-row items-center justify-between gap-4 text-xs ${isFooterDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
          <div className="flex items-center gap-2 font-medium text-center sm:text-left flex-wrap justify-center sm:justify-start">
          ...
          <div className="flex items-center space-x-2 sm:space-x-3 font-semibold text-[11px] flex-wrap justify-center sm:justify-end">
            <span className="hover:text-emerald-500 transition cursor-pointer flex items-center gap-1">
              <HeartHandshake className="w-3.5 h-3.5 text-emerald-500" /> Layanan BLUD
            </span>
            <span>&bull;</span>
            
            {/* TAMBAHAN: Tombol Panggilan Darurat IT */}
            <button 
              onClick={() => setShowEmergencyModal(true)}
              className="transition flex items-center gap-1 text-[10px] font-bold px-2.5 py-1 rounded-xl border border-rose-300 bg-rose-50 text-rose-700 hover:bg-rose-100 cursor-pointer active:scale-95 shadow-xs"
              title="Kirim Sinyal Darurat ke Ruang Server"
            >
              <AlertTriangle className="w-3 h-3 text-rose-600" /> Darurat IT
            </button>
            <span>&bull;</span>

            <button 
              onClick={() => setIsFooterDarkMode(!isFooterDarkMode)}
              className={`transition text-[10px] font-bold px-2 py-1 rounded-xl border cursor-pointer active:scale-95 ${isFooterDarkMode ? 'bg-slate-800 text-amber-400 border-slate-700' : 'bg-slate-100 text-slate-700 border-slate-200'}`}
            ...
            <button 
              ...
              className={`transition flex items-center gap-1 font-bold px-2 py-1 rounded-xl border cursor-pointer active:scale-95 ${isFooterDarkMode ? 'bg-slate-800 text-slate-300 border-slate-700 hover:text-white' : 'bg-slate-100 text-slate-700 border-slate-200'}`}
            ...
            >
              <Headphones className="w-3 h-3" /> Bantuan
            </button>
          </div>
        </div>
      ...
      {/* TAMBAHAN: Modal Panggilan Darurat IT (Emergency Alert Modal) */}
      {showEmergencyModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border border-rose-200 p-6 rounded-3xl shadow-2xl w-full max-w-md text-slate-800 animate-in zoom-in-95 space-y-4">
            <div className="flex items-center justify-between border-b border-rose-100 pb-3">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h4 className="font-black text-sm text-slate-900">Panggilan Darurat IT &amp; Server</h4>
                  <p className="text-[11px] text-rose-600 font-semibold">Prioritas Penanganan Mendesak</p>
                </div>
              </div>
              <button onClick={() => setShowEmergencyModal(false)} className="text-slate-400 hover:text-slate-700 font-bold text-sm cursor-pointer">✕</button>
            </div>
            ...
            {emergencySent ? (
              <div className="py-8 text-center space-y-3">
                ...
                <h5 className="font-black text-sm text-slate-900">Sinyal Darurat Terkirim!</h5>
                <p className="text-xs text-slate-500">Notifikasi prioritas telah diteruskan ke perangkat siaga Administrator &amp; Tim IT.</p>
              </div>
            ) : (
              <form onSubmit={handleSendEmergencyAlert} className="space-y-3 text-xs">
                <p className="text-slate-600 leading-relaxed">
                  Gunakan fitur ini hanya jika terjadi gangguan kritis (Contoh: Server SIMRS down, jaringan anjlok total, atau error database saat pelayanan darurat).
                </p>
                <div>
                  <label className="block font-bold text-slate-700 mb-1">Pilih Unit / Kendala Kritis:</label>
                  <select className="w-full bg-slate-50 border border-slate-200 rounded-xl p-2.5 text-xs text-slate-800 font-medium focus:outline-none focus:border-rose-500">
                    <option>⚠️ Server SIMRS Khanza Tidak Merespon</option>
                    <option>🌐 Jaringan Internet / LAN RS Putus Total</option>
                    <option>⚡ Kendala Listrik / UPS Ruang Server</option>
                    <option>💻 Lainnya (Darurat Medis / IT)</option>
                  </select>
                </div>
                <div className="pt-2 flex gap-2">
                  <button
                    ...
                    className="w-1/2 bg-slate-100 text-slate-700 py-2.5 rounded-xl font-bold hover:bg-slate-200 transition cursor-pointer"
                  >
                    Batal
                  </button>
                  <button
                    type="submit"
                    className="w-1/2 bg-rose-600 text-white py-2.5 rounded-xl font-bold hover:bg-rose-700 transition cursor-pointer shadow-md shadow-rose-600/20 flex items-center justify-center gap-1.5"
                  >
                    <Send className="w-3.5 h-3.5" /> Kirim Sinyal
                  </button>
                </div>
              </form>
            )}
