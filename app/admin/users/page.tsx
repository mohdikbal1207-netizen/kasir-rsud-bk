...
export default function AdminVerificationPage() {
  ...
  // State Input Form Edit Data Pegawai
    ...
    try {
      ...
      const combinedData: SystemUser[] = usersData.map((user) => {
        ...
        return {
          ...user,
          profiles: profile,
          nama_lengkap: user.nama_lengkap || profile?.nama_lengkap || '',
          nik: user.nik || profile?.nik || '',
          nip: user.nip || profile?.nip || '',
          tempat_lahir: user.tempat_lahir || profile?.tempat_lahir || '',
  ...
  // Buka Modal Blokir Darurat Akun
  const openBlockModal = (user: SystemUser) => {
    setSelectedUser(user);
    setAdminNote('PERINGATAN KEAMANAN: Akses akun SIMRS Anda telah DIBLOKIR oleh Administrator RSUD Bukit Kerman karena terdeteksi aktivitas mencurigakan atau pelanggaran kebijakan sistem.');
    setModalMode('block');
  };
  ...
  const openEditModal = (user: SystemUser) => {
    setSelectedUser(user);
    setEditFormData({
      nama_lengkap: user.profiles?.nama_lengkap || user.nama_lengkap || '',
      nik: user.profiles?.nik || user.nik || '',
      nip: user.profiles?.nip || user.nip || '',
      tempat_lahir: user.profiles?.tempat_lahir || user.tempat_lahir || '',
      tanggal_lahir: user.profiles?.tanggal_lahir || user.tanggal_lahir || '',
      unit_kerja: user.profiles?.unit_kerja || user.unit_kerja || '',
      no_telepon: user.profiles?.no_telepon || user.no_telepon || '',
      role: user.role || 'kasir'
    });
    setModalMode('edit');
  };

  // Eksekusi Blokir Darurat Akun Staf
  const handleConfirmBlockUser = async () => {
    ...
    try {
      ...
      if (err instanceof Error) {
        setMessage({ type: 'error', text: `Gagal memblokir akun: ${err.message}` });
      } else {
        setMessage({ type: 'error', text: 'Terjadi kesalahan saat memblokir akun.' });
      }
    } finally {
  ...
  // Eksekusi Buka Blokir (Unblock / Pulihkan Akses)
  const handleUnblockUser = async (user: SystemUser) => {
    ...
    try {
      ...
      const phone = user.profiles?.no_telepon || user.no_telepon;
      const namaStaf = user.profiles?.nama_lengkap || user.nama_lengkap || 'Staf';
      ...
      if (err instanceof Error) {
        setMessage({ type: 'error', text: `Gagal membuka blokir: ${err.message}` });
      }
    } finally {
      setActionLoadingId(null);
    }
  ...
  const handleConfirmEditUser = async () => {
    ...
    try {
      ...
      if (err instanceof Error) {
        ...
        setMessage({ type: 'error', text: 'Terjadi kesalahan saat memperbarui data pegawai.' });
      }
    } finally {
      setIsSubmittingModal(false);
      setActionLoadingId(null);
    ...
    try {
      const { error: userError } = await supabase
        ...
        .update({ 
          ...
          unit_kerja: assignedRole.toUpperCase()
        })
        .eq('id', selectedUser.id);
  ...
  const handleRestoreUser = async (user: SystemUser) => {
    ...
    try {
      const { error: userError } = await supabase
        .from('users')
        .update({ role: 'kasir', is_active: false, unit_kerja: 'KASIR' })
        .eq('id', user.id);
  ...
  const handleDeletePermanent = async (user: SystemUser) => {
    const name = user.profiles?.nama_lengkap || user.nama_lengkap || 'Staf';
    if (!confirm(`Hapus permanen data atas nama "${name}" dari sistem?`)) return;
    setActionLoadingId(user.id);
    try {
  ...
  const exportToCSV = async () => {
    ...
    const rows = filteredUsers.map((u) => [
      ...
      `"${u.email || '-'}"`,
      `"${u.profiles?.nik || u.nik || '-'}"`,
      `"${u.profiles?.nip || u.nip || '-'}"`,
      `"${u.profiles?.unit_kerja || u.unit_kerja || u.role || '-'}"`,
      `"${u.profiles?.no_telepon || u.no_telepon || '-'}"`,
  ...
  const filteredUsers = currentList.filter((item) => {
    ...
    if (sortBy === 'newest') {
      ...
      const nameA = a.profiles?.nama_lengkap || a.nama_lengkap || '';
      const nameB = b.profiles?.nama_lengkap || b.nama_lengkap || '';
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between selection:bg-emerald-500 selection:text-white relative overflow-hidden">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center space-x-3.5">
                      ...
                      <div className="min-w-0 flex-1">
                        <h2 className="text-sm font-bold text-slate-900 truncate" title={namaLengkap}>
                          {namaLengkap}
                        </h2>
                        <p className="text-xs text-emerald-700 font-semibold truncate uppercase" title={unitKerja}>
                          {unitKerja}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="bg-slate-50 border border-slate-200/70 rounded-2xl p-3.5 space-y-2 text-xs text-slate-600">
                      <div className="flex items-center justify-between truncate">
                        ...
                        {noTelepon && (
                          <a
                            href={`https://wa.me/${formatPhoneForWA(noTelepon)}`}
                            target="_blank"
                      ...
                      <div className="grid grid-cols-2 gap-1 pt-1 border-t border-slate-200/60">
                        <div className="flex items-center space-x-1.5 truncate">
                          ...
                          <span className="truncate">NIK: <strong className="text-slate-800">{nik}</strong></span>
                        </div>
                        <div className="flex items-center space-x-1.5 truncate">
                          <FileBadge className="w-3.5 h-3.5 text-slate-400 flex-shrink-0" />
                          <span className="truncate">NIP: <strong className="text-slate-800">{nip}</strong></span>
                        </div>
                      </div>
                    ...
                    {activeTab === 'pending' && (
                      <>
                        ...
                        <button
                          ...
                          disabled={isActionProcessing}
                          className="bg-slate-100 hover:bg-emerald-50 border border-slate-200 hover:border-emerald-300 text-slate-700 hover:text-emerald-700 p-2.5 rounded-2xl transition flex items-center justify-center cursor-pointer shadow-sm"
                    ...
                    {activeTab === 'active' && (
                      <>
                        <button
                          onClick={() => openEditModal(item)}
                          disabled={isActionProcessing}
                          className="flex-1 bg-slate-100 hover:bg-emerald-50 border border-slate-200 hover:border-emerald-300 text-slate-700 hover:text-emerald-700 font-bold text-xs py-2.5 px-3 rounded-2xl transition flex items-center justify-center space-x-1.5 cursor-pointer shadow-sm"
                        >
                          <Edit3 className="w-4 h-4 text-emerald-600" />
                          <span>Edit Data</span>
                        </button>
                        ...
                        {/* TOMBOL BLOKIR / BUKA BLOKIR KEAMANAN DARURAT */}
                        {item.lockout_until && new Date(item.lockout_until) > new Date() ? (
                          <button
                            onClick={() => handleUnblockUser(item)}
                            disabled={isActionProcessing}
                            className="bg-amber-50 hover:bg-amber-100 border border-amber-300 text-amber-800 p-2.5 rounded-2xl transition flex items-center justify-center cursor-pointer shadow-sm"
                            title="Buka Blokir Akses Akun"
                          >
                          ...
                          <button
                            ...
                            disabled={isActionProcessing}
                            className="bg-rose-50 hover:bg-rose-100 border border-rose-300 text-rose-800 p-2.5 rounded-2xl transition flex items-center justify-center cursor-pointer shadow-sm"
                            title="Blokir Darurat Akses Staf via WA"
                          >
      ...
      {modalMode === 'block' && selectedUser && (
        <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-6 space-y-4 animate-in fade-in zoom-in duration-200 border border-slate-100">
            ...
            <div className="space-y-1">
              <h2 className="text-base font-black text-slate-900">{selectedUser.nama_lengkap || selectedUser.profiles?.nama_lengkap || 'Staf'}</h2>
              <p className="text-xs text-slate-500">Akun akan langsung terkunci dari sistem dan pesan peringatan keamanan terkirim otomatis ke WhatsApp.</p>
            </div>

            <div>
              <label className="text-[11px] font-bold text-slate-600 uppercase tracking-wider flex items-center space-x-1 mb-1.5">
                <MessageSquareText className="w-3.5 h-3.5 text-rose-600 mr-1" />
                <span>Pesan Peringatan Keamanan (WhatsApp)</span>
              </label>
              <textarea 
                rows={4}
                value={adminNote}
            ...
            <div className="flex items-center gap-3 pt-2">
              ...
              <button onClick={handleConfirmBlockUser} disabled={isSubmittingModal} className="flex-1 bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs py-3 rounded-2xl transition shadow-lg shadow-rose-600/20 flex items-center justify-center space-x-2 cursor-pointer disabled:opacity-50">
                {isSubmittingModal ? (
                  <div className="flex items-center space-x-2">
          ...
          <div className="bg-white rounded-3xl shadow-2xl max-w-lg w-full p-6 space-y-4 animate-in fade-in zoom-in duration-200 border border-slate-100 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="inline-flex items-center space-x-2 bg-emerald-50 text-emerald-700 px-3 py-1 rounded-xl text-xs font-bold">
                <Edit3 className="w-4 h-4 text-emerald-600" />
                <span>Edit Informasi Data Pegawai</span>
              </div>
              <button onClick={() => setModalMode(null)} className="w-8 h-8 rounded-full bg-slate-100 hover:bg-slate-200 flex items-center justify-center text-slate-500 transition cursor-pointer">
                <X className="w-4 h-4" />
              </button>
            ...
            <div className="space-y-3 text-xs">
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-slate-50 border border-slate-300 rounded-xl px-3 py-2 text-slate-800 font-medium focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
                />
              </div>
              ...
              <div>
                ...
                <select 
                  ...
                  className="w-full bg-slate-50 border border-slate-300 rounded-xl px-3 py-2 text-slate-800 font-bold focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
                >
                  <option value="kasir">KASIR</option>
                  <option value="manajemen">MANAJEMEN</option>
            ...
            <div className="flex items-center gap-3 pt-3 border-t border-slate-100">
              <button onClick={() => setModalMode(null)} disabled={isSubmittingModal} className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs py-3 rounded-2xl transition cursor-pointer">Batal</button>
              <button onClick={handleConfirmEditUser} disabled={isSubmittingModal} className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs py-3 rounded-2xl transition shadow-lg shadow-emerald-600/20 flex items-center justify-center space-x-2 cursor-pointer disabled:opacity-50">
                {isSubmittingModal ? (
                  <div className="flex items-center space-x-2">
                    <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
                    <span>Simpan...</span>
      ...
      {/* MODAL DETAIL DOKUMEN & BERKAS PENDUKUNG */}
          ...
          <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full p-6 space-y-5 animate-in fade-in zoom-in duration-200 border border-slate-100 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h2 className="text-base font-black text-slate-900">{selectedUser.nama_lengkap || selectedUser.profiles?.nama_lengkap || 'Tanpa Nama'}</h2>
                  <p className="text-xs text-emerald-700 font-bold uppercase">{selectedUser.unit_kerja || selectedUser.profiles?.unit_kerja || selectedUser.role || 'Staf SIMRS'}</p>
                  <p className="text-[11px] text-slate-400 font-mono mt-0.5">ID Akun: {selectedUser.id}</p>
                </div>
              </div>

              {(selectedUser.no_telepon || selectedUser.profiles?.no_telepon) && (
                <a
                  href={`https://wa.me/${formatPhoneForWA(selectedUser.no_telepon || selectedUser.profiles?.no_telepon)}`}
                  target="_blank"
                  rel="noopener noreferrer"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
              <div className="bg-slate-50 p-4 rounded-2xl space-y-2 border border-slate-200/60">
                ...
                <p><strong>NIK:</strong> {selectedUser.nik || selectedUser.profiles?.nik || '-'}</p>
                <p><strong>NIP:</strong> {selectedUser.nip || selectedUser.profiles?.nip || '-'}</p>
                <p><strong>TTL:</strong> {selectedUser.tempat_lahir || selectedUser.profiles?.tempat_lahir || '-'}, {selectedUser.tanggal_lahir || selectedUser.profiles?.tanggal_lahir || '-'}</p>
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="bg-slate-900 rounded-2xl p-4 text-center text-slate-300 text-xs flex flex-col items-center justify-center space-y-2 min-h-[160px]">
                {selectedUser.foto_url || selectedUser.profiles?.foto_url ? (
                  <div className="relative w-full h-48 rounded-xl overflow-hidden border border-slate-700">
                    <img src={selectedUser.foto_url || selectedUser.profiles?.foto_url || ''} alt="Dokumen Pratinjau" className="w-full h-full object-contain" />
                  </div>
                ) : (
                  <>
