        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 print:hidden">
          <div className="bg-white rounded-3xl max-w-2xl w-full p-6 sm:p-8 shadow-2xl border border-slate-200 space-y-4 max-h-[90vh] flex flex-col justify-between">
            ...
            <div className="overflow-y-auto space-y-4 pr-1 flex-1">
              {/* BANNER STATUS VERIFIKASI ADMIN */}
            ...
            <div className="bg-amber-50/40 border border-amber-200/80 rounded-2xl p-5 space-y-3 font-mono text-xs text-slate-800">
              ...
              <div className="pt-2 text-center">
                <span className={`inline-block px-3 py-1 rounded-full text-[10px] font-bold uppercase font-sans ${
                  receiptRecord.status_bayar === 'lunas' ? 'bg-emerald-100 text-emerald-800 border border-emerald-300' : 
                  (receiptRecord.status_bayar === 'ditolak' || receiptRecord.status_bayar === 'dibatalkan' || String(receiptRecord.status_verifikasi).toLowerCase() === 'ditolak') ? 'bg-rose-100 text-rose-800 border border-rose-300 font-black' : 'bg-amber-100 text-amber-800 border border-amber-300'
                }`}>
                  Status: {
                    receiptRecord.status_bayar === 'lunas' ? 'LUNAS (TERVERIFIKASI)' : 
                    (receiptRecord.status_bayar === 'ditolak' || receiptRecord.status_bayar === 'dibatalkan' || String(receiptRecord.status_verifikasi).toLowerCase() === 'ditolak') ? 'DITOLAK / DIBATALKAN' : 'MENUNGGU VERIFIKASI ADMIN'
                  }
                </span>
              </div>
            </div>
