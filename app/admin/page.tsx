...
export default function AdminDashboardPage() {
  ...
  // State untuk Status Realtime Maintenance Mode
  ...
  const [targetLogoutUser, setTargetLogoutUser] = useState<{ id: string; nama: string } | null>(null);
  const [lastUpdatedTime, setLastUpdatedTime] = useState<string>('');

  const router = useRouter();

  // 1. Hitung jumlah data yang masih pending verifikasi akun pegawai & transaksi kasir + Status Maintenance
  const fetchCounts = async () => {
    try {
      ...
      // Ambil Status Realtime Maintenance Mode
      ...
      if (settingsData?.value) {
        setIsMaintenanceActive(Boolean(settingsData.value.is_active));
      }
  ...
  // 3. Catat / Perbarui sesi aktif Admin secara otomatis & Kirim Notifikasi WhatsApp Fonnte
  useEffect(() => {
    async function recordAdminSession() {
      try {
        ...
        // Upsert sesi ke tabel user_sessions (memanfaatkan Unique Constraint pada user_id)
        await supabase
          .from('user_sessions')
          .upsert({
            user_id: userId,
        ...
        // INTEGRASI BARU: Kirim Peringatan WhatsApp via Fonnte saat Admin Login
        const notificationKey = `rsud_admin_notified_${userId}`;
        if (!sessionStorage.getItem(notificationKey)) {
          try {
            await fetch('/api/admin/notify-login', {
              method: 'POST',
  ...
  const handleForceLogout = async (userId: string, nama: string) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.error || result.message || 'Gagal mengakhiri sesi.');
  ...
  const filteredSessions = useMemo(() => {
    if (!sessionSearchTerm.trim()) return activeSessions;
    const term = sessionSearchTerm.toLowerCase();
    return activeSessions.filter(s => 
      (s.nama_lengkap || '').toLowerCase().includes(term) ||
      (s.role || '').toLowerCase().includes(term)
    );
  }, [activeSessions, sessionSearchTerm]);

  const totalPendingTasks = pendingCount + rincianObatCount;
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between selection:bg-emerald-500 selection:text-white relative overflow-hidden">
      ...
      <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 flex-1 space-y-8 relative z-10">
        ...
        {/* Ringkasan Eksekutif Metrics Bar */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-white border border-slate-200/80 p-5 rounded-3xl shadow-sm flex items-center space-x-4">
            <div className="p-3 bg-amber-50 text-amber-600 rounded-2xl border border-amber-200">
              <AlertCircle className="w-6 h-6" />
            </div>
            <div>
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">Verifikasi Menunggu</span>
              <span className="text-2xl font-black text-slate-900">{totalPendingTasks} Berkas</span>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200/80 p-5 rounded-3xl shadow-sm flex items-center space-x-4">
            ...
            <div>
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">Staf Online Aktif</span>
              <span className="text-2xl font-black text-slate-900">{activeSessions.length} Pegawai</span>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200/80 p-5 rounded-3xl shadow-sm flex items-center space-x-4">
            ...
            <div>
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">Shift Kasir Aktif</span>
              <span className="text-2xl font-black text-slate-900">{absensiPendingCount} Petugas</span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            {/* 8. Modul Pengumuman & Alert System */}
            <div 
            ...
            {/* 9. Modul Kontrol Maintenance Mode */}
            <div 
              onClick={() => router.push('/admin/maintenance')}
              className="bg-white border border-amber-200/90 rounded-3xl p-6 shadow-lg shadow-amber-100/50 flex flex-col justify-between space-y-6 transition hover:shadow-xl hover:border-amber-400 cursor-pointer group relative overflow-hidden"
            >
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="w-12 h-12 bg-amber-50 border border-amber-200 rounded-2xl text-amber-600 flex items-center justify-center shadow-sm group-hover:scale-105 transition">
                  ...
                  {isMaintenanceActive ? (
                    <span className="bg-amber-500 text-white font-bold text-[10px] px-2.5 py-1 rounded-full shadow-sm animate-pulse flex items-center gap-1">
                      <ShieldAlert className="w-3 h-3" />
                      <span>Aktif (Terkunci)</span>
                    </span>
                  ) : (
                    <span className="bg-slate-100 text-slate-600 font-bold text-[10px] px-2.5 py-1 rounded-full border border-slate-200">
                      Normal Mode
                    </span>
                  )}
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-slate-500 leading-relaxed">
                    Atur status perbaikan sistem, pesan pengumuman, dan daftar email bypass.
                  </p>
                </div>
              </div>
            ...
            {/* 10. Modul Pusat Keamanan IP & Anti Brute-Force */}
            <div 
              onClick={() => router.push('/admin/security')}
              className="bg-white border border-rose-200/90 rounded-3xl p-6 shadow-lg shadow-rose-100/50 flex flex-col justify-between space-y-6 transition hover:shadow-xl hover:border-rose-400 cursor-pointer group relative overflow-hidden"
            >
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="w-12 h-12 bg-rose-50 border border-rose-200 rounded-2xl text-rose-600 flex items-center justify-center shadow-sm group-hover:scale-105 transition">
                  ...
                  <span className="bg-rose-500 text-white font-bold text-[10px] px-2.5 py-1 rounded-full shadow-sm flex items-center gap-1">
                    <ShieldCheck className="w-3 h-3" />
                    <span>Auto-Blacklist</span>
                  </span>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-slate-500 leading-relaxed">
                    Pantau alamat IP mencurigakan, log percobaan login gagal, dan buka blokir manual.
                  </p>
                </div>
              </div>

              <div className="pt-4 border-t border-slate-100 flex items-center justify-between text-xs font-bold text-rose-600 group-hover:translate-x-1 transition-transform">
                <span>Buka Pusat Keamanan</span>
                <ArrowRight className="w-4 h-4" />
              </div>
            </div>

            {/* 11. Modul Jejak Audit Database (Audit Logs) */}
            <div 
              onClick={() => router.push('/admin/audit-logs')}
              className="bg-white border border-emerald-200/90 rounded-3xl p-6 shadow-lg shadow-emerald-100/50 flex flex-col justify-between space-y-6 transition hover:shadow-xl hover:border-emerald-400 cursor-pointer group relative overflow-hidden"
            >
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="w-12 h-12 bg-emerald-50 border border-emerald-200 rounded-2xl text-emerald-600 flex items-center justify-center shadow-sm group-hover:scale-105 transition">
                ...
                <div className="space-y-1">
                  <h3 className="text-base font-bold text-slate-900 group-hover:text-emerald-700 transition">
                    Jejak Audit Database
                  </h3>
                  <p className="text-xs text-slate-500 leading-relaxed">
                    Pantau rekaman otomatis perubahan data, INSERT, UPDATE, dan DELETE di tingkat database.
                  </p>
                </div>
              </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="space-y-1">
              <div className="flex items-center space-x-2">
                ...
                {lastUpdatedTime && (
                  <span className="text-[10px] font-mono bg-slate-100 text-slate-500 px-2 py-0.5 rounded-lg border border-slate-200">
            ...
            <div className="flex items-center space-x-3 w-full sm:w-auto">
              <div className="relative flex-1 sm:w-64">
                ...
                <input 
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-9 pr-3 py-2 text-xs focus:outline-none focus:border-emerald-500 transition"
                />
              </div>
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-100 font-medium">
                {isSessionsLoading ? (
                  ...
                  <tr>
                    <td colSpan={4} className="text-center py-8 text-slate-400">
                      {sessionSearchTerm ? 'Tidak ditemukan staf aktif dengan kata kunci tersebut.' : 'Tidak ada staf yang sedang aktif atau tercatat masuk saat ini.'}
                    </td>
                  </tr>
                ) : (
                  filteredSessions.map((session) => (
                    <tr key={session.id} className="hover:bg-slate-50/70 transition">
                      ...
                      <td className="p-3.5 text-center">
                        <button
                          onClick={() => setTargetLogoutUser({ id: session.user_id, nama: session.nama_lengkap || 'Staf' })}
                          disabled={actionLoadingId === session.user_id}
                          className="inline-flex items-center space-x-1.5 bg-rose-50 hover:bg-rose-100 text-rose-700 font-bold px-3 py-1.5 rounded-xl transition cursor-pointer disabled:opacity-50 shadow-sm active:scale-95"
                        >
      ...
      {/* Modal Konfirmasi Force Logout Kustom */}
      {targetLogoutUser && (
        <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-white border border-slate-200 p-6 rounded-3xl shadow-2xl w-full max-w-sm text-center space-y-4">
            ...
            <div>
              <h4 className="font-bold text-sm text-slate-900">Konfirmasi Akhiri Sesi</h4>
              <p className="text-xs text-slate-500 mt-1 leading-relaxed">
                Apakah Anda yakin ingin mengakhiri sesi login secara paksa untuk <strong className="text-slate-800">{targetLogoutUser.nama}</strong>?
              </p>
            </div>
            <div className="flex gap-3 pt-2">
              <button
                type="button"
                onClick={() => setTargetLogoutUser(null)}
              ...
              <button
                type="button"
                onClick={() => handleForceLogout(targetLogoutUser.id, targetLogoutUser.nama)}
                className="w-1/2 bg-rose-600 hover:bg-rose-700 text-white py-2.5 rounded-xl text-xs font-bold transition cursor-pointer shadow-lg shadow-rose-600/20"
              >
                Ya, Akhiri Sesi
              </button>
            </div>
