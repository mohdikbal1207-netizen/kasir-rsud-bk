...
export default function KasirLayout({ children }: { children: React.ReactNode }) {
  ...
  // Pengelola Identitas & Quick Lock Screen
  const [activeUserEmail, setActiveUserEmail] = useState<string>('');
  const [activeUserRole, setActiveUserRole] = useState<string>('Kasir SIMRS');
  ...
  // Loading State & Durasi Sesi & Latency DB
  ...
  const ingestAuditLog = async (action: string, targetId: string, details: string) => {
    try {
      ...
      const userEmail = session?.user?.email || 'system_kasir';
  ...
  const handleAutoLogout = useCallback(async () => {
    try {
      ...
      await ingestAuditLog('KASIR_AUTO_LOGOUT', activeUserEmail, 'Session expired due to user inactivity.');
  ...
  const resetInactivityTimers = useCallback(async () => {
    ...
    }, 25 * 60 * 1000); // Alert setelah 25 menit inaktif

    inactivityTimerRef.current = setTimeout(() => {
      handleAutoLogout();
    }, 30 * 60 * 1000); // Logout otomatis setelah 30 menit
  ...
  useEffect(() => {
    ...
    // [OPTIMASI EGRESS]: Interval diperpanjang dari 5s ke 15s guna menghindari lonjakan egress query database berlebih
    const revocationInterval = setInterval(checkRemoteRevocation, 15000);

    // Realtime subscription ke tabel user_sessions untuk respons seketika saat admin melakukan revoke
    const channel = supabase
      .channel('kasir-session-revocation-sync')
      .on(
        'postgres_changes',
        {
          event: 'UPDATE',
  ...
  // BroadcastChannel Sync Antar-Tab Kasir
  useEffect(() => {
    if (typeof window !== 'undefined' && 'BroadcastChannel' in window) {
      broadcastChannelRef.current = new BroadcastChannel('simrs_kasir_session_channel');
  ...
  useEffect(() => {
    const handleKeyDownShortcuts = (e: KeyboardEvent) => {
      if ((e.ctrlKey || e.metaKey || e.altKey) && e.key.toLowerCase() === 'l') {
        ...
        playAlertSound('lock');
        triggerToast('Layar Kasir berhasil terkunci instan.');
  ...
  // Verifikasi Hak Akses Kasir (Logika Utama Asli Tetap Dijaga)
  useEffect(() => {
    async function verifyKasirAccess() {
      try {
        ...
        if (userRole.includes('admin')) {
          setActiveUserRole('Administrator (Mode Kasir)');
        } else {
          setActiveUserRole('Petugas Kasir SIMRS');
        }

        if (userRole !== 'kasir' && !userRole.includes('admin')) {
          await supabase.auth.signOut();
  ...
  // Pengamanan Unlock dengan Proteksi Rate Limiting & Account Lockout Sinkron
  const handleUnlockScreen = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Cek status Lockout di tabel public.users sebelum autentikasi
      ...
      if (error) {
        ...
        if (activeUserEmail !== 'mohdikbal1207@gmail.com') {
          ...
          if (currentData) {
            ...
            if (newAttempts >= 4) {
              setLockError('⚠️ Terlalu banyak percobaan salah. Akun terkunci 15 menit.');
        ...
        triggerToast('Layar Kasir berhasil dibuka kembali.');
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center text-white space-y-4 select-none">
        ...
        <div className="text-center space-y-1">
          ...
          <p className="text-[11px] text-slate-400">Memverifikasi Hak Akses Modul Kasir...</p>
        </div>
      </div>
    );
  }
    ...
    <>
      ...
      {!isOnline && (
        <div className="bg-rose-600 text-white text-center py-2 px-4 text-xs font-bold flex items-center justify-center gap-2 sticky top-0 z-50 shadow-lg animate-pulse">
          ...
          <span>Koneksi Internet Terputus! Sesi kasir berpotensi terkendala saat cetak kwitansi.</span>
        </div>
      )}
      ...
      {/* Floating Status Bar Kasir (Tampil di Desktop) */}
      <div className="fixed bottom-4 left-4 z-40 hidden lg:flex items-center gap-3 bg-slate-900/90 border border-slate-800 backdrop-blur-md px-3.5 py-2 rounded-2xl shadow-2xl text-[11px] text-slate-300 font-mono print:hidden selection:bg-none">
        <span className="flex h-2 w-2 relative" title={isChannelConnected ? "Sync Tab Kasir Aktif" : "Sync Standalone"}>
          <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
          <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
        </span>

        <span className="font-bold text-slate-200 flex items-center gap-1.5">
          <Receipt className="w-3.5 h-3.5 text-emerald-400" /> Kasir Node Active
        </span>
        ...
        <button 
          ...
          onClick={() => {
            ...
            playAlertSound('lock');
            triggerToast('Layar Kasir terkunci.');
      ...
      {/* Modal Peringatan Sesi Inaktif Kasir */}
      {showTimeoutWarning && !isQuickLocked && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-slate-900 border border-amber-500/30 p-6 rounded-3xl shadow-2xl w-full max-w-sm text-slate-100 space-y-4 text-center">
            ...
            <div>
              <h4 className="font-black text-sm text-white">Sesi Kasir Akan Berakhir</h4>
              <p className="text-[11px] text-slate-400 mt-0.5">Sistem mendeteksi tidak ada aktivitas transaksi</p>
            </div>
            ...
            <p className="text-xs text-slate-300 leading-relaxed">
              Tekan tombol di bawah ini untuk melanjutkan pengoperasian Modul Kasir.
            </p>
            ...
            <button 
              ...
              onClick={() => {
                resetInactivityTimers();
                playAlertSound('success');
                triggerToast('Sesi Kasir berhasil diperpanjang.');
              }}
            ...
            >
              ...
              <span>Lanjutkan Sesi Kasir</span>
            </button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/95 backdrop-blur-xl flex flex-col items-center justify-center p-4 text-white animate-in fade-in">
          <div className="bg-slate-900/90 border border-slate-800 p-8 rounded-3xl shadow-2xl w-full max-w-md space-y-6 text-center backdrop-blur-md relative overflow-hidden">
            ...
            <div className="space-y-1">
              ...
              <h3 className="text-lg font-black text-white">Layar Kasir Terkunci</h3>
      ...
      {/* Modal Konfirmasi Logout Manual Kasir */}
      {showLogoutConfirm && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl shadow-2xl w-full max-w-sm text-center space-y-4">
            ...
            <div>
              <h4 className="font-bold text-sm text-white">Konfirmasi Keluar Kasir</h4>
              <p className="text-xs text-slate-400 mt-1">Apakah Anda yakin ingin mengakhiri sesi Kasir ini?</p>
            </div>
