...
import { 
  ...
  Layers, FileSpreadsheet, Download, Calculator, Banknote
} from 'lucide-react';
...
export default function InputRawatInapPage() {
  ...
  const [searchingRm, setSearchingRm] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    const savedDraft = localStorage.getItem('ranap_input_draft');
    if (savedDraft) {
      try {
        const parsed = JSON.parse(savedDraft);
        if (parsed && parsed.namaPasien) {
          setPatientData(prev => ({ ...prev, ...parsed.patientData }));
          if (parsed.billingItems) setBillingItems(parsed.billingItems);
        }
      } catch (e) {
  ...
  useEffect(() => {
    if (!isFormLocked && patientData.statusVerifikasi === 'NEW') {
      localStorage.setItem('ranap_input_draft', JSON.stringify({ patientData, billingItems }));
    }
  }, [patientData, billingItems, isFormLocked]);
  ...
  const handleExportCSV = () => {
    if (filteredBillings.length === 0) {
      setModalNotif({
        ...
        type: 'warning',
        title: 'Data Kosong',
        message: 'Tidak ada data rekapitulasi yang dapat diekspor sesuai filter saat ini.'
      });
      return;
    }

    let csvContent = "data:text/csv;charset=utf-8,";
    csvContent += "No. Reg,Nama Pasien,NIK,Ruang Perawatan,Jenis Penjaminan,Metode Pembayaran,Status,Total Biaya (Rp),Ditanggung BPJS (Rp),Wajib Bayar Umum (Rp)\n";
    ...
    filteredBillings.forEach(b => {
      const row = [
        ...
        b.jenis_penjaminan,
        b.metode_pembayaran,
        b.status_verifikasi || 'PENDING_VERIFIKASI',
        b.total_biaya || 0,
        b.total_ditanggung || 0,
        b.total_selisih || 0
      ].join(",");
      csvContent += row + "\n";
    });
    ...
    link.setAttribute("download", `Rekap_Billing_Rawat_Inap_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const handleCariPasienByRM = async () => {
    if (!patientData.noReg || patientData.noReg.trim().length < 3) {
      setModalNotif({
        show: true,
        type: 'warning',
        title: 'No. RM Belum Valid',
        message: 'Masukkan Nomor RM / No. Registrasi yang valid untuk mencari data kunjungan sebelumnya.'
      });
      return;
    }
    ...
    try {
      const { data, error } = await supabase
        .from('ranap_billing_header')
        .select('*')
        .eq('no_reg', patientData.noReg.trim())
        .order('created_at', { ascending: false })
        .limit(1);
      ...
      if (data && data.length > 0) {
        ...
        setPatientData(prev => ({
          ...
          nikPasien: pasienLama.nik_pasien || '',
        ...
        setModalNotif({
          ...
          message: `Berhasil memuat data dari No. RM ${pasienLama.no_reg} atas nama ${pasienLama.nama_pasien}.`
        });
      } else {
        setModalNotif({
          show: true,
          type: 'info',
          title: 'No. RM Baru',
          message: 'Tidak ditemukan riwayat kunjungan dengan Nomor RM tersebut di database.'
        });
      }
    } catch (err: any) {
      console.error('Gagal mencari No. RM:', err);
      setModalNotif({
        show: true,
        type: 'error',
  ...
  const nominalBayarTunai = parseNumber(bayarTunai);
  const kembalianTunai = nominalBayarTunai > totalSelisih ? nominalBayarTunai - totalSelisih : 0;
  ...
  const handleSubmitToDatabase = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.removeItem('ranap_input_draft');
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        <div className="flex items-center justify-between border-b border-slate-200 pb-3 print:hidden">
          ...
          {activeTab === 'input' && !isFormLocked && (
            <div className="hidden sm:flex items-center space-x-1.5 px-3 py-1.5 bg-emerald-50 text-emerald-700 rounded-xl text-[11px] font-bold border border-emerald-200">
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-ping"></span>
              <span>Auto-Save Draft Aktif</span>
            </div>
          )}
        </div>
        ...
        {/* TAB 1: FORMULIR INPUT RAWAT INAP */}
        {activeTab === 'input' && (
          <div className="bg-white rounded-3xl shadow-xl border border-slate-200 p-6 sm:p-8 space-y-6 animate-fade-in">
              ...
              <div className="bg-emerald-50 border-2 border-emerald-300 rounded-2xl p-4 flex items-center space-x-3 text-emerald-900 shadow-sm print:hidden">
                ...
                <div>
                  <span className="font-black text-xs uppercase bg-emerald-200 text-emerald-900 px-2 py-0.5 rounded-md border border-emerald-300">
                    FORMULIR SIAP DIISI / INPUT BARU (AUTO-SAVE AKTIF)
                  </span>
                  <p className="text-xs font-medium text-emerald-800 mt-1">
                    Progress input Anda tersimpan otomatis di browser. Masukkan Nomor RM / No. Reg untuk mencari riwayat kunjungan pasien.
                  </p>
                </div>
              </div>
            )}
            ...
            <form onSubmit={handleSubmitToDatabase} className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 bg-slate-50 p-5 rounded-2xl border border-slate-200 print:bg-transparent print:border-none print:p-0">
                <div>
                  <label className="block text-xs font-bold text-slate-600 mb-1">No. Registrasi / RM (Cari Riwayat)</label>
                  <div className="flex space-x-1">
                    <input 
                      ...
                      className="w-full px-3 py-2 rounded-xl border border-slate-300 text-xs font-mono font-bold bg-white text-slate-900 disabled:bg-slate-100"
                    />
                    {!isFormLocked && (
                      <button
                        type="button"
                        onClick={handleCariPasienByRM}
                        disabled={searchingRm}
                        ...
                        title="Cari riwayat berdasarkan Nomor RM"
                      >
                        <UserSearch className={`w-4 h-4 ${searchingRm ? 'animate-spin' : ''}`} />
                      </button>
                    )}
                  </div>
                </div>
              ...
              {!isFormLocked && patientData.metodePembayaran.toLowerCase().includes('tunai') && totalSelisih > 0 && (
                <div className="bg-gradient-to-r from-emerald-900 to-slate-900 text-white rounded-3xl p-5 shadow-lg border border-emerald-600/40 flex flex-col md:flex-row items-center justify-between gap-4 print:hidden">
                  <div className="flex items-center space-x-3">
                    <div className="w-12 h-12 bg-emerald-700/60 rounded-2xl flex items-center justify-center">
                      <Calculator className="w-6 h-6 text-emerald-300" />
                    </div>
                    <div>
                      <h4 className="text-xs font-black uppercase tracking-wider text-emerald-300">Kalkulator Kembalian Kasir</h4>
                      <p className="text-xs text-slate-300">Total tagihan umum yang harus dibayar tunai: <strong className="text-white font-mono">Rp {formatNumber(totalSelisih)}</strong></p>
                    </div>
                  </div>

                  <div className="flex items-center space-x-3 w-full md:w-auto">
                    <div>
                      <label className="block text-[10px] font-bold uppercase text-slate-300 mb-1">Uang Diterima Dari Pasien (Rp)</label>
                      <input
                        type="text"
                        value={bayarTunai}
                        onChange={(e) => setbayarTunai(e.target.value)}
                        placeholder="Contoh: 500000"
                        className="px-3 py-2 bg-white text-slate-900 rounded-xl text-xs font-mono font-bold w-44 border border-emerald-400"
                      />
                    </div>
                    <div className="bg-emerald-800/80 px-4 py-2 rounded-xl border border-emerald-500/50">
                      <span className="block text-[10px] font-bold uppercase text-emerald-200">Uang Kembalian</span>
                      <span className="text-sm font-mono font-black text-white">Rp {formatNumber(kembalianTunai)}</span>
                    </div>
                  </div>
                </div>
              )}
      ...
      {activeTab === 'input' && !isFormLocked && (
        <div className="sticky bottom-4 z-40 max-w-5xl mx-auto w-full bg-slate-900/95 backdrop-blur-md text-white px-6 py-4 rounded-2xl shadow-2xl border border-emerald-500/50 flex flex-wrap items-center justify-between gap-4 print:hidden animate-fade-in">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <span className="text-[10px] text-slate-400 font-bold uppercase block">Ringkasan Sesi Aktif</span>
              <span className="text-xs font-bold text-emerald-400 uppercase">{patientData.namaPasien || 'Belum ada nama pasien'}</span>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-6 text-xs font-mono">
            <div>
              ...
              <span className="font-black text-white text-sm">Rp {formatNumber(totalJumlah)}</span>
            </div>
            <div className="hidden sm:block border-l border-slate-700 pl-4">
              <span className="text-slate-400 block text-[10px]">DITANGGUNG BPJS</span>
              <span className="font-semibold text-emerald-400 text-sm">Rp {formatNumber(totalDitanggung)}</span>
            </div>
            <div className="border-l border-slate-700 pl-4">
              <span className="text-slate-400 block text-[10px]">SISA BAYAR UMUM</span>
              <span className="font-black text-rose-400 text-sm">Rp {formatNumber(totalSelisih)}</span>
            </div>
          </div>
          ...
          <button
            ...
            disabled={loading}
            className="bg-emerald-600 hover:bg-emerald-500 text-white px-6 py-2.5 rounded-xl text-xs font-bold transition shadow-lg cursor-pointer flex items-center space-x-2"
          >
            <Save className="w-4 h-4" />
            <span>{loading ? 'Mengirim...' : 'Kirim Tagihan'}</span>
          </button>
        </div>
      )}
