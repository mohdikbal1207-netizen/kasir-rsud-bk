...
function LoginFormContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // PERBAIKAN: Cari profil di tabel `users` berdasarkan EMAIL utama (dan id sebagai cadangan)
      ...
      if (!userData) {
        // Fallback pencarian dengan ID jika email tidak cocok
      ...
      // Jalur khusus untuk Super Admin Utama
      const isSuperAdmin = loginEmail === 'mohdikbal1207@gmail.com';
      ...
      // Penentuan role dan nama lengkap
      const userRole = isSuperAdmin ? 'admin' : (userData?.role?.toLowerCase() || 'kasir');
      const namaLengkap = userData?.nama_lengkap || (isSuperAdmin ? 'Administrator' : 'Staf RSUD');
      const isActiveStatus = isSuperAdmin ? true : (userData?.is_active ?? true);
      ...
      // 4. Catat Sesi Aktif ke Tabel `user_sessions`
      try {
