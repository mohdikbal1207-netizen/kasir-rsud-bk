...
function LoginFormInner() {
  ...
  // HANDLER LOGIN DENGAN PENCEGAHAN SILENT FAIL
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setErrorMessage(null);
    setSuccessMessage(null);
    setNoticeMessage(null);
    setDebugLog('Memulai autentikasi...');
    ...
    try {
      setLoadingStepText('Menghubungkan ke Supabase Auth...');
      ...
      if (authError) {
        setDebugLog(`Auth Error Code: ${authError.status} | Msg: ${authError.message}`);
        throw new Error(
          authError.message === 'Invalid login credentials' 
            ? 'Password atau email salah. Pastikan user terdaftar di Supabase Authentication > Users.' 
            : authError.message
        );
      }

      if (!authData?.user) {
        throw new Error('Sesi Auth tidak mengembalikan data pengguna.');
      }

      setDebugLog(`Auth Berhasil! User ID: ${authData.user.id}`);
      setLoadingStepText('Menyiapkan Sesi Pengguna...');

      // 2. Tentukan Jalur Redirect
      let targetUrl = '/kasir';
      
      if (redirectTo) {
        targetUrl = redirectTo;
      } else if (inputEmail === 'mohdikbal1207@gmail.com') {
        targetUrl = '/admin';
      }

      setSuccessMessage(`Login Berhasil! Mengalihkan ke ${targetUrl}...`);

      // Gunakan router.push & router.refresh agar cookie Next.js ter-update
      setTimeout(() => {
          ...
          <div className="w-full lg:col-span-6 flex items-center justify-center">
            <div className="w-full max-w-md space-y-4">
              ...
              <div className={`${isDarkMode ? 'bg-slate-900 border-slate-800 text-slate-100 shadow-slate-950/80' : 'bg-white border-slate-200 text-slate-800 shadow-xl'} border rounded-3xl p-6 sm:p-8 backdrop-blur-xl relative overflow-hidden transition-transform ${shakeCard ? 'animate-bounce' : ''}`}>
                ...
                {debugLog && (
                  <div className="mb-4 p-2.5 bg-slate-100 border border-slate-300 font-mono text-[10px] text-slate-700 rounded-xl overflow-x-auto">
                    {debugLog}
                  </div>
                )}
