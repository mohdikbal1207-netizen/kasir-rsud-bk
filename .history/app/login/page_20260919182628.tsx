...
function LoginFormContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Normalisasi input email secara bersih
      const rawInput = emailOrUsername.trim().toLowerCase();
      const loginEmail = rawInput.includes('@') ? rawInput : `${rawInput}@gmail.com`;
      ...
      // 2. Ambil data dari tabel `users` berdasarkan UUID id dari Supabase Auth
      ...
      // Fallback pencarian berdasarkan email jika ID belum sinkron
      ...
      // 3. Validasi Keberadaan Akun
      if (!finalUserData) {
        // Khusus Super Admin utama jika belum terdaftar di tabel public.users
        if (loginEmail === 'mohdikbal1207@gmail.com') {
          finalUserData = {
            role: 'admin',
            is_active: true,
            nama_lengkap: 'mohd ikbal'
          };
        } else {
          await supabase.auth.signOut();
          throw new Error('Data profil pengguna tidak ditemukan di tabel database SIMRS.');
        }
      }

      // 4. Validasi status keaktifan akun (is_active)
      ...
      // 5. Catat Sesi Aktif ke Tabel `user_sessions`
      try {
        await supabase.from('user_sessions').insert([
          {
            user_id: authData.user.id,
            nama_lengkap: finalUserData.nama_lengkap || 'Staf RSUD',
            role: finalUserData.role || 'staf',
            is_active: true
          }
        ]);
      } catch (sessionErr) {
        console.warn('Sesi tidak dapat dicatat ke user_sessions:', sessionErr);
      }

      setLoadingStepText('Menyiapkan Sesi Pengguna...');
      setSuccessMessage(`Login berhasil! Selamat datang, ${finalUserData.nama_lengkap || 'Staf'}. Mengalihkan...`);

      // 6. Pengarahan mutlak berdasarkan role
      const userRole = (finalUserData.role || '').toLowerCase();
