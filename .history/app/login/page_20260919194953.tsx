...
function LoginFormInner() {
  ...
  // ================= FITUR BARU: PEREKAM GAGAL LOGIN IP (BRUTE-FORCE PROTECTION) =================
  const recordFailedIpToDatabase = async () => {
    try {
      ...
      if (clientIp) {
        await supabase.rpc('record_failed_login_ip', { target_ip: clientIp });
      }
    } catch (err) {
      console.error('Gagal mencatat IP gagal login:', err);
    }
  };
  // =========================================================================================
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!cleanInput || !cleanPassword) {
      triggerError('Email dan Kata Sandi wajib diisi!');
      return;
    }
    ...
    try {
      ...
      if (rememberMe) {
        localStorage.setItem('rsud_saved_login_id', cleanInput);
      } else {
        localStorage.removeItem('rsud_saved_login_id');
      }

      // 1. Cek Data User & Status Lockout di Tabel public.users (Kecuali Super Admin Utama)
      if (inputEmail !== 'mohdikbal1207@gmail.com') {
        const { data: userData, error: userCheckError } = await supabase
          .from('users')
          .select('failed_attempts, lockout_until, is_active')
          .eq('email', inputEmail)
          .maybeSingle();

        if (userData) {
          // Cek Apakah Akun Sedang Terkunci (Lockout Aktif)
          if (userData.lockout_until && new Date(userData.lockout_until) > new Date()) {
            const remainingMinutes = Math.ceil((new Date(userData.lockout_until).getTime() - new Date().getTime()) / 60000);
            await recordFailedIpToDatabase();
            throw new Error(`⚠️ Akun terkunci sementara karena terlalu banyak percobaan gagal. Silakan coba lagi dalam ${remainingMinutes} menit.`);
          }
      ...
      if (authError) {
        // Catat kegagalan IP ke sistem keamanan backend database
        await recordFailedIpToDatabase();

        // Jika Password Salah, Tangani Logika Rate Limiting & Lockout
        if (inputEmail !== 'mohdikbal1207@gmail.com') {
          ...
          if (currentUserData) {
            const newFailedAttempts = (currentUserData.failed_attempts || 0) + 1;
            let lockoutTime = null;

            // Jika sudah 4 kali salah, kunci akun selama 15 menit
            ...
            if (newFailedAttempts >= 4) {
              throw new Error('⚠️ Terlalu banyak percobaan salah. Akun Anda diblokir sementara selama 15 menit demi keamanan.');
            } else {
              throw new Error(`Kata sandi salah! Sisa percobaan: ${4 - newFailedAttempts} kali lagi sebelum akun dikunci.`);
            }
          }
        }
      ...
      setLoadingStepText('Memverifikasi Hak Akses...');

      // 3. Cek Role & Reset Gagal Login Menjadi 0 jika Berhasil
      let userRole = 'kasir';
      let namaLengkap = inputEmail.split('@')[0];

      if (inputEmail === 'mohdikbal1207@gmail.com') {
        userRole = 'admin';
        namaLengkap = 'Mohd Ikbal (Admin)';
      } else {
        ...
        if (userData) {
          userRole = userData.role?.toLowerCase() || 'kasir';
          namaLengkap = userData.nama_lengkap || namaLengkap;
        }

        // Reset ulang riwayat percobaan gagal setelah login sukses
        await supabase
          .from('users')
          .update({
            failed_attempts: 0,
      ...
      setLoadingStepText('Mengalihkan Halaman...');
      setSuccessMessage(`Login Berhasil! Selamat datang ${namaLengkap}.`);

      // 4. Tentukan Target Pengarahan
      let targetUrl = '/kasir';
      if (redirectTo) {
      ...
      if (err instanceof Error) {
        ...
        triggerError('Terjadi kesalahan jaringan/sistem.');
      }
    } finally {
      setIsLoading(false);
  ...
  const handleContactIT = () => {
    const message = encodeURIComponent(
      `Halo Admin IT RSUD Bukit Kerman, saya mengalami kendala login pada Portal Staf SIMRS (Akun: ${emailOrUsername || '-'}).`
    );
          ...
          <div className="hidden lg:flex lg:col-span-6 flex-col justify-between p-8 xl:p-10 rounded-3xl bg-gradient-to-br from-emerald-950 via-teal-900 to-slate-950 text-white shadow-xl relative overflow-hidden">
            ...
            <div className="space-y-6 relative z-10 pt-6">
              ...
              <div className="p-4 rounded-2xl border bg-white/5 border-white/10 text-slate-300 text-xs flex items-center space-x-2.5">
                ...
                <span>Proteksi Rate Limiting, Auto-IP Blacklisting &amp; enkripsi tingkat lanjut aktif.</span>
              </div>
            </div>
          </div>
          ...
          <div className="w-full lg:col-span-6 flex items-center justify-center">
            <div className="w-full max-w-md space-y-4">
              ...
              <div className={`${isDarkMode ? 'bg-slate-900 border-slate-800 text-slate-100 shadow-slate-950/80' : 'bg-white border-slate-200 text-slate-800 shadow-xl'} border rounded-3xl p-6 sm:p-8 backdrop-blur-xl relative overflow-hidden transition-transform ${shakeCard ? 'animate-bounce' : ''}`}>
                ...
                {noticeMessage && (
                  <div className="mb-4 p-3.5 bg-amber-50 border border-amber-200 rounded-2xl text-xs text-amber-800 flex items-start space-x-2.5 shadow-sm">
                ...
                {successMessage && (
                  <div className="mb-4 p-3.5 bg-emerald-50 border border-emerald-200 rounded-2xl text-xs text-emerald-700 flex items-start space-x-2.5 animate-in fade-in duration-200 shadow-sm">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder="mohdikbal1207@gmail.com"
                  ...
                  <div className="space-y-1.5">
                    ...
                    {isCapsLockOn && (
                      <p className="text-[10px] text-amber-500 font-medium flex items-center gap-1 mt-1 pl-1">
