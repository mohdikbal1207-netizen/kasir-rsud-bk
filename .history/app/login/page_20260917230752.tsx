...
function LoginFormContent() {
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-slate-950 text-slate-100' : 'bg-white text-slate-800'} flex flex-col justify-between selection:bg-emerald-500 selection:text-white transition-colors duration-300`}>
      ...
      {/* Full Screen Split-Screen Layout */}
      <main className="w-full flex-1 grid grid-cols-1 lg:grid-cols-12 items-stretch">
        
        {/* Kolom Kiri (50%): Penuh Warna Gradasi Rumah Sakit */}
        <div className="hidden lg:flex lg:col-span-6 flex-col justify-between p-12 xl:p-16 bg-gradient-to-br from-emerald-950 via-teal-900 to-slate-950 text-white relative overflow-hidden shadow-inner">
        ...
        {/* Kolom Kanan (50%): Area Form Login dengan Background Kontras Bersih */}
        <div className={`w-full lg:col-span-6 flex items-center justify-center p-6 sm:p-10 lg:p-12 ${isDarkMode ? 'bg-slate-950' : 'bg-slate-50'}`}>
          ...
          <div className="w-full max-w-md space-y-4">
            ...
            <div className={`${isDarkMode ? 'bg-slate-900 border-slate-800 text-slate-100 shadow-slate-950/80' : 'bg-white border-slate-200/90 text-slate-800 shadow-xl'} border rounded-3xl p-6 sm:p-8 backdrop-blur-xl relative overflow-hidden transition-transform ${shakeCard ? 'animate-bounce' : ''}`}>
