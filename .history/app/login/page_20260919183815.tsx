...
function LoginFormInner() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    console.log('>>> [DEBUG SIMRS] Tombol login diklik!', { emailOrUsername, password });
    ...
    try {
      setLoadingStepText('Memeriksa Kredensial Supabase Auth...');
      ...
      // 1. Langsung Tembak Supabase Auth (Email + Password)
      console.log('>>> [DEBUG SIMRS] Mencoba auth.signInWithPassword ke:', inputEmail);
      ...
      if (authError) {
        console.error('>>> [DEBUG SIMRS] Auth Error:', authError);
        throw new Error(`Gagal Masuk: ${authError.message}`);
      }
      ...
      console.log('>>> [DEBUG SIMRS] Login Auth Sukses! User ID:', authData.user.id);
      setLoadingStepText('Memverifikasi Hak Akses...');

      // 2. Ambil Profil dari tabel `users` (Optional Fail-safe)
      let userRole = 'staf';
      let namaLengkap = inputEmail.split('@')[0];

      // Bypass khusus email admin utama
      if (inputEmail === 'mohdikbal1207@gmail.com') {
        ...
        if (userData) {
          ...
          if (userData.is_active === false) {
            ...
            throw new Error('Akun Anda dinonaktifkan oleh Admin.');
          }
        }
      }

      setLoadingStepText('Menyiapkan Sesi Dashboard...');
      setSuccessMessage(`Login Berhasil! Selamat datang ${namaLengkap}. Mengalihkan...`);

      // 3. Pengarahan Langsung Tanpa Basa-Basi
      let targetUrl = '/admin';
      if (redirectTo) {
      ...
      console.log('>>> [DEBUG SIMRS] Redirecting ke:', targetUrl);
            ...
            <div className="w-full max-w-md space-y-4">
              ...
              <div className={`${isDarkMode ? 'bg-slate-900 border-slate-800 text-slate-100 shadow-slate-950/80' : 'bg-white border-slate-200 text-slate-800 shadow-xl'} border rounded-3xl p-6 sm:p-8 backdrop-blur-xl relative overflow-hidden transition-transform ${shakeCard ? 'animate-bounce' : ''}`}>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  {/* TOMBOL MASUK DENGAN TYPE SUBMIT YANG PASTI AKTIF */}
                  <button
                    type="submit"
