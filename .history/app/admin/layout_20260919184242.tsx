...
import { Loader2, WifiOff, Activity, Lock, KeyRound, BellRing, Wrench, TimerReset } from 'lucide-react';
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  // VERIFIKASI KEAMANAN BEBAS MENCEGAT / BANCI RETRY
  useEffect(() => {
    let isMounted = true;

    async function verifyAdminAccess() {
      try {
        // Cek Sesi Supabase Auth dengan toleransi Waktu (3x Retry)
        let sessionData = await supabase.auth.getSession();
        let session = sessionData.data.session;

        if (!session?.user) {
          // Beri kesempatan retry 1 detik jika cookie belum terbaca
          await new Promise((res) => setTimeout(res, 1000));
          const retryData = await supabase.auth.getSession();
          session = retryData.data.session;
        }
        ...
        // BISA DIBYPASS KHUSUS EMAIL KAMU
        const isSuperAdminEmail = userEmail === 'mohdikbal1207@gmail.com';
        ...
        console.error('Kesalahan verifikasi admin:', err);
        if (isMounted) router.replace('/login');
      }
    }
