...
import { Wrench, ShieldCheck, Save, Loader2, AlertTriangle, CheckCircle2, RefreshCcw, ArrowLeft, Activity, Lock, Unlock, ExternalLink, Sparkles, Clock, Mail, Eye, X, ShieldAlert, PhoneCall, Copy, Check, SearchCode, Zap, Send, MessageSquare } from 'lucide-react';
...
export default function MaintenanceControlPage() {
  ...
  // State Pengaturan Notifikasi Fonnte WhatsApp
    ...
    try {
      ...
      if (data?.value) {
        ...
        if (data.value.notification) {
          setEnableNotification(Boolean(data.value.notification.enabled));
          setTargetGroup(data.value.notification.target || '');
          setCustomToken(data.value.notification.token || '');
        }
      }
      if (data?.updated_at) {
  ...
  // Fungsi Eksekusi Kirim Pesan via Fonnte WhatsApp API
  const sendFonnteBroadcast = async (status: boolean, msg: string, est: string) => {
    if (!enableNotification || !targetGroup) return;
    ...
    try {
      // Mengirim request langsung ke endpoint Fonnte
      await fetch('https://api.fonnte.com/send', {
        method: 'POST',
        headers: {
          'Authorization': customToken.trim(), // Menggunakan token yang diisi di form
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({
          target: targetGroup.trim(),
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Kirim Broadcast Fonnte WhatsApp secara otomatis
      await sendFonnteBroadcast(isActive, message, estimatedFinish);

      setStatusMessage('Pengaturan Maintenance & Broadcast Fonnte WhatsApp berhasil diperbarui!');
      playSound('success');
      fetchSettings();
    } catch (err: unknown) {
      ...
      <main className="w-full max-w-4xl mx-auto px-4 sm:px-6 py-10 flex-1 space-y-6 relative z-10">
        ...
        {isLoading ? (
          ...
          <form onSubmit={handleSave} className="bg-white border border-slate-200/90 rounded-3xl p-6 sm:p-8 shadow-xl shadow-slate-200/50 space-y-6">
            ...
            {/* Konfigurasi Broadcast Fonnte WhatsApp API */}
            <div className="p-5 bg-emerald-50/60 border border-emerald-200 rounded-2xl space-y-4">
              <div className="flex items-center justify-between border-b border-emerald-200/60 pb-3">
                <div className="flex items-center space-x-2">
                  <MessageSquare className="w-4 h-4 text-emerald-600" />
                  <span className="text-xs font-bold text-slate-800 uppercase tracking-wider">Broadcast Otomatis Fonnte WhatsApp</span>
                </div>
              ...
              {enableNotification && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1 animate-in fade-in">
                  <div className="space-y-1.5">
                    <label className="block text-[11px] font-bold text-slate-700 uppercase">Target (Nomor HP / Grup ID WhatsApp):</label>
                    <input
                      type="text"
                      value={targetGroup}
                      onChange={(e) => setTargetGroup(e.target.value)}
                      placeholder="Cth: 628228xxxxxx atau ID Grup WhatsApp"
                      className="w-full bg-white border border-emerald-200 rounded-xl px-3.5 py-2 text-xs font-mono text-slate-800 focus:outline-none focus:border-emerald-500"
                    />
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      placeholder="Masukkan Token Fonnte (tersedia di .env.local)..."
                      className="w-full bg-white border border-emerald-200 rounded-xl px-3.5 py-2 text-xs font-mono text-slate-800 focus:outline-none focus:border-emerald-500"
                    />
                  </div>
                </div>
