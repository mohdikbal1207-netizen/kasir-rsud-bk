...
export default function AdminDashboardPage() {
  ...
  // State Papan Pengumuman / Alert Banner
  ...
  const [editPriority, setEditPriority] = useState<'darurat' | 'mendesak' | 'informasi'>('informasi');
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between selection:bg-emerald-500 selection:text-white relative overflow-hidden">
      ...
      <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 flex-1 space-y-8 relative z-10">
        ...
        {/* ================= MODUL ALERT BANNER & PENGUMUMAN DIREKSI / ADMIN ================= */}
        <div className={`p-6 rounded-3xl border shadow-xl transition-all relative overflow-hidden backdrop-blur-xl ${
          announcement.priority === 'darurat' ? 'bg-rose-600 text-white border-rose-500 shadow-rose-600/20' :
          announcement.priority === 'mendesak' ? 'bg-amber-500 text-slate-950 border-amber-400 shadow-amber-500/20' :
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 relative z-10">
            <div className="flex items-start gap-4">
              <div className={`p-3.5 rounded-2xl flex-shrink-0 ${
                announcement.priority === 'darurat' ? 'bg-white/20 text-white' :
                announcement.priority === 'mendesak' ? 'bg-slate-950/10 text-slate-950' :
                'bg-emerald-500/20 text-emerald-400'
              }`}>
                {announcement.priority === 'darurat' ? <AlertTriangle className="w-6 h-6 animate-bounce" /> :
                 announcement.priority === 'mendesak' ? <BellRing className="w-6 h-6 animate-pulse" /> :
                 <Megaphone className="w-6 h-6" />}
              </div>
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <span className={`text-[10px] font-black uppercase px-2.5 py-0.5 rounded-full ${
                    announcement.priority === 'darurat' ? 'bg-white text-rose-700' :
                    announcement.priority === 'mendesak' ? 'bg-slate-950 text-amber-400' :
                    'bg-emerald-400 text-slate-950'
                  }`}>
                    PENGUMUMAN SENSISTIF — {announcement.priority}
                  </span>
                ...
                <p className={`text-xs max-w-4xl leading-relaxed ${
                  announcement.priority === 'mendesak' ? 'text-slate-900 font-medium' : 'text-slate-200'
                }`}>
            ...
            <button
              ...
              className={`px-4 py-2.5 rounded-2xl text-xs font-bold transition flex items-center gap-2 cursor-pointer shadow-md flex-shrink-0 ${
                announcement.priority === 'darurat' ? 'bg-white text-rose-700 hover:bg-rose-50' :
                announcement.priority === 'mendesak' ? 'bg-slate-950 text-amber-400 hover:bg-slate-900' :
                'bg-emerald-500 text-slate-950 hover:bg-emerald-400'
              }`}
            >
              <Edit3 className="w-4 h-4" /> Kelola Pengumuman
            </button>
          </div>
        </div>
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 sm:p-8 shadow-2xl border border-slate-200 space-y-5 animate-in fade-in zoom-in duration-200">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2 text-slate-900 font-black text-sm uppercase">
                ...
                <span>Pengaturan Papan Pengumuman Admin</span>
              </div>
              <button 
            ...
            <form onSubmit={handleSaveAnnouncement} className="space-y-4 text-xs">
              <div>
                ...
                <input
                  ...
                  placeholder="Contoh: PEMELIHARAAN JARINGAN SERVER"
                  className="w-full bg-slate-50 border border-slate-300 rounded-xl p-3 focus:outline-none font-bold text-slate-900"
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-50 border border-slate-300 rounded-xl p-3 focus:outline-none font-bold"
                >
                  <option value="informasi">Informasi Umum (Tema Gelap)</option>
                  <option value="mendesak">Mendesak / Penting (Tema Kuning Amber)</option>
                  <option value="darurat">Darurat Medis / System Down (Tema Merah Rose)</option>
                </select>
              </div>
              ...
              <div className="flex justify-end gap-2 pt-2 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold shadow flex items-center gap-1.5 cursor-pointer"
                >
                  <CheckCircle2 className="w-4 h-4" /> Terbitkan Pengumuman
                </button>
              </div>
            </form>
          </div>
