  ...
  useEffect(() => {
    async function verifyAdminAccess() {
      try {
        ...
        // Jalur Khusus Fail-safe untuk Email Utama Admin RSUD
        ...
        // 3. Normalisasi role
        const userRole = (userData?.role || (isSuperAdminEmail ? 'admin' : '')).toLowerCase().trim();

        // 4. Validasi Mutlak: Cek apakah role bernilai 'admin' atau merupakan email admin utama
        if (userRole !== 'admin' && !isSuperAdminEmail) {
          ...
          await supabase.auth.signOut();
          alert(`Akses Ditolak! Akun (${userEmail}) tidak memiliki hak akses sebagai Administrator.`);
          router.replace('/login');
          return;
        }

        // Jika terkonfirmasi admin, izinkan masuk!
