...
export default function AdminIGDPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setMessage(null);
    try {
      // PERBAIKAN: Fetch detail_pemeriksaan_igd beserta nama tindakan jika ada
      ...
      if (data) {
        const mapped: TransactionItem[] = data.map((h: any) => {
          ...
          // Status baru menjadi 'lunas' HANYA jika Admin sudah memberikan persetujuan (status_verifikasi === 'disetujui' / 'Disetujui')
  ...
  const handleApprove = async (tx: TransactionItem) => {
    ...
    try {
      ...
      if (error) throw new Error(error.message);
      setMessage({ type: 'success', text: `Transaksi IGD ${tx.id} (${tx.nama_pasien}) berhasil disetujui LUNAS oleh Admin.` });
      fetchData();
    } catch (err: unknown) {
