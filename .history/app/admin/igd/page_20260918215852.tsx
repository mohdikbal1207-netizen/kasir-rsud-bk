...
export default function AdminIGDPage() {
    ...
    try {
      ...
      if (data) {
        const mapped: TransactionItem[] = data.map((h: any) => {
          ...
          const isVerifiedApproved = verifikasiRaw === 'disetujui' || verifikasiRaw === 'lunas';
          ...
          return {
            ...
            status_bayar: isVerifiedApproved ? 'lunas' : 'pending',
    ...
    try {
      if (confirmModal.actionType === 'approve' && confirmModal.targetTx) {
        ...
        const { error } = await supabase
          .from('pemeriksaan_igd_header')
          .update({ 
            status_verifikasi: 'Direvisi', 
            status_bayar: 'Dibatalkan',
            catatan_admin: adminNoteInput || 'Transaksi dibatalkan / membutuhkan revisi berkas.'
          })
          .eq('id', tx.db_id);

        if (error) throw new Error(error.message);
        showToast('info', 'Transaksi Dibatalkan', `Transaksi ${tx.id} berhasil dibatalkan.`);
  ...
  const handleReject = (tx: TransactionItem) => {
    ...
    setConfirmModal({
      isOpen: true,
      title: 'Batalkan Transaksi IGD',
      description: `Apakah Anda yakin ingin membatalkan transaksi ${tx.id} (${tx.nama_pasien})?`,
      actionType: 'reject',
      targetTx: tx
    });
  };
  ...
  const handleSendWhatsApp = (tx: TransactionItem) => {
    const text = `Halo RSUD Bukit Kerman, informasi kasir IGD untuk pasien *${tx.nama_pasien}* (No. RM: ${tx.no_rm}) dengan ID Transaksi *${tx.id}* senilai *${formatRupiah(tx.total_biaya)}* status saat ini: *${tx.status_bayar.toUpperCase()}*. Catatan Admin: ${tx.catatan_admin || '-'}`;
  ...
  const handleExportCustom = () => {
    ...
    if (exportFormat === 'csv') {
      const headers = ['ID Transaksi', 'No RM', 'Nama Pasien', 'Triase', 'Penjaminan', 'Total Biaya (IDR)', 'Status Pembayaran', 'Catatan Admin', 'Tanggal Transaksi'];
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between selection:bg-amber-500 selection:text-white relative overflow-hidden">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full uppercase ${
                          ...
                          {tx.status_bayar === 'lunas' ? 'Sudah Diverifikasi' : 'Menunggu Verifikasi'}
                        </span>
                      </div>
                      ...
                      <tr key={tx.id} className="hover:bg-slate-50/80 transition">
                        ...
                        <td className="p-4 text-center">
                          <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase ${
                            ...
                            {tx.status_bayar === 'lunas' ? 'Lunas' : 'Menunggu Verifikasi'}
                          </span>
                        </td>
                        <td className="p-4 text-center">
          ...
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-200 space-y-4">
            <div className="flex items-center gap-3">
              <div className={`p-3 rounded-2xl ${
                ...
                confirmModal.actionType === 'reject' ? 'bg-amber-100 text-amber-600' : 'bg-emerald-100 text-emerald-600'
              }`}>
      ...
      {/* DETAIL MODAL (DENGAN TABEL DAFTAR ITEM RINCIAN TINDAKAN MEDIS) */}
            ...
            <div className="bg-amber-50/40 border border-amber-200/80 rounded-2xl p-5 space-y-3 font-mono text-xs text-slate-800">
              ...
              <div className="space-y-1 text-[11px] font-sans">
                ...
                <div className="flex justify-between">
                  ...
                  <strong className="text-amber-800 italic">{receiptRecord.catatan_admin || '-'}</strong>
                </div>
                <div className="flex justify-between">
              ...
              <div className="pt-2 text-center">
                <span className={`inline-block px-3 py-1 rounded-full text-[10px] font-bold uppercase font-sans ${
                  ...
                  Status: {receiptRecord.status_bayar === 'lunas' ? 'LUNAS (TERVERIFIKASI)' : 'MENUNGGU VERIFIKASI ADMIN'}
                </span>
              </div>
            </div>
