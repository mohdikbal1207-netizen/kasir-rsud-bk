...
export default function AdminIGDPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Murni mengambil data dari Supabase tanpa data dummy fallback
      setTransactions(data || []);
    } catch (err: unknown) {
      console.error(err);
      if (err instanceof Error) {
        setMessage({ type: 'error', text: `Gagal memuat data verifikasi IGD: ${err.message}` });
      } else {
        setMessage({ type: 'error', text: 'Gagal memuat data verifikasi IGD dari database.' });
      }
      setTransactions([]);
    } finally {
      setIsLoading(false);
  ...
  const handleDelete = async (tx: TransactionItem) => {
    if (!confirm(`PERINGATAN: Hapus permanen data transaksi ${tx.id} (${tx.nama_pasien}) dari database RSUD Bukit Kerman? Tindakan ini tidak dapat dibatalkan.`)) return;
    setIsSubmitting(true);
    try {
      const { error } = await supabase
        .from('transaksi_pasien')
      ...
      if (error) throw new Error(error.message);
      setMessage({ type: 'success', text: `Data transaksi ${tx.id} berhasil dihapus dari sistem.` });
  ...
  const handleBatchApprove = async () => {
    ...
    if (!confirm(`Setujui ${selectedIds.length} transaksi terpilih menjadi LUNAS secara massal?`)) return;
    setIsSubmitting(true);
    try {
  ...
  const handleSendWhatsApp = (tx: TransactionItem) => {
    const text = `Halo RSUD Bukit Kerman, informasi verifikasi IGD untuk pasien *${tx.nama_pasien}* (No. RM: ${tx.no_rm}) dengan ID Transaksi *${tx.id}* senilai *${formatRupiah(tx.total_biaya)}* status saat ini: *${tx.status_bayar.toUpperCase()}*. Terima kasih.`;
  ...
  }, [transactions, searchTerm, filterTanggal, triaseFilter, penjaminanFilter]);
  ...
  const stats = useMemo(() => {
    ...
    return { totalPending, totalPendingMerah, totalLunas, totalPendapatan };
  }, [transactions]);
      ...
      <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 flex-1 space-y-6 relative z-10 pb-24 print:p-0 print:max-w-none">
        ...
        {isLoading ? (
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-12 text-center space-y-3 shadow-sm">
            ...
            <h2 className="text-sm font-bold text-slate-700">Tidak ada data verifikasi IGD dari database.</h2>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((tx) => {
