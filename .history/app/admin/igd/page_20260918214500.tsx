    ...
    try {
      ...
      if (data) {
        const mapped: TransactionItem[] = data.map((h: any) => {
          const isLunas = h.status_verifikasi === 'Disetujui' || h.status_bayar === 'Lunas' || h.status_bayar === 'lunas';
          return {
            id: `IGD-${h.id}`,
            db_id: h.id,
            no_rm: h.no_rm || '',
        ...
        let filteredByTab = mapped;
        if (activeTab === 'pending') {
          filteredByTab = mapped.filter(t => t.status_bayar === 'pending');
        } else if (activeTab === 'lunas') {
          filteredByTab = mapped.filter(t => t.status_bayar === 'lunas');
        }

        setTransactions(filteredByTab);
      } else {
        setTransactions([]);
      }
    } catch (err: unknown) {
