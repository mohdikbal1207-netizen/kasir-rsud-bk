...
import { 
  ...
  Download, CheckSquare2, Square, Layers, Banknote
} from 'lucide-react';
...
const penyebut = (nilai: number): string => {
  ...
  if (bilangan < 12) {
    ...
    hasil = " Seribu" + penyebut(bilangan - 1000);
...
export default function AdminRanapPage() {
  ...
  // State untuk Batch Selection (Pilih Banyak Data)
  ...
  // Kalkulasi Keuangan Global untuk Statistik Admin
  ...
  const toggleSelectAllAdmin = () => {
    if (selectedAdminRowIds.length === filteredPatients.length) {
      setSelectedAdminRowIds([]);
    } else {
      setSelectedAdminRowIds(filteredPatients.map(p => p.no_reg));
    }
  };
    ...
    <div className="min-h-screen flex flex-col bg-slate-100 font-sans print:bg-white">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 py-8 space-y-6 print:p-0 print:m-0 print:max-w-none">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 print:hidden">
          <div className="bg-slate-900 text-white p-5 rounded-3xl shadow-md border border-slate-800 flex flex-col justify-between">
            ...
            <div className="mt-3">
              ...
              <p className="text-[10px] text-slate-400 mt-1">Seluruh rekapitulasi ranap masuk</p>
            </div>
          </div>

          <div className="bg-teal-800 text-white p-5 rounded-3xl shadow-md border border-teal-700 flex flex-col justify-between">
            ...
            <div className="mt-3">
              ...
              <p className="text-[10px] text-teal-200 mt-1">Klaim penjaminan asuransi</p>
            </div>
          </div>

          <div className="bg-cyan-900 text-white p-5 rounded-3xl shadow-md border border-cyan-800 flex flex-col justify-between">
            ...
            <div className="mt-3">
              ...
              <p className="text-[10px] text-cyan-200 mt-1">Selisih bayar / mandiri</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4 print:hidden">
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden print:hidden">
              ...
              <thead>
                <tr className="bg-slate-100 text-slate-700 uppercase font-bold border-b border-slate-300 text-[10px]">
                  <th className="p-3.5 text-center w-12">
                    <input 
                      type="checkbox"
                      checked={selectedAdminRowIds.length === filteredPatients.length && filteredPatients.length > 0}
      ...
      {showDetailModal && selectedPatient && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-sm animate-fade-in print:p-0 print:bg-white print:static print:inset-auto">
          <div className="bg-white border border-slate-200 w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden p-6 text-slate-800 relative max-h-[92vh] flex flex-col print:shadow-none print:border-none print:max-h-none print:p-0">
            ...
            <div className="hidden print:block font-sans text-slate-900 text-[11px] p-2 space-y-3">
              
              {/* Kop Surat Resmi */}
              <div className="flex items-center border-b-2 border-slate-900 pb-2 mb-3">
                <div className="w-14 flex-shrink-0 flex items-center justify-center">
                  <div className="w-12 h-12 bg-emerald-700 rounded-full flex items-center text-white font-bold text-xs justify-center text-center">RSUD</div>
                </div>
                <div className="flex-1 text-center px-2">
                  <h4 className="text-xs font-bold uppercase tracking-wider">PEMERINTAH KABUPATEN KERINCI</h4>
                  <h3 className="text-xs font-bold uppercase tracking-wider">DINAS KESEHATAN</h3>
                  <h2 className="text-sm font-black uppercase tracking-tight text-emerald-900">RSUD BUKIT KERMAN</h2>
                  <p className="text-[9px] text-slate-600">Desa Pondok, Kecamatan Bukit Kerman, Kode Pos: 37176</p>
                  <p className="text-[9px] text-slate-600">Website: https://rsudbukitkerman.kerincikab.go.id • Email: rsudbukitkerman@gmail.com</p>
                </div>
                <div className="w-14 flex-shrink-0 flex items-center justify-center">
                  <div className="w-12 h-12 border border-emerald-800 rounded-full flex items-center justify-center text-emerald-800 font-bold text-[10px]">LOGO</div>
                </div>
              </div>

              <div className="text-center font-bold underline uppercase text-xs tracking-wide">
                PERINCIAN BIAYA PERAWATAN PASIEN RAWAT INAP
              </div>

              {/* Biodata Pasien Print */}
              <div className="grid grid-cols-2 gap-x-4 border border-slate-800 p-2.5 rounded text-[10px]">
              ...
              <table className="w-full border-collapse border border-slate-800 text-[10px]">
                <thead>
                  <tr className="bg-slate-200 text-slate-900 font-bold uppercase text-center border-b border-slate-800">
                    <th className="border border-slate-800 p-1.5 w-8">No</th>
                    <th className="border border-slate-800 p-1.5 text-left">Uraian Biaya</th>
                    <th className="border border-slate-800 p-1.5 w-12">Vol</th>
                    <th className="border border-slate-800 p-1.5 text-right">Tarif (Rp)</th>
                    <th className="border border-slate-800 p-1.5 text-right">Jumlah (Rp)</th>
                    <th className="border border-slate-800 p-1.5 text-right">Ditanggung (Pihak 3)</th>
                    <th className="border border-slate-800 p-1.5 text-right">Selisih (Bayar)</th>
                  </tr>
                </thead>
                <tbody>
                ...
                <tfoot>
                  <tr className="bg-slate-100 font-bold">
                    <td colSpan={4} className="border border-slate-800 p-1.5 text-right uppercase">Jumlah Total :</td>
                    <td className="border border-slate-800 p-1.5 text-right font-mono">Rp {formatNumber(selectedPatient.total_biaya || 0)}</td>
                    <td className="border border-slate-800 p-1.5 text-right font-mono">Rp {formatNumber(selectedPatient.total_ditanggung || 0)}</td>
                    <td className="border border-slate-800 p-1.5 text-right font-mono">Rp {formatNumber(selectedPatient.total_selisih || 0)}</td>
                  </tr>
                </tfoot>
              </table>

              {/* Terbilang */}
              <div className="border border-slate-800 p-2 rounded text-[10px] flex items-center space-x-2">
              ...
              <div className="grid grid-cols-3 pt-4 text-center text-[10px]">
                <div>
