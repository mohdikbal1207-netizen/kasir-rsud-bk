...
import { Loader2, ShieldAlert, TimerReset, Wifi, WifiOff, Activity, ShieldCheck, Lock, KeyRound, Wrench, FileSpreadsheet } from 'lucide-react';
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  // TAMBAHAN: State untuk Audit Trail Log System & Banner Pemeliharaan Server
  const [currentSessionId, setCurrentSessionId] = useState<string>('');
  const [isMaintenanceMode, setIsMaintenanceMode] = useState<boolean>(false);

  // TAMBAHAN: Fungsi utilitas untuk mencatat Jejak Keamanan (Audit Log)
  ...
  }, [resetInactivityTimers, logAuditActivity]);
  ...
  useEffect(() => {
    async function verifyAdminAccess() {
      try {
        ...
        const userEmail = session.user.email || 'Admin RSUD';
        ...
        if (userRole !== 'admin') {
          logAuditActivity('UNAUTHORIZED_ACCESS_ATTEMPT', `User dengan role ${userRole} mencoba mengakses admin panel.`);
          await supabase.auth.signOut();
        ...
        logAuditActivity('ADMIN_VERIFIED_SUCCESSFULLY', 'Akses administrator terkonfirmasi.');
      } catch (err) {
  ...
  return (
    <>
      ...
      {/* TAMBAHAN: Banner Informasi Mode Pemeliharaan Terjadwal (Opsional) */}
      {isMaintenanceMode && (
      ...
      {/* TAMBAHAN: Watermark Keamanan Halus Anti Leak Foto Layar */}
