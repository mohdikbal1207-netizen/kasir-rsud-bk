  ...
  useEffect(() => {
    async function verifyAdminAccess() {
      try {
        ...
        setCurrentSessionId(session.user.id.substring(0, 8).toUpperCase());

        // 2. Kunci Utama: Ambil role dari tabel 'users' berdasarkan EMAIL (Bukan berdasarkan ID)
        ...
        if (userError) {
          console.error('Gagal mengambil data dari tabel users:', userError);
        }

        // 3. Normalisasi role yang didapatkan dari tabel 'users'
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center text-white space-y-3">
        ...
        <p className="text-xs font-medium text-slate-400">Memverifikasi tingkat keamanan Administrator di tabel users...</p>
      </div>
    );
  }
