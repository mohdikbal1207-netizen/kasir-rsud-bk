...
export default function AdminPengumumanPage() {
  ...
  // Penambahan State Aksi Massal (Bulk Selection)
  ...
  // Manajemen Fitur Pemilihan Massal (Bulk Actions)
  ...
  const handleBulkUpdateStatus = async (status: boolean) => {
    ...
    try {
      const { error } = await supabase.from('pengumuman').update({ is_active: status }).in('id', selectedIds);
      if (error) throw error;
      showToast(`${selectedIds.length} Pengumuman berhasil di-${status ? 'aktifkan' : 'nonaktifkan'}!`, 'success');
      setSelectedIds([]);
      fetchAnnouncements();
    } catch (err: any) {
  ...
  const handleBulkDeleteExecute = async () => {
    if (selectedIds.length === 0) return;
    try {
      const { error } = await supabase.from('pengumuman').delete().in('id', selectedIds);
      if (error) throw error;
      showToast(`${selectedIds.length} Pengumuman berhasil dihapus secara permanen.`, 'success');
      setSelectedIds([]);
  ...
  // Kalkulator Badge Waktu Hitung Mundur
  const getTimeRemainingBadge = (p: Pengumuman) => {
    const nowTime = new Date().getTime();
    const startTime = new Date(p.tanggal_mulai).getTime();
    const endTime = p.tanggal_selesai ? new Date(p.tanggal_selesai).getTime() : null;

    if (startTime > nowTime) {
      const diffHours = Math.ceil((startTime - nowTime) / (1000 * 60 * 60));
      return <span className="text-[10px] font-bold text-sky-700 bg-sky-50 px-2 py-0.5 rounded-md border border-sky-200">⏱ Tayang dlm {diffHours}j</span>;
    }

    if (endTime) {
      if (endTime < nowTime) {
        return <span className="text-[10px] font-bold text-slate-500 bg-slate-100 px-2 py-0.5 rounded-md">Expired</span>;
      }
      const diffDays = Math.ceil((endTime - nowTime) / (1000 * 60 * 60 * 24));
      if (diffDays <= 1) {
        return <span className="text-[10px] font-bold text-rose-700 bg-rose-50 px-2 py-0.5 rounded-md border border-rose-200 animate-pulse">⚠️ Berakhir Hari Ini</span>;
      }
      return <span className="text-[10px] font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-200">⏳ Sisa {diffDays} Hari</span>;
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between selection:bg-purple-500 selection:text-white pb-24">
      {toast && (
          ...
          <div className="flex items-center justify-between gap-2 overflow-x-auto pb-1 scrollbar-none">
            ...
            {filteredAnnouncements.length > 0 && (
              <button
                ...
                className="text-xs font-bold text-purple-700 bg-purple-50 hover:bg-purple-100 border border-purple-200 px-3 py-2 rounded-2xl transition shrink-0 flex items-center gap-1.5 cursor-pointer"
              >
                {selectedIds.length === filteredAnnouncements.length ? <CheckSquare className="w-4 h-4 text-purple-600" /> : <Square className="w-4 h-4 text-purple-400" />}
                {selectedIds.length === filteredAnnouncements.length ? 'Batal Pilih Semua' : 'Pilih Semua'}
              </button>
            )}
          </div>

          {/* FILTER CHIPS TAG LOKAL */}
          <div className="flex items-center gap-1.5 flex-wrap pt-1">
            <span className="text-[10px] font-bold text-slate-400 uppercase flex items-center gap-1 mr-1">
              <Tag className="w-3 h-3" /> Tag Cepat:
            </span>
            {['BPJS', 'Kasir', 'Tarif', 'Shift', 'Sistem'].map((tag) => (
              <button
                key={tag}
                ...
                className={`text-[10px] font-bold px-2.5 py-1 rounded-xl transition cursor-pointer border ${
                  searchTerm === tag ? 'bg-purple-600 text-white border-purple-600' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-100'
                }`}
              >
                #{tag}
              </button>
            ...
            {searchTerm && (
              <button
                ...
                className="text-[10px] font-bold text-rose-600 bg-rose-50 hover:bg-rose-100 px-2 py-1 rounded-xl transition cursor-pointer"
              >
                Reset Cari
              </button>
            )}
              ...
              return (
                <div
                  ...
                  className={`bg-white border rounded-3xl p-6 shadow-sm transition flex flex-col sm:flex-row items-start justify-between gap-4 relative overflow-hidden ${
                    isSelected ? 'bg-purple-50/50 border-purple-500 ring-2 ring-purple-500/30' : isPinned ? 'ring-2 ring-purple-600 border-purple-400 bg-purple-50/20' : !p.is_active ? 'opacity-50 border-slate-200 bg-slate-50/50' : p.prioritas === 'TINGGI' ? 'border-amber-400 ring-2 ring-amber-400/20' : 'border-slate-200'
                  }`}
                >
                  <div className="flex items-start gap-3.5 flex-1">
                    ...
                    <button
                      ...
                      className="mt-1.5 cursor-pointer text-slate-400 hover:text-purple-600 transition shrink-0"
                      title="Pilih Pengumuman Ini"
                    >
      ...
      {selectedIds.length > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 bg-slate-900 text-white px-6 py-3.5 rounded-2xl shadow-2xl flex items-center gap-4 border border-slate-700 animate-in slide-in-from-bottom-5">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="bg-emerald-600 hover:bg-emerald-700 text-white text-[11px] font-bold px-3 py-1.5 rounded-xl transition flex items-center gap-1 cursor-pointer"
            >
              <Power className="w-3.5 h-3.5" /> Aktifkan
            </button>
            <button
              onClick={() => handleBulkUpdateStatus(false)}
              className="bg-slate-700 hover:bg-slate-600 text-slate-200 text-[11px] font-bold px-3 py-1.5 rounded-xl transition flex items-center gap-1 cursor-pointer"
            >
              <Power className="w-3.5 h-3.5 text-slate-400" /> Nonaktifkan
            </button>
            <button
              onClick={() => setShowBulkDeleteConfirm(true)}
              className="bg-rose-600 hover:bg-rose-700 text-white text-[11px] font-bold px-3 py-1.5 rounded-xl transition flex items-center gap-1 cursor-pointer"
            >
          ...
          <button
            ...
            className="p-1 hover:bg-slate-800 rounded-full ml-2 cursor-pointer text-slate-400 hover:text-white"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-4 animate-in fade-in">
            ...
            <div className="text-center space-y-1">
              <h3 className="text-base font-black text-slate-900">Hapus {selectedIds.length} Pengumuman?</h3>
              <p className="text-xs text-slate-500">Seluruh pengumuman yang terpilih akan dihapus permanen dari Supabase database.</p>
            </div>
              ...
              <form onSubmit={handleSave} className="space-y-4 text-xs">
                ...
                <div>
                  <div className="flex justify-between items-center mb-1">
                    <label className="font-bold text-slate-700">Judul Pengumuman <span className="text-rose-500">*</span></label>
                ...
                <div>
                  <div className="flex justify-between items-center mb-1">
                    <label className="font-bold text-slate-700">Isi Pesan Pengumuman <span className="text-rose-500">*</span></label>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full bg-slate-50 border rounded-2xl p-3 font-mono text-slate-800 focus:outline-none ${
                        tanggalSelesai && new Date(tanggalSelesai) < new Date(tanggalMulai) ? 'border-rose-500 bg-rose-50/30' : 'border-slate-200 focus:border-purple-500'
                      }`}
                    />
                    {tanggalSelesai && new Date(tanggalSelesai) < new Date(tanggalMulai) && (
                      <span className="text-[10px] font-bold text-rose-600 mt-1 block">⚠️ Tanggal selesai tidak boleh sebelum tanggal mulai</span>
                    )}
                  </div>
                </div>
