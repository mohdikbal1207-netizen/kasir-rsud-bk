...
export default function AdminPengumumanPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('admin_pengumuman_changes')
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between">
      ...
      <main className="w-full max-w-5xl mx-auto px-4 sm:px-6 py-8 flex-1 space-y-6">
        {/* HEADER BAR */}
        <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">Terbitkan pesan pengumuman atau peringatan darurat ke seluruh dashboard staf &amp; kasir RSUD.</p>
            </div>
          </div>
          <button
            ...
            announcements.map((p) => (
              <div
                ...
                className={`bg-white border rounded-3xl p-6 shadow-sm transition flex flex-col sm:flex-row items-start justify-between gap-4 relative overflow-hidden ${
                  !p.is_active ? 'opacity-50 border-slate-200 bg-slate-50/50' : p.kategori === 'MENDESAK' ? 'border-amber-300' : 'border-slate-200'
                }`}
              >
                <div className="flex items-start gap-4 flex-1">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 ${
                    p.kategori === 'MENDESAK' ? 'bg-amber-100 text-amber-700' : 'bg-sky-100 text-sky-700'
                  }`}>
                    {p.kategori === 'MENDESAK' ? <Bell className="w-6 h-6 animate-bounce" /> : <Info className="w-6 h-6" />}
                  </div>

                  <div className="space-y-2 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${
                        p.kategori === 'MENDESAK' ? 'bg-amber-500 text-white' : 'bg-sky-500 text-white'
                      }`}>
                        {p.kategori}
                      </span>
                      <span className="text-[11px] font-mono text-slate-400">
                        {p.nomor_pengumuman || `ANN-${p.id}`} • {new Date(p.created_at).toLocaleString('id-ID')}
                      </span>
                      <button
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 sm:p-8 shadow-2xl space-y-5 animate-in fade-in">
            ...
            <form onSubmit={handleSave} className="space-y-4 text-xs">
              <div>
                ...
                >
                  <option value="INFORMASI">INFORMASI (Pengumuman Umum)</option>
                  <option value="MENDESAK">MENDESAK (Pemberitahuan Penting / Darurat)</option>
                  <option value="SISTEM">SISTEM (Maintenance / Bridging BPJS)</option>
                  <option value="DOKTER">DOKTER (Jadwal Dokter / Layanan)</option>
                </select>
              </div>
              ...
              <div>
                ...
                <textarea
                  rows={4}
                  placeholder="Tuliskan isi pesan rinci yang akan ditayangkan di halaman utama kasir..."
                  value={isi}
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                <label htmlFor="isActiveCheck" className="font-bold text-slate-700 cursor-pointer">
                  Terbitkan Langsung ke Halaman Kasir (Aktif)
                </label>
              </div>
