...
export default function AdminPengumumanPage() {
  ...
  // PENAMBAHAN: Ekspor Rekap Seluruh Pengumuman Aktif ke WA / Clipboard
  ...
  // PENAMBAHAN: Pembersihan Massal Pengumuman Kedaluwarsa
  const handleBulkDeactivateExpired = async () => {
    const now = new Date();
    const expiredIds = announcements
      .filter(a => a.is_active && a.tanggal_selesai && new Date(a.tanggal_selesai) < now)
      .map(a => a.id);
  ...
  const filteredAnnouncements = announcements.filter((p) => {
    ...
    return matchSearch && matchCat && matchStat && matchTarget;
  });
