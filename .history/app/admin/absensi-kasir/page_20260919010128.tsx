...
export default function AdminAbsensiPage() {
  ...
  // Office Geofencing Location State (Dari Tabel Terpisah absensi_office_location)
  ...
  const fetchSettings = useCallback(async () => {
    try {
      ...
      console.error('Gagal memuat pengaturan shift:', err);
    }
  }, []);
  ...
  }, [fetchRecords, fetchSettings, fetchOfficeLocation]);

  // Google Maps Hybrid Tile Layer Initialization
  useEffect(() => {
    if (activeTab === 'radius' && typeof window !== 'undefined') {
    ...
    function initMap() {
      ...
      if (!mapInstanceRef.current) {
        ...
        const marker = L.marker([officeLocation.latitude, officeLocation.longitude], { draggable: true }).addTo(map);
        ...
        if (markerRef.current) {
          markerRef.current.setLatLng([officeLocation.latitude, officeLocation.longitude]);
        }
      }
    }
  ...
  const handleSaveSetting = async () => {
    ...
    try {
      ...
      if (editingSetting.id && editingSetting.id !== 0) {
        const { error } = await supabase.from('absensi_settings').update(payload).eq('id', editingSetting.id);
        if (error) throw error;
      } else {
        const { error } = await supabase.from('absensi_settings').insert([payload]);
        if (error) throw error;
      }

      showToast('Konfigurasi shift berhasil disimpan!', 'success');
      addLog(`Menyimpan shift: ${editingSetting.nama_shift}`);
      setShiftModal(false);
      setEditingSetting(null);
      fetchSettings();
    } catch (err: any) {
      console.error('Gagal simpan shift:', err);
      const errorDetail = err?.message || err?.error_description || err?.details || (typeof err === 'object' ? JSON.stringify(err, null, 2) : String(err));
      showToast(`Gagal menyimpan shift: ${errorDetail}`, 'error');
    }
  };
  ...
  const handleSaveOfficeLocation = async () => {
    ...
    try {
      const payload = {
        nama_lokasi: officeLocation.nama_lokasi,
        latitude: officeLocation.latitude,
        longitude: officeLocation.longitude,
        radius_meter: officeLocation.radius_meter,
  ...
  const filteredRecords = records.filter(r => {
    ...
    if (radiusFilter === 'valid') {
      matchRadius = (r.jarak_meter ?? 999) <= officeLocation.radius_meter;
    } else if (radiusFilter === 'invalid') {
      matchRadius = (r.jarak_meter ?? 0) > officeLocation.radius_meter;
    }
      ...
      <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 flex-1 space-y-6">
        ...
        <div className="bg-white border border-slate-200/80 rounded-3xl p-6 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 print:hidden">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <button onClick={() => { fetchRecords(); fetchSettings(); fetchOfficeLocation(); }} className="bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs px-3.5 py-3 rounded-2xl transition flex items-center gap-1.5 cursor-pointer" title="Sinkronisasi Data">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 print:hidden">
          ...
          >
            ...
            <div className="text-left">
              ...
              <span className={`text-[10px] ${activeTab === 'shift' ? 'text-purple-200' : 'text-slate-400'}`}>{settings.length} shift aktif</span>
            </div>
          </button>
          ...
          >
            ...
            <div className="text-left">
              ...
              <span className={`text-[10px] ${activeTab === 'radius' ? 'text-purple-200' : 'text-slate-400'}`}>Radius {officeLocation.radius_meter}m</span>
            </div>
          </button>
        </div>
              ...
              <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto flex-wrap">
                  ...
                  >
                    ...
                    <option value="invalid">Di Luar Radius (&gt; {officeLocation.radius_meter}m)</option>
                  </select>
                        ...
                        <tr key={r.id} className="hover:bg-slate-50 transition">
                          ...
                          <td className="p-3.5 text-center">
                            <span className={`font-mono font-bold px-2 py-0.5 rounded-lg text-[11px] ${
                              (r.jarak_meter ?? 0) <= officeLocation.radius_meter ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-rose-50 text-rose-700 border border-rose-200'
                            }`} title={(r.jarak_meter ?? 0) <= officeLocation.radius_meter ? 'Valid Dalam Radius' : 'Di Luar Radius Resmi'}>
        ...
        {/* KONTEN TAB 3: RADIUS & GEOFENCING GPS (DARI TABEL absensi_office_location) */}
        {activeTab === 'radius' && (
          <div className="bg-white border border-slate-200/80 rounded-3xl p-6 sm:p-8 shadow-sm space-y-6 animate-in fade-in">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">Tersimpan di tabel khusus `absensi_office_location`. Klik langsung pada peta atau seret marker.</p>
              </div>
              <button
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-1 space-y-4 bg-slate-50 p-6 rounded-2xl border border-slate-200/80">
                ...
                <div>
                  ...
                  />
                  <span className="text-[10px] text-slate-400 mt-1 block">Batas toleransi jarak absen pegawai dari titik pusat RSUD.</span>
                </div>
              ...
              {/* GOOGLE MAPS HYBRID TILE LAYER (LENGKAP DENGAN NAMA TEMPAT & JALAN) */}
              <div className="lg:col-span-2 bg-slate-100 rounded-2xl overflow-hidden border border-slate-200 shadow-inner relative flex flex-col min-h-[400px]">
                <div className="absolute top-4 left-4 z-40 bg-slate-900/90 backdrop-blur-sm text-white px-4 py-2 rounded-xl text-xs font-bold shadow-lg flex items-center gap-2 border border-slate-700 pointer-events-none">
                  <span className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-ping"></span>
                  <span>Google Maps: Klik peta atau geser pin marker</span>
                </div>
                
                <div ref={mapRef} className="w-full flex-1 min-h-[400px] z-10" />
