...
export default function AdminAbsensiPage() {
  ...
  const handleSaveSetting = async () => {
    ...
    try {
      ...
      addLog(`Memperbarui konfigurasi: ${editingSetting.nama_shift}`);
      setShiftModal(false);
  ...
  const handleDeleteSetting = async (id: number) => {
    if (confirm('Hapus konfigurasi ini?')) {
      ...
      showToast('Konfigurasi berhasil dihapus.', 'error');
      addLog(`Menghapus konfigurasi ID: ${id}`);
      fetchSettings();
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between selection:bg-purple-500 selection:text-white">
      ...
      <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 flex-1 space-y-6">
        ...
        {/* TAB NAVIGATION UTAMA (PISAH TAB YANG TEGAS) */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 print:hidden">
          ...
          >
            ...
            <div className="text-left">
              ...
              <span className={`text-[10px] ${activeTab === 'shift' ? 'text-purple-200' : 'text-slate-400'}`}>Jam masuk &amp; batas toleransi</span>
            </div>
          </button>
          ...
          >
            ...
            <div className="text-left">
              ...
              <span className={`text-[10px] ${activeTab === 'radius' ? 'text-purple-200' : 'text-slate-400'}`}>Batas jarak maksimum RSUD</span>
            </div>
          </button>
        </div>
        ...
        {/* KONTEN TAB 2: MANAJEMEN SHIFT KERJA */}
        {activeTab === 'shift' && (
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
              <div>
                <h3 className="text-base font-black uppercase text-slate-900 flex items-center gap-2.5">
                  <Clock4 className="w-5 h-5 text-purple-600" /> Manajemen Jam Shift Kerja
                </h3>
                <p className="text-xs text-slate-500 mt-0.5">Konfigurasi jam masuk, jam pulang, serta jendela toleransi absensi pegawai RSUD Bukit Kerman.</p>
              </div>
              <button
                onClick={() => {
                  setEditingSetting({
                ...
                settings.map((s) => (
                  <div key={s.id} className="bg-slate-50 border border-slate-200/80 rounded-2xl p-5 space-y-4 shadow-sm hover:border-purple-300 transition">
                    <div className="flex justify-between items-start">
                      <div>
                        <span className="text-[10px] font-black uppercase bg-purple-100 text-purple-800 px-2.5 py-1 rounded-full">{s.hari}</span>
                        <h4 className="text-base font-black text-slate-900 mt-2">Shift {s.nama_shift}</h4>
                      </div>
                    ...
                    <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-200/60">
                      <button onClick={() => { setEditingSetting(s); setShiftModal(true); }} className="px-3.5 py-1.5 bg-sky-50 hover:bg-sky-100 text-sky-700 border border-sky-200 rounded-xl font-bold text-xs cursor-pointer flex items-center gap-1">
        ...
        {/* KONTEN TAB 3: RADIUS & GEOFENCING GPS */}
        {activeTab === 'radius' && (
          <div className="bg-white border border-slate-200/80 rounded-3xl p-6 sm:p-8 shadow-sm space-y-6 animate-in fade-in">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
              <div>
                <h3 className="text-base font-black uppercase text-slate-900 flex items-center gap-2.5">
                  <Target className="w-5 h-5 text-emerald-600" /> Pengaturan Radius &amp; Geofencing GPS
                </h3>
                <p className="text-xs text-slate-500 mt-0.5">Tentukan batas toleransi jarak absolut dari titik koordinat RSUD Bukit Kerman untuk validasi presensi pegawai.</p>
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-gradient-to-br from-slate-900 to-purple-950 text-white rounded-3xl p-6 shadow-xl space-y-4">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-2xl bg-white/10 flex items-center justify-center text-emerald-400 font-bold border border-white/10">
                    <MapPin className="w-6 h-6" />
                  </div>
                  <div>
                    <span className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Lokasi Utama RSUD</span>
                    <h4 className="text-lg font-black tracking-wide">Bukit Kerman, Kerinci</h4>
                  </div>
                </div>
                <p className="text-xs text-slate-300 leading-relaxed">
                  Sistem secara otomatis menghitung koordinat GPS perangkat pegawai saat melakukan absensi seluler dan membandingkannya dengan radius maksimum yang dikonfigurasi.
                </p>
                <div className="pt-2 flex items-center gap-2">
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/20 text-emerald-300 font-mono text-xs font-bold border border-emerald-500/30">
                    <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span> Geofencing Aktif
                  </span>
                </div>
              </div>

              <div className="space-y-4">
                <h4 className="text-xs font-black uppercase text-slate-700 tracking-wider">Daftar Radius Shift Terdaftar</h4>
                <div className="space-y-3">
                  {settings.map((s) => (
                    <div key={s.id} className="bg-slate-50 border border-slate-200/80 rounded-2xl p-4 flex items-center justify-between text-xs">
                      <div>
                        <strong className="text-slate-900 block font-bold">Shift {s.nama_shift} ({s.hari})</strong>
                        <span className="text-[11px] text-slate-500">Maksimal jarak GPS yang diizinkan</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="font-mono font-black text-emerald-700 bg-emerald-50 px-3 py-1.5 rounded-xl border border-emerald-200">
                          {s.radius_meter} Meter
                        </span>
                        <button 
                          onClick={() => { setEditingSetting(s); setShiftModal(true); }}
                          className="px-3 py-1.5 bg-slate-900 hover:bg-slate-800 text-white font-bold rounded-xl cursor-pointer transition"
                        >
                          Ubah Radius
                        </button>
                      </div>
      ...
      {/* MODAL EDIT / TAMBAH SHIFT & RADIUS (LEBIH LUAS DAN RAPI) */}
      {shiftModal && editingSetting && (
        <div className="fixed inset-0 z-70 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-2xl w-full p-8 shadow-2xl space-y-6 text-xs animate-in fade-in max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center border-b border-slate-100 pb-4">
              <div>
                <h3 className="text-base font-black uppercase text-slate-900">
                  {editingSetting.id ? 'Ubah Konfigurasi Shift & Radius' : 'Tambah Konfigurasi Shift Baru'}
                </h3>
                <p className="text-slate-500 text-[11px]">Sesuaikan jam kerja dan parameter radius geofencing secara presisi.</p>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <div>
                <label className="font-bold text-slate-700 block mb-1.5 uppercase text-[10px] tracking-wider">Hari Berlaku</label>
                <input type="text" value={editingSetting.hari} onChange={(e) => setEditingSetting({ ...editingSetting, hari: e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3 font-bold text-slate-900 focus:outline-none focus:border-purple-500" placeholder="Contoh: Senin - Jumat / Setiap Hari" />
              </div>

              <div>
                <label className="font-bold text-slate-700 block mb-1.5 uppercase text-[10px] tracking-wider">Jam Masuk</label>
                <input type="text" value={editingSetting.jam_masuk} onChange={(e) => setEditingSetting({ ...editingSetting, jam_masuk: e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3 font-mono font-bold text-purple-700 focus:outline-none focus:border-purple-500" placeholder="07:30" />
              </div>
              <div>
                <label className="font-bold text-slate-700 block mb-1.5 uppercase text-[10px] tracking-wider">Jam Pulang</label>
                <input type="text" value={editingSetting.jam_pulang} onChange={(e) => setEditingSetting({ ...editingSetting, jam_pulang: e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3 font-mono font-bold text-purple-700 focus:outline-none focus:border-purple-500" placeholder="14:00" />
              </div>

              <div>
                <label className="font-bold text-slate-700 block mb-1.5 uppercase text-[10px] tracking-wider">Batas Awal Masuk</label>
                <input type="text" value={editingSetting.batas_awal_masuk} onChange={(e) => setEditingSetting({ ...editingSetting, batas_awal_masuk: e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3 font-mono text-slate-700 focus:outline-none focus:border-purple-500" placeholder="06:00" />
              </div>
              <div>
                <label className="font-bold text-slate-700 block mb-1.5 uppercase text-[10px] tracking-wider">Batas Akhir Masuk</label>
                <input type="text" value={editingSetting.batas_akhir_masuk} onChange={(e) => setEditingSetting({ ...editingSetting, batas_akhir_masuk: e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3 font-mono text-slate-700 focus:outline-none focus:border-purple-500" placeholder="08:00" />
              </div>

              <div>
                <label className="font-bold text-slate-700 block mb-1.5 uppercase text-[10px] tracking-wider">Batas Awal Pulang</label>
                <input type="text" value={editingSetting.batas_awal_pulang} onChange={(e) => setEditingSetting({ ...editingSetting, batas_awal_pulang: e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3 font-mono text-slate-700 focus:outline-none focus:border-purple-500" placeholder="14:00" />
              </div>
              <div>
                <label className="font-bold text-slate-700 block mb-1.5 uppercase text-[10px] tracking-wider">Batas Akhir Pulang</label>
                <input type="text" value={editingSetting.batas_akhir_pulang} onChange={(e) => setEditingSetting({ ...editingSetting, batas_akhir_pulang: e.target.value })} className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3 font-mono text-slate-700 focus:outline-none focus:border-purple-500" placeholder="18:00" />
              </div>

              <div>
                <label className="font-bold text-slate-700 block mb-1.5 uppercase text-[10px] tracking-wider">Radius GPS Maksimum (Meter)</label>
                <input type="number" value={editingSetting.radius_meter} onChange={(e) => setEditingSetting({ ...editingSetting, radius_meter: Number(e.target.value) })} className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3 font-mono font-bold text-emerald-700 focus:outline-none focus:border-purple-500" placeholder="150" />
              </div>
              <div>
