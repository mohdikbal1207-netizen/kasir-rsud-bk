...
export default function AdminAbsensiPage() {
  ...
  const handleExportCSV = () => {
    ...
    const headers = ["Nama Pegawai", "Jabatan", "Shift", "Kehadiran", "Keterangan", "Status", "Tanggal"];
    const rows = records.map(r => [`"${r.nama_pegawai}"`, `"${r.jabatan}"`, r.shift, r.status_kehadiran, `"${r.keterangan}"`, r.status_verifikasi, r.created_at]);
          ...
          <div className="border border-slate-200 rounded-2xl overflow-hidden">
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 font-medium">
                {isLoading ? (
                  <tr><td colSpan={6} className="text-center py-8 text-slate-400">Memuat rekap absensi...</td></tr>
                ) : paginatedRecords.length === 0 ? (
                  <tr><td colSpan={6} className="text-center py-12 text-slate-400">Tidak ada data absensi ditemukan.</td></tr>
                ) : (
                  paginatedRecords.map((r) => (
                    <tr key={r.id} className="hover:bg-slate-50">
                      <td className="p-3.5 font-bold text-slate-900">
                        ...
                        {r.email_user && <span className="block text-[9px] text-purple-600 font-mono">Akun: {r.email_user}</span>}
                      </td>
