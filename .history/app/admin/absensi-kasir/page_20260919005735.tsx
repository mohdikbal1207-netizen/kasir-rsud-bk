...
export default function AdminAbsensiPage() {
  ...
  const [officeLocation, setOfficeLocation] = useState<OfficeLocation>({
    ...
    latitude: -2.1894798609722,
    longitude: 101.557932135093,
    radius_meter: 150
  });
  const [isSavingLocation, setIsSavingLocation] = useState<boolean>(false);

  // Interactive Leaflet Map Ref (High-Res Satellite View like Google Maps)
  ...
  // Interactive Satellite Map Initialization Effect (Esri World Imagery HD)
  useEffect(() => {
    ...
    function initMap() {
      ...
      if (!mapInstanceRef.current) {
        ...
        // Menggunakan Esri World Imagery (Satelit HD ala Google Maps Satellite View)
  ...
  const handleSaveSetting = async () => {
    ...
    try {
      const payload = {
        ...
        radius_meter: Number(editingSetting.radius_meter || 150),
      ...
      showToast(`Gagal menyimpan shift: ${errorMsg}`, 'error');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between selection:bg-purple-500 selection:text-white">
      ...
      <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 flex-1 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 print:hidden">
          ...
          >
            ...
            <div className="text-left">
              ...
              <span className={`text-[10px] ${activeTab === 'radius' ? 'text-purple-200' : 'text-slate-400'}`}>Peta satelit &amp; radius</span>
            </div>
          </button>
        </div>
        ...
        {/* KONTEN TAB 3: RADIUS & GEOFENCING GPS (DENGAN PETA SATELIT HD) */}
        {activeTab === 'radius' && (
          <div className="bg-white border border-slate-200/80 rounded-3xl p-6 sm:p-8 shadow-sm space-y-6 animate-in fade-in">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">Klik langsung pada peta satelit atau seret marker untuk menentukan titik koordinat absolut instansi.</p>
              </div>
              <button
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 bg-slate-100 rounded-2xl overflow-hidden border border-slate-200 shadow-inner relative flex flex-col min-h-[400px]">
                <div className="absolute top-4 left-4 z-40 bg-slate-900/90 backdrop-blur-sm text-white px-4 py-2 rounded-xl text-xs font-bold shadow-lg flex items-center gap-2 border border-slate-700 pointer-events-none">
                  ...
                  <span>Satellite HD: Klik peta atau geser pin marker</span>
                </div>
