...
export default function AdminVerificationPage() {
  ...
  // State untuk Modal Interaktif (Pengganti Prompt Bawaan Browser)
  ...
  // Buka Modal Persetujuan & Atur Role
  const openApproveModal = (user: PendingUser) => {
    setSelectedUser(user);
    setAssignedRole(user.role || user.profiles?.unit_kerja || 'Kasir / Staf');
  ...
  // Eksekusi Persetujuan Akun dengan Role Baru
  const handleConfirmVerify = async () => {
    ...
    try {
      // 1. Update status aktif dan role pada tabel users
      ...
      // 2. Sinkronkan unit kerja di tabel profiles jika diubah
      const { error: profileError } = await supabase
        .from('profiles')
        .update({ unit_kerja: assignedRole })
        .eq('id', selectedUser.id);

      if (profileError) throw profileError;

      const namaStaf = selectedUser.profiles?.nama_lengkap || 'Staf';
      setMessage({ type: 'success', text: `Akun pegawai atas nama "${namaStaf}" berhasil disetujui dengan hak akses [${assignedRole}].` });
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between selection:bg-emerald-500 selection:text-white relative overflow-hidden">
      ...
      {/* --- MODAL INTERAKTIF: PERSETUJUAN & PENENTUAN ROLE --- */}
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-6 space-y-5 animate-in fade-in zoom-in duration-200 border border-slate-100">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-500">
                Tentukan atau konfirmasikan hak akses role / unit penugasan resmi untuk akun ini sebelum diaktifkan ke dalam sistem SIMRS.
              </p>
            </div>

            <div className="space-y-1.5">
              <label className="text-[11px] font-bold text-slate-600 uppercase tracking-wider block">
                Hak Akses / Unit Tugas (Role)
              </label>
              <div className="relative">
                ...
                <input 
                  ...
                  placeholder="Contoh: Kasir, Perawat, Dokter, IT Admin"
                  className="w-full bg-slate-50 border border-slate-300 rounded-2xl pl-10 pr-3.5 py-2.5 text-xs text-slate-800 font-medium focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500"
                />
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 pt-2">
              ...
              <button
                ...
                disabled={isSubmittingModal || !assignedRole.trim()}
