...
export default function AdminVerificationPage() {
  ...
  // Fungsi Tambahan: Reset Password Langsung oleh Admin
  const handleResetPassword = async (id: string, nama: string) => {
    const newPassword = prompt(`Masukkan kata sandi baru (minimal 6 karakter) untuk staf "${nama || 'Staf'}":`);
    ...
    try {
      ...
      setMessage({ type: 'success', text: `Kata sandi untuk akun "${nama || 'Staf'}" berhasil diatur ulang.` });
    } catch (err: unknown) {
      if (err instanceof Error) {
              ...
              return (
                ...
                >
                  ...
                  <div className="space-y-2 pt-2 border-t border-slate-100">
                    ...
                    {/* Tombol Tambahan: Atur Ulang Sandi Staf */}
                    <button
                      onClick={() => handleResetPassword(item.id, profile?.nama_lengkap || '')}
                      disabled={isActionProcessing}
