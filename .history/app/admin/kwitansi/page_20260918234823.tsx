...
export default function AdminKwitansiPage() {
  ...
  const handleExportCSV = () => {
    ...
    const headers = ["Nomor Bukti", "Kode Akun", "Sudah Terima Dari", "Untuk Pembayaran", "Jumlah", "Lembar", "Status Verifikasi"];
    const rows = records.map(r => [r.nomor_bukti, r.kode_akun, `"${r.sudah_terima_dari}"`, `"${r.untuk_pembayaran}"`, r.jumlah, `"${r.keterangan_lembar}"`, r.status_verifikasi]);
          ...
          <div className="border border-slate-200 rounded-2xl overflow-hidden">
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 font-medium">
                {isLoading ? (
                  <tr><td colSpan={8} className="text-center py-8 text-slate-400">Memuat data master...</td></tr>
                ) : filteredRecords.length === 0 ? (
                  <tr><td colSpan={8} className="text-center py-12 text-slate-400">Tidak ada data kwitansi ditemukan.</td></tr>
                ) : (
