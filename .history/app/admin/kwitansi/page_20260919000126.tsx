...
export default function AdminKwitansiPage() {
  ...
  const [viewingRecord, setViewingRecord] = useState<KwitansiRecord | null>(null);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === '/' && document.activeElement?.tagName !== 'INPUT' && document.activeElement?.tagName !== 'TEXTAREA') {
        e.preventDefault();
        document.getElementById('search-admin-input')?.focus();
      }
    };
  ...
  const handleCopyNomor = (nomor: string) => {
    ...
    setCopiedId(nomor);
    showToast(`Nomor bukti ${nomor} disalin ke clipboard.`, 'info');
  ...
  const filteredRecords = records.filter(r => {
    ...
    if (sortBy === 'nominal-tertinggi') return (b.jumlah || 0) - (a.jumlah || 0);
    if (sortBy === 'nominal-terendah') return (a.jumlah || 0) - (b.jumlah || 0);
    return 0;
  });
  ...
  // Statistik Ringkas Admin & Analisis Akun
  ...
  const totalATK = records.filter(r => r.kode_akun === '5.2.2.01.01' && r.status_verifikasi === 'Disetujui').reduce((acc, curr) => acc + (curr.jumlah || 0), 0);
  const totalBahanObat = records.filter(r => r.kode_akun === '5.2.2.02.01' && r.status_verifikasi === 'Disetujui').reduce((acc, curr) => acc + (curr.jumlah || 0), 0);
  const totalJasaMedis = records.filter(r => r.kode_akun === '5.2.2.03.01' && r.status_verifikasi === 'Disetujui').reduce((acc, curr) => acc + (curr.jumlah || 0), 0);

  // Estimasi Pagu Plafon Anggaran BLUD untuk Analisis Progress Bar
  const estimatedPlafon = 1000000000;
  const percentageUsed = Math.min(Math.round((totalApprovedNominal / estimatedPlafon) * 100), 100);
      ...
      <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 flex-1 space-y-6">
        <div className="bg-white border border-slate-200/80 rounded-3xl p-6 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 print:hidden">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <button
              onClick={() => { setShowAnalyticsModal(true); addLog('Membuka laporan analisis anggaran per akun.'); }}
              className="bg-purple-50 hover:bg-purple-100 text-purple-700 font-bold text-xs px-4 py-3 rounded-2xl transition flex items-center gap-2 border border-purple-200 cursor-pointer"
            >
              <BarChart3 className="w-4 h-4 text-purple-600" /> Analisis Akun BLUD
            </button>
        ...
        <div className="bg-white border border-slate-200/80 rounded-3xl p-6 shadow-sm space-y-4 print:hidden">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto flex-wrap">
              ...
              {/* FILTER TANGGAL ADMIN & SHORTCUTS */}
              <div className="relative w-full sm:w-auto flex items-center gap-2 flex-wrap">
                ...
                <button 
                  ...
                  className="bg-purple-50 hover:bg-purple-100 text-purple-700 font-bold text-[10px] px-2.5 py-2 rounded-xl transition cursor-pointer"
                  title="Filter Hari Ini"
                >
                  Hari Ini
                </button>
                <button 
                  onClick={() => { setFilterDate(new Date().toISOString().slice(0, 7)); setCurrentPage(1); }}
                  className="bg-purple-50 hover:bg-purple-100 text-purple-700 font-bold text-[10px] px-2.5 py-2 rounded-xl transition cursor-pointer"
              ...
              <div className="relative w-full sm:w-auto flex items-center gap-2">
                ...
                >
                  ...
                  <option value="nominal-terendah">Nominal: Terendah</option>
                </select>
              </div>
            </div>
                    ...
                    <tr key={r.id} className="hover:bg-slate-50">
                      ...
                      <td className="p-3.5 font-mono font-bold text-slate-900">
                        <div className="flex items-center gap-1.5">
                          ...
                          <button 
                            onClick={() => handleCopyNomor(r.nomor_bukti)} 
                            className="p-1 hover:bg-slate-200 text-slate-400 hover:text-slate-700 rounded transition cursor-pointer"
                            title="Salin Nomor Bukti"
                          >
                            {copiedId === r.nomor_bukti ? <Check className="w-3 h-3 text-emerald-600" /> : <Copy className="w-3 h-3" />}
                          </button>
                        </div>
                      ...
                      <td className="p-3.5 text-center">
                        ...
                        }`} title={r.status_verifikasi === 'Ditolak' && r.catatan_penolakan ? `Catatan: ${r.catatan_penolakan}` : ''}>
                          {r.status_verifikasi} {r.status_verifikasi === 'Ditolak' && r.catatan_penolakan && ' ℹ️'}
                        </span>
                      </td>
                      <td className="p-3.5 text-center">
                        <div className="flex items-center justify-center gap-1">
                          <button onClick={() => setViewingRecord(r)} className="p-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl cursor-pointer" title="Pratinjau Detail"><Eye className="w-3.5 h-3.5 text-slate-600" /></button>
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-slate-200 space-y-4 animate-in fade-in">
            <div className="flex items-center justify-between border-b pb-3">
              <h3 className="text-sm font-black uppercase text-slate-900 flex items-center gap-2">
                <Receipt className="w-5 h-5 text-purple-600" /> Detail Arsip Kwitansi #{viewingRecord.nomor_bukti}
              </h3>
              <button onClick={() => setViewingRecord(null)} className="p-1 hover:bg-slate-100 rounded-full cursor-pointer"><X className="w-5 h-5 text-slate-500" /></button>
            </div>

            <div className="space-y-3 text-xs">
              <div className="grid grid-cols-2 gap-3 bg-slate-50 p-3 rounded-2xl border">
                <div>
                  <span className="text-slate-400 block text-[10px] uppercase font-bold">Nomor Bukti</span>
                  <span className="font-mono font-bold text-slate-900">{viewingRecord.nomor_bukti}</span>
                </div>
                <div>
                  <span className="text-slate-400 block text-[10px] uppercase font-bold">Kode Akun</span>
                  <span className="font-mono font-bold text-slate-900">{viewingRecord.kode_akun}</span>
                </div>
              </div>

              <div>
                <span className="text-slate-400 block text-[10px] uppercase font-bold">Sudah Terima Dari</span>
                <p className="font-bold text-slate-800 uppercase">{viewingRecord.sudah_terima_dari}</p>
              </div>

              <div>
                <span className="text-slate-400 block text-[10px] uppercase font-bold">Untuk Pembayaran</span>
                <p className="text-slate-700 uppercase leading-relaxed">{viewingRecord.untuk_pembayaran}</p>
              </div>

              <div className="grid grid-cols-2 gap-3 bg-purple-50 p-3 rounded-2xl border border-purple-200">
                <div>
                  <span className="text-purple-600 block text-[10px] uppercase font-bold">Jumlah Nominal</span>
                  <span className="font-mono font-black text-purple-900 text-sm">{formatRupiah(viewingRecord.jumlah)}</span>
                </div>
                <div>
                  <span className="text-purple-600 block text-[10px] uppercase font-bold">Keterangan Lembar</span>
                  <span className="font-bold text-purple-900">{viewingRecord.keterangan_lembar}</span>
                </div>
              </div>

              <div>
                <span className="text-slate-400 block text-[10px] uppercase font-bold">Terbilang (Banyaknya Uang)</span>
                <p className="italic uppercase bg-slate-50 p-2 rounded-xl border text-slate-700">{viewingRecord.banyaknya_uang}</p>
              </div>

              {viewingRecord.status_verifikasi === 'Ditolak' && viewingRecord.catatan_penolakan && (
                <div className="bg-rose-50 border border-rose-200 p-3 rounded-xl text-rose-800">
                  ...
                  <p>{viewingRecord.catatan_penolakan}</p>
                </div>
              )}
            </div>

            <div className="flex justify-end gap-2 pt-2 border-t">
              <button onClick={() => { setPrintRecord(viewingRecord); setViewingRecord(null); }} className="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-xl text-xs font-bold cursor-pointer shadow">Cetak Resmi</button>
              <button onClick={() => setViewingRecord(null)} className="px-4 py-2 bg-slate-200 text-slate-700 rounded-xl text-xs font-bold cursor-pointer">Tutup</button>
            </div>
          </div>
        </div>
      )}

      {/* MODAL ANALISIS KEUANGAN PER KODE AKUN & PROGRESS BAR PLAFON */}
      {showAnalyticsModal && (
        <div className="fixed inset-0 z-70 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-slate-200 space-y-5 animate-in fade-in">
            <div className="flex items-center justify-between border-b pb-3">
              <h3 className="text-sm font-black uppercase text-slate-900 flex items-center gap-2">
                <BarChart3 className="w-5 h-5 text-purple-600" /> Analisis Keuangan Akun BLUD RSUD Bukit Kerman
              </h3>
            ...
            {/* PROGRESS BAR ESTIMASI PLAFON ANGGARAN */}
            <div className="bg-purple-50 border border-purple-200 rounded-2xl p-4 space-y-2">
              <div className="flex justify-between text-xs font-bold text-purple-900">
                <span>Penyerapan Terhadap Pagu Plafon BLUD</span>
                <span className="font-mono">{percentageUsed}% ({formatRupiah(totalApprovedNominal)})</span>
              </div>
              <div className="w-full bg-purple-200 h-2.5 rounded-full overflow-hidden">
                <div className="bg-purple-600 h-full rounded-full transition-all duration-500" style={{ width: `${percentageUsed}%` }}></div>
              </div>
              <p className="text-[10px] text-purple-700">Estimasi Plafon Operasional: {formatRupiah(estimatedPlafon)}</p>
            </div>

            <div className="space-y-3 text-xs">
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 flex items-center justify-between">
                <div>
                  <span className="text-slate-400 uppercase text-[10px] font-bold">5.2.2.01.01 (Belanja ATK)</span>
                  <h4 className="text-base font-black font-mono text-slate-900 mt-0.5">{formatRupiah(totalATK)}</h4>
                </div>
                <div className="w-9 h-9 bg-purple-100 text-purple-700 rounded-xl flex items-center justify-center font-bold">ATK</div>
              </div>

              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 flex items-center justify-between">
                <div>
                  <span className="text-slate-400 uppercase text-[10px] font-bold">5.2.2.02.01 (Belanja Bahan &amp; Obat)</span>
                  <h4 className="text-base font-black font-mono text-slate-900 mt-0.5">{formatRupiah(totalBahanObat)}</h4>
                </div>
                <div className="w-9 h-9 bg-emerald-100 text-emerald-700 rounded-xl flex items-center justify-center font-bold">OBT</div>
              </div>

              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 flex items-center justify-between">
                <div>
                  <span className="text-slate-400 uppercase text-[10px] font-bold">5.2.2.03.01 (Belanja Jasa Medis)</span>
                  <h4 className="text-base font-black font-mono text-slate-900 mt-0.5">{formatRupiah(totalJasaMedis)}</h4>
                </div>
                <div className="w-9 h-9 bg-sky-100 text-sky-700 rounded-xl flex items-center justify-center font-bold">JAS</div>
              </div>
            </div>

            <div className="flex justify-end pt-2 border-t">
              <button onClick={() => setShowAnalyticsModal(false)} className="px-5 py-2.5 bg-purple-600 hover:bg-purple-700 text-white text-xs font-bold rounded-xl cursor-pointer shadow">Tutup Analisis</button>
            </div>
          </div>
        </div>
      )}
