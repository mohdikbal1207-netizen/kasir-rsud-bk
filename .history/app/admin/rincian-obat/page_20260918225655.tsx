...
export default function AdminRincianObatPage() {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, statusFilter, serviceFilter, dateFilter, startDateFilter, endDateFilter]);
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between selection:bg-purple-500 selection:text-white">
      ...
      <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 flex-1 space-y-6">
        ...
        <div className="bg-white border border-slate-200/80 rounded-3xl p-6 shadow-sm space-y-4">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-2xl text-xs font-bold">
                ...
                <button onClick={() => setDateFilter('hari_ini')} className={`px-2.5 py-1.5 rounded-xl transition cursor-pointer flex items-center gap-1 ${dateFilter === 'hari_ini' ? 'bg-white text-purple-800 shadow-sm' : 'text-slate-500'}`}><Calendar className="w-3 h-3 text-purple-600" /> Hari Ini</button>
                <button onClick={() => setDateFilter('custom')} className={`px-2.5 py-1.5 rounded-xl transition cursor-pointer ${dateFilter === 'custom' ? 'bg-white text-purple-800 shadow-sm' : 'text-slate-500'}`}>Kustom</button>
              </div>
          ...
          {dateFilter === 'custom' && (
            <div className="bg-purple-50/70 border border-purple-200 p-4 rounded-2xl flex flex-wrap items-center gap-4 text-xs font-bold">
          ...
          {selectedIds.length > 0 && (
            <div className="bg-purple-600 text-white p-3 rounded-2xl flex items-center justify-between text-xs font-bold shadow-md">
              ...
              <thead>
                <tr className="bg-slate-100 text-slate-700 font-black uppercase border-b border-slate-200 text-[10px]">
                  ...
                  <th onClick={() => handleSort('created_at')} className="p-3.5 cursor-pointer hover:bg-slate-200 transition">
                    <div className="flex items-center gap-1">ID / Transaksi <ArrowUpDown className="w-3 h-3 text-slate-400" /></div>
                  </th>
                  <th onClick={() => handleSort('nama_pasien')} className="p-3.5 cursor-pointer hover:bg-slate-200 transition">
                    <div className="flex items-center gap-1">Pasien &amp; RM <ArrowUpDown className="w-3 h-3 text-slate-400" /></div>
                  </th>
          ...
          {sortedRecords.length > 0 && (
            <div className="flex flex-col sm:flex-row items-center justify-between pt-4 border-t border-slate-100 text-xs">
