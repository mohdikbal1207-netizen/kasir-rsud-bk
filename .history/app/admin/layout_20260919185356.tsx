...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const [activeUserRole, setActiveUserRole] = useState<string>('Administrator');
  ...
  useEffect(() => {
    ...
    if (isAuthorized) {
      checkLatency();
      latencyInterval = setInterval(checkLatency, 30000); // Cek tiap 30 detik
    }
  ...
  // Fitur Baru: Broadcast Channel Sync Antar-Tab Browser
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    async function verifyAdminAccess() {
      try {
        ...
        if (isMounted) {
          setActiveUserRole('Administrator SIMRS');
          setIsAuthorized(true);
          setIsLoading(false);
        }
      } catch {
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center text-white space-y-4 select-none">
        ...
        <div className="text-center space-y-1">
          ...
          <p className="text-[11px] text-slate-400">Memverifikasi Hak Akses Administrator SIMRS...</p>
        </div>
      </div>
    );
  }
    ...
    <>
      ...
      {!isOnline && (
        <div className="bg-rose-600 text-white text-center py-2 px-4 text-xs font-bold flex items-center justify-center gap-2 sticky top-0 z-50 shadow-lg animate-pulse">
          ...
          <span>Koneksi Internet Terputus! Beberapa fungsi sinkronisasi realtime terhenti.</span>
        </div>
      )}
      ...
      {/* Floating Status Bar Tambahan (Hanya Tampil di Desktop) */}
      <div className="fixed bottom-4 left-4 z-40 hidden lg:flex items-center gap-3 bg-slate-900/90 border border-slate-800 backdrop-blur-md px-3.5 py-2 rounded-2xl shadow-2xl text-[11px] text-slate-300 font-mono print:hidden selection:bg-none">
        ...
        <button 
          ...
          title="Kunci Layar Instan (Pintasan: Ctrl + L)"
        >
          <Lock className="w-3.5 h-3.5 text-amber-400" /> Lock (Ctrl+L)
        </button>
      </div>

      {/* Modal Peringatan Sesi Hampir Habis */}
      {showTimeoutWarning && !isQuickLocked && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-slate-900 border border-amber-500/30 p-6 rounded-3xl shadow-2xl w-full max-w-sm text-slate-100 space-y-4 text-center">
            ...
            <div>
              <h4 className="font-black text-sm text-white">Sesi Akan Segera Berakhir</h4>
            ...
            <div className="bg-slate-950 border border-slate-800 py-3 rounded-2xl relative overflow-hidden">
              <span className="text-[10px] uppercase font-bold tracking-wider text-slate-500 block mb-0.5">Otomatis Logout Dalam</span>
              <span className="text-2xl font-black font-mono text-amber-400 tracking-wider">
                {formatCountdown(secondsRemaining)}
              </span>
              
              {/* Linear Visual Progress Indicator */}
              <div 
            ...
            <p className="text-xs text-slate-300 leading-relaxed">
              Tekan tombol di bawah ini untuk terus melanjutkan pekerjaan di Portal Administrator.
            </p>
            ...
            <button 
              ...
              className="w-full bg-emerald-600 hover:bg-emerald-500 text-white py-3 rounded-2xl text-xs font-bold transition cursor-pointer shadow-lg shadow-emerald-600/20 flex items-center justify-center gap-2"
            >
            ...
            <form onSubmit={handleUnlockScreen} className="space-y-4 text-left">
              <div>
                ...
                <div className="relative">
                  <input 
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded-2xl pl-4 pr-12 py-3 text-xs text-white focus:outline-none focus:border-emerald-500 transition placeholder-slate-600"
                  />
                  <button
                    type="button"
              ...
              <div className="flex gap-3 pt-1">
                <button
                  ...
                  className="w-1/3 bg-slate-800 hover:bg-slate-700 text-slate-300 py-3 rounded-2xl text-xs font-bold transition cursor-pointer flex items-center justify-center gap-1.5"
                >
                ...
                <button
                  type="submit"
                  className="w-2/3 bg-emerald-600 hover:bg-emerald-500 text-white py-3 rounded-2xl text-xs font-bold transition cursor-pointer shadow-lg shadow-emerald-600/20 flex items-center justify-center gap-1.5"
                >
      ...
      {/* Modal Konfirmasi Logout Manual saat Layar Terkunci */}
      {showLogoutConfirm && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl shadow-2xl w-full max-w-sm text-center space-y-4">
            ...
            <div>
              <h4 className="font-bold text-sm text-white">Konfirmasi Keluar</h4>
              <p className="text-xs text-slate-400 mt-1">Apakah Anda yakin ingin mengakhiri sesi Administrator ini?</p>
            </div>
            <div className="flex gap-3 pt-2">
              <button
                type="button"
                onClick={() => setShowLogoutConfirm(false)}
                className="w-1/2 bg-slate-800 hover:bg-slate-700 text-slate-300 py-2.5 rounded-xl text-xs font-bold transition cursor-pointer"
              >
                Batal
              </button>
              <button
                ...
                className="w-1/2 bg-rose-600 hover:bg-rose-500 text-white py-2.5 rounded-xl text-xs font-bold transition cursor-pointer shadow-lg shadow-rose-600/20"
              >
                Ya, Keluar
              </button>
            </div>
