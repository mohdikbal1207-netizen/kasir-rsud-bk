...
export default function AdminRawatJalanVerification() {
  ...
  const [activeTab, setActiveTab] = useState<'pending' | 'lunas' | 'semua'>('pending');
  ...
  const totalNominal = filtered.reduce((acc, curr) => acc + Number(curr.total_biaya || 0), 0);
      ...
      <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 flex-1 space-y-6 relative z-10 pb-24 print:p-0 print:m-0 print:max-w-none">
        ...
        <div className="bg-white/90 border border-slate-200 rounded-3xl p-6 shadow-xl backdrop-blur-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4 print:hidden">
          ...
          <div className="flex items-center gap-2 bg-slate-100 p-1.5 rounded-2xl border border-slate-200">
            ...
            >
              ...
              <span>Pending Verifikasi ({totalPending})</span>
            </button>
            <button
            ...
            >
              ...
              <span>Sudah Lunas ({totalLunas})</span>
            </button>
            <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 print:hidden">
          <div className="md:col-span-2 bg-white border border-slate-200 rounded-2xl p-3 shadow-sm flex items-center space-x-3">
            ...
            <input 
              type="text" 
              placeholder="Cari berdasarkan Nama Pasien, No. RM, atau ID Transaksi..."
              value={searchTerm}
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-3.5 shadow-sm flex items-center justify-between">
            <div className="space-y-0.5">
              <p className="text-[10px] font-bold text-slate-400 uppercase">Jumlah Pasien Tampil</p>
              <p className="text-base font-black text-slate-800">{filtered.length} Pasien</p>
            </div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-3.5 shadow-sm flex items-center justify-between">
            <div className="space-y-0.5">
              <p className="text-[10px] font-bold text-slate-400 uppercase">Total Akumulasi Nominal</p>
        ...
        {isLoading ? (
          <div className="text-center py-16 text-slate-400 font-medium flex flex-col items-center space-y-2 print:hidden">
            ...
            <span>Memuat data transaksi dari database...</span>
          </div>
        ) : filtered.length === 0 ? (
          <div className="bg-white border border-slate-200 rounded-3xl p-12 text-center space-y-3 shadow-sm print:hidden">
            ...
            <p className="text-xs text-slate-400">Belum ada input transaksi rawat jalan terbaru dari modul kasir.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 print:hidden">
            {filtered.map(tx => {
              ...
              return (
                <div key={tx.id} className="bg-white border border-slate-200/90 rounded-3xl p-6 shadow-md hover:shadow-xl transition space-y-4 flex flex-col justify-between">
                  <div className="space-y-3">
                    ...
                    <div className="bg-slate-50 border border-slate-200 rounded-2xl p-3 text-xs space-y-1 font-mono">
                      ...
                      <div className="flex justify-between font-bold pt-1 border-t border-slate-200 text-slate-900">
                        ...
                        <span className="text-teal-700 font-black">{formatRupiah(tx.total_biaya)}</span>
                      </div>
                    </div>
                  </div>
