...
export default function AdminRawatJalanVerification() {
  ...
  // Fitur Baru: Verifikasi Massal (Batch Approve)
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between selection:bg-teal-500 selection:text-white relative overflow-hidden">
      ...
      <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 flex-1 space-y-6 relative z-10 pb-24 print:p-0 print:m-0 print:max-w-none">
        ...
        {/* TOOLBAR TAMBAHAN: FILTER METODE, TANGGAL, & EKSPOR EXCEL */}
        {activeTab !== 'pasien' && (
          <div className="bg-white/80 border border-slate-200 rounded-2xl p-3.5 shadow-sm backdrop-blur-md flex flex-col sm:flex-row items-center justify-between gap-3 print:hidden">
            ...
            <button
              ...
              className="px-4 py-2 bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-200 text-xs font-bold rounded-xl transition flex items-center space-x-2 cursor-pointer w-full sm:w-auto justify-center"
            >
              <FileSpreadsheet className="w-4 h-4 text-emerald-600" />
              <span>Ekspor Laporan CSV ({filteredTransactions.length})</span>
            </button>
          </div>
        )}
