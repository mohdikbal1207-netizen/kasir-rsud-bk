...
export default function AdminRawatJalanPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      console.error(err);
      setMessage({ type: 'error', text: 'Gagal memuat data verifikasi Rawat Jalan.' });
    } finally {
      setIsLoading(false);
    }
  }, [activeTab]);
  ...
  const handleApprove = async (tx: TransactionItem) => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: `Transaksi Rawat Jalan ${tx.id} (${tx.nama_pasien}) disetujui LUNAS.` });
      fetchData();
    } catch (err: unknown) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between selection:bg-teal-500 selection:text-white relative overflow-hidden">
      <div className="absolute top-0 left-1/4 w-[600px] h-[600px] bg-teal-200/40 rounded-full blur-[140px] pointer-events-none -z-10"></div>
      
      <AdminHeader 
        title="RSUD BUKIT KERMAN" 
        subtitle="Verifikasi Admin — Rawat Jalan"
      ...
      <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 flex-1 space-y-6 relative z-10 pb-24">
        <div className="bg-white/90 border border-slate-200/80 rounded-3xl p-6 shadow-xl shadow-slate-200/50 backdrop-blur-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <h1 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight flex items-center space-x-2">
              ...
              <span>Verifikasi Pembayaran Rawat Jalan</span>
            </h1>
            <p className="text-xs text-slate-500">Pusat audit dan persetujuan billing poliklinik dan resep obat jalan.</p>
          </div>
          <button
            onClick={fetchData}
            disabled={isLoading}
        ...
        {message && (
          <div className={`p-4 rounded-2xl text-xs flex items-start space-x-2.5 border shadow-sm ${
            message.type === 'success' ? 'bg-teal-50 border-teal-200 text-teal-800' : 'bg-rose-50 border-rose-200 text-rose-800'
          }`}>
            {message.type === 'success' ? <CheckCircle2 className="w-4 h-4 text-teal-600 flex-shrink-0 mt-0.5" /> : <AlertCircle className="w-4 h-4 text-rose-500 flex-shrink-0 mt-0.5" />}
        ...
        {isLoading ? (
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-12 text-center space-y-3 shadow-sm">
            ...
            <h2 className="text-sm font-bold text-slate-700">Tidak ada data verifikasi Rawat Jalan.</h2>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((tx) => (
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full uppercase ${
                      tx.status_bayar === 'lunas' ? 'bg-teal-50 text-teal-700 border border-teal-200' : 'bg-amber-50 text-amber-700 border border-amber-200'
                    }`}>
                  ...
                  <div className="space-y-0.5">
                    ...
                    <p className="text-xs text-teal-700 font-mono font-semibold">No. RM: {tx.no_rm}</p>
                  </div>
                  ...
                  {tx.status_bayar === 'pending' ? (
                    <>
                      <button
                        ...
                        className="flex-1 bg-teal-600 hover:bg-teal-700 text-white font-bold text-xs py-2.5 px-3 rounded-2xl transition shadow-md flex items-center justify-center space-x-1.5 cursor-pointer disabled:opacity-50"
                      >
                    ...
                    <div className="w-full text-center py-2 bg-teal-50 border border-teal-200 text-teal-800 rounded-2xl text-xs font-bold flex items-center justify-center space-x-1.5">
