...
export default function ExecutiveManagementDashboard() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between selection:bg-emerald-500 selection:text-white print:bg-white">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {rajalData
                      .filter(item => item.nama_pasien?.toLowerCase().includes(searchRajal.toLowerCase()) || item.no_rm?.includes(searchRajal) || item.poli_tujuan?.toLowerCase().includes(searchRajal.toLowerCase()))
                      .map((item, idx) => (
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-md space-y-4">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {igdData
                      .filter(item => item.nama_pasien?.toLowerCase().includes(searchIgd.toLowerCase()) || item.no_rm?.includes(searchIgd) || item.dokter_pemeriksa?.toLowerCase().includes(searchIgd.toLowerCase()))
                      .map((item, idx) => (
                      <tr key={idx} className="hover:bg-slate-50 transition">
            ...
            <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-md space-y-4">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {ranapData
                      .filter(item => item.nama_pasien?.toLowerCase().includes(searchRanap.toLowerCase()) || item.no_reg?.includes(searchRanap) || item.ruang?.toLowerCase().includes(searchRanap.toLowerCase()))
                      .map((item, idx) => (
                      <tr key={idx} className="hover:bg-slate-50 transition">
