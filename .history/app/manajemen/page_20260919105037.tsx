...
const extractDateUtility = (curr: Record<string, any>) => {
...
export default function ExecutiveManagementDashboard() {
  ...
  const [pageRajal, setPageRajal] = useState(1);
  const [pageIgd, setPageIgd] = useState(1);
  const [pageRanap, setPageRanap] = useState(1);
  ...
  useEffect(() => setPageRajal(1), [searchRajal, sortConfig, filterPeriod]);
  useEffect(() => setPageIgd(1), [searchIgd, sortConfig, filterPeriod]);
  useEffect(() => setPageRanap(1), [searchRanap, sortConfig, filterPeriod]);
  ...
  const rajalChartData = useMemo(() => {
    const grouped = currentRajalData.reduce((acc: Record<string, any>, curr: any) => {
  ...
  const triaseDistro = useMemo(() => {
    const grouped = currentIgdData.reduce((acc: Record<string, any>, curr: any) => {
  ...
  const topDiagnosaData = useMemo(() => {
    const grouped = currentRanapData.reduce((acc: Record<string, any>, curr: any) => {
  ...
  const paginatedRajal = filteredRajal.slice((pageRajal - 1) * ITEMS_PER_PAGE, pageRajal * ITEMS_PER_PAGE);
  const paginatedIgd = filteredIgd.slice((pageIgd - 1) * ITEMS_PER_PAGE, pageIgd * ITEMS_PER_PAGE);
  const paginatedRanap = filteredRanap.slice((pageRanap - 1) * ITEMS_PER_PAGE, pageRanap * ITEMS_PER_PAGE);

  const renderPaginationFooter = (currentPage: number, setPage: (p: number | ((prev: number) => number)) => void, totalItems: number) => {
    ...
    return (
      <div className="flex flex-col sm:flex-row items-center justify-between px-4 py-3 bg-white border-t border-slate-200 print:hidden sticky bottom-[44px] z-10 shadow-[0_-4px_6px_-1px_rgb(0,0,0,0.05)] rounded-b-xl">
        <span className="text-xs font-medium text-slate-500 mb-2 sm:mb-0">
          Menampilkan <strong className="text-slate-700">{startItem} - {endItem}</strong> dari <strong className="text-slate-700">{totalItems}</strong> entri
        </span>
        <div className="flex items-center gap-1.5 bg-slate-50 border border-slate-200 p-1 rounded-xl">
          <button
            onClick={() => setPage(p => Math.max(1, p - 1))}
            disabled={currentPage === 1}
            className="p-1.5 rounded-lg text-slate-600 disabled:opacity-30 hover:bg-white hover:shadow-sm transition"
          >
            <ChevronLeft className="w-4 h-4" />
          </button>
          <span className="text-[11px] font-bold text-slate-700 px-2 min-w-[60px] text-center">
  ...
  const getFilterLabel = () => {
    if (filterPeriod === 'hari_ini') return 'Data Hari Ini';
    if (filterPeriod === 'minggu_ini') return '7 Hari Terakhir';
  ...
  const maxQtyFarmasi = topObatData.length > 0 ? Math.max(...topObatData.map(o => o.jumlah)) : 1;
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between selection:bg-emerald-500 selection:text-white print:bg-white relative">
      ...
      <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 flex-1 space-y-6 relative print:p-0 print:m-0 print:w-full print:max-w-none">
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-md flex flex-col md:flex-row items-start md:items-center justify-between gap-4 print:hidden relative overflow-hidden">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="flex items-center bg-slate-100 border border-slate-200 rounded-2xl px-3 py-2 gap-2 text-xs font-semibold text-slate-600 hover:bg-slate-200 transition-colors cursor-pointer">
        ...
        {activeTab === 'rajal' && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
                    ...
                    <ComposedChart data={rajalChartData} margin={{ top: 15, right: 15, left: 20, bottom: 10 }}>
                      ...
                      <Tooltip 
                        formatter={(v: any, name: any) => [
                          String(name) === 'Pendapatan' ? `Rp ${Number(v).toLocaleString('id-ID')}` : `${v} Pasien`, 
                          String(name)
                        ]} 
                        contentStyle={tooltipStyle} 
                      />
            ...
            <div className="bg-white border border-slate-200 rounded-3xl print:rounded-xl p-6 shadow-sm space-y-4">
              ...
              <div className="overflow-x-auto max-h-[600px] print:max-h-none overflow-y-auto rounded-t-xl border-x border-t border-slate-200 custom-scrollbar relative">
                <table className="w-full text-left text-xs">
                  <thead className="bg-slate-100 text-slate-600 uppercase font-bold sticky top-0 z-20 shadow-sm print:shadow-none print:border-b print:border-slate-300">
                    <tr>
                  ...
                  <tbody className="divide-y divide-slate-100 print:divide-slate-200 bg-white">
                    {paginatedRajal.length > 0 ? (
                      paginatedRajal.map((item, idx) => (
                  ...
                  {filteredRajal.length > 0 && (
                    <tfoot className="bg-slate-800 text-white font-bold sticky bottom-0 z-20 shadow-[0_-4px_6px_-1px_rgb(0,0,0,0.1)] print:shadow-none print:text-slate-900 print:bg-slate-100">
                      <tr>
              ...
              {renderPaginationFooter(pageRajal, setPageRajal, filteredRajal.length)}
            </div>
          </div>
        )}
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={triaseDistro} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={60} outerRadius={90} paddingAngle={3}>
                          {triaseDistro.map((entry: any, index: number) => {
            ...
            <div className="bg-white border border-slate-200 rounded-3xl print:rounded-xl p-6 shadow-sm space-y-4">
              ...
              <div className="overflow-x-auto max-h-[600px] print:max-h-none overflow-y-auto rounded-t-xl border-x border-t border-slate-200 custom-scrollbar relative">
                <table className="w-full text-left text-xs">
                  ...
                  <tbody className="divide-y divide-slate-100 print:divide-slate-200 bg-white">
                    {paginatedIgd.length > 0 ? (
                      paginatedIgd.map((item, idx) => (
                        <tr key={idx} className="hover:bg-slate-50 transition-colors print:break-inside-avoid">
              ...
              {renderPaginationFooter(pageIgd, setPageIgd, filteredIgd.length)}
            </div>
          </div>
        )}
            ...
            <div className="bg-white border border-slate-200 rounded-3xl print:rounded-xl p-6 shadow-sm space-y-4">
              ...
              <div className="overflow-x-auto max-h-[600px] print:max-h-none overflow-y-auto rounded-t-xl border-x border-t border-slate-200 custom-scrollbar relative">
                <table className="w-full text-left text-xs">
                  ...
                  <tbody className="divide-y divide-slate-100 print:divide-slate-200 bg-white">
                    {paginatedRanap.length > 0 ? (
                      paginatedRanap.map((item, idx) => (
                        <tr key={idx} className="hover:bg-slate-50 transition-colors print:break-inside-avoid">
              ...
              {renderPaginationFooter(pageRanap, setPageRanap, filteredRanap.length)}
            </div>
          </div>
        )}
                ...
                <table className="w-full text-left text-xs">
                  <thead className="bg-slate-100 text-slate-600 uppercase font-bold print:border-b print:border-slate-300">
                    <tr>
                      ...
                      <th className="p-3 text-right w-48">Kuantitas Keluar (Unit)</th>
                      <th className="p-3 text-right w-56">Subtotal Estimasi Nilai</th>
                    </tr>
                  </thead>
                    ...
                    {topObatData.map((item, idx) => (
                      <tr key={idx} className="hover:bg-slate-50 transition-colors print:break-inside-avoid">
                        ...
                        <td className="p-3 text-right">
                          <div className="font-mono font-bold text-purple-600">{item.jumlah}</div>
                          <div className="w-full bg-purple-100 rounded-full h-1 mt-1 overflow-hidden flex justify-end print:hidden">
                            <div className="bg-purple-500 h-full rounded-full transition-all duration-1000" style={{ width: `${(item.jumlah / maxQtyFarmasi) * 100}%` }} />
                          </div>
                        </td>
      ...
      {toastMessage && (
        <div className="fixed bottom-6 right-6 z-50 animate-in slide-in-from-bottom-5 fade-out duration-300 print:hidden">
          <div className="bg-emerald-600 text-white p-4 rounded-2xl shadow-xl flex items-start gap-3 w-80 border border-emerald-500">
            ...
            <button onClick={() => setToastMessage(null)} className="text-emerald-200 hover:text-white transition bg-emerald-700/50 p-1 rounded-lg">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
