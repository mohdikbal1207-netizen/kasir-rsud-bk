...
export default function ExecutiveManagementDashboard() {
  ...
  // Derived Data for Area Chart (Trend Kunjungan - 3 Pilar)
  const trendKunjunganData = useMemo(() => {
    const grouped: Record<string, any> = {};

    // Helper untuk mengambil tanggal bersih (YYYY-MM-DD)
    const extractDate = (dateStr: string) => {
      if (!dateStr) return 'N/A';
      return dateStr.split(' ')[0] || dateStr.split('T')[0];
    };

    // Helper untuk mapping kunjungan per layanan
    const processData = (sourceData: any[], key: 'rajal' | 'igd' | 'ranap') => {
      sourceData.forEach((curr: any) => {
        // Fallback untuk kolom tanggal yang mungkin berbeda nama (tanggal / waktu_masuk / created_at)
        const dateRaw = curr.tanggal || curr.waktu_masuk || curr.tgl_masuk || curr.created_at || 'N/A';
    ...
    processData(rajalData, 'rajal');
    processData(igdData, 'igd');
    processData(ranapData, 'ranap');
    
    // Sort urut tanggal & konversi ke format mudah dibaca, ambil maksimal 14 hari
      ...
      <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 flex-1 space-y-6">
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6 animate-in fade-in">
            ...
            {/* ROW GRAFIK BAWAH (TREN KUNJUNGAN - GABUNGAN 3 PILAR) */}
            <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-md space-y-4">
              <div className="flex justify-between items-center">
                <h3 className="text-sm font-bold text-slate-800 uppercase flex items-center gap-2">
                  <LineIcon className="w-4 h-4 text-blue-500" /> Tren Kunjungan Pasien Harian (Rajal, IGD, Inap)
                </h3>
              </div>
