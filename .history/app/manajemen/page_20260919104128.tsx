...
import { 
  ...
  ArrowUpDown,
  Filter,
  ArrowUp, // TAMBAHAN: Ikon untuk Scroll to Top
  Timer // TAMBAHAN: Ikon untuk Auto-Refresh
} from 'lucide-react';
import { 
  ResponsiveContainer, 
  BarChart, 
...
export default function ExecutiveManagementDashboard() {
  ...
  // TAMBAHAN: Efek untuk Auto-Refresh (Live Kiosk Mode) - 60 Detik
  useEffect(() => {
    let interval: NodeJS.Timeout;
    if (isAutoRefresh) {
      interval = setInterval(() => {
        fetchAllData(false); // Silent refresh (tanpa overlay loading)
      }, 60000); 
    }
    return () => clearInterval(interval);
  }, [isAutoRefresh, fetchAllData]);

  // TAMBAHAN: Efek untuk mendeteksi scroll window (Scroll to Top)
  useEffect(() => {
    const handleScroll = () => {
  ...
  const filterByDatePeriod = useCallback((data: any[]) => {
    ...
    return data.filter(item => {
      ...
      if (!dateStr) return true; // Fallback jika benar-benar tidak ada tanggal
  ...
  // TAMBAHAN: Konfigurasi style khusus untuk Recharts Tooltip agar lebih elegan
  const tooltipStyle = {
    borderRadius: '12px',
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between selection:bg-emerald-500 selection:text-white print:bg-white relative">
            ...
            <div className="bg-white border border-slate-200 rounded-3xl print:rounded-xl p-6 shadow-sm space-y-4">
              ...
              <div className="overflow-x-auto max-h-[500px] print:max-h-none overflow-y-auto rounded-xl border border-slate-200 custom-scrollbar relative">
                <table className="w-full text-left text-xs">
                  ...
                  {/* TAMBAHAN: Sticky Summary Footer Tabel */}
                  {filteredRajal.length > 0 && (
            ...
            <div className="bg-white border border-slate-200 rounded-3xl print:rounded-xl p-6 shadow-sm space-y-4">
              ...
              <div className="overflow-x-auto max-h-[500px] print:max-h-none overflow-y-auto rounded-xl border border-slate-200 custom-scrollbar relative">
                <table className="w-full text-left text-xs">
                  ...
                  {/* TAMBAHAN: Sticky Summary Footer Tabel IGD */}
                  {filteredIgd.length > 0 && (
                    <tfoot className="bg-slate-800 text-white font-bold sticky bottom-0 z-10 shadow-[0_-4px_6px_-1px_rgb(0,0,0,0.1)] print:shadow-none print:text-slate-900 print:bg-slate-100">
                      <tr>
            ...
            <div className="bg-white border border-slate-200 rounded-3xl print:rounded-xl p-6 shadow-sm space-y-4">
              ...
              <div className="overflow-x-auto max-h-[500px] print:max-h-none overflow-y-auto rounded-xl border border-slate-200 custom-scrollbar relative">
                <table className="w-full text-left text-xs">
                  ...
                  {/* TAMBAHAN: Sticky Summary Footer Tabel RANAP */}
                  {filteredRanap.length > 0 && (
                    <tfoot className="bg-slate-800 text-white font-bold sticky bottom-0 z-10 shadow-[0_-4px_6px_-1px_rgb(0,0,0,0.1)] print:shadow-none print:text-slate-900 print:bg-slate-100">
                      <tr>
                        <td colSpan={6} className="p-3 text-right uppercase tracking-wider text-[11px]">Grand Total (Filtered):</td>
      ...
      {/* Tailwind global classes for scrollbar & Advanced Print CSS */}
      <style dangerouslySetInnerHTML={{__html: `
        ...
        @media print {
          ...
          /* Cegah tabel terpotong jelek di tengah-tengah text */
          table { page-break-inside: auto; width: 100%; border-collapse: collapse; }
          tr { page-break-inside: avoid; page-break-after: auto; }
          thead { display: table-header-group; }
          tfoot { display: table-footer-group; }
          
          /* Hilangkan bayangan untuk menghemat tinta */
