...
export default function ExecutiveManagementDashboard() {
  ...
  // Derived Data for Area Chart (Trend Kunjungan)
  const trendKunjunganData = useMemo(() => {
    const grouped = rajalData.reduce((acc: any, curr: any) => {
      const date = curr.tanggal || 'N/A';
      if (date !== 'N/A') {
        if (!acc[date]) acc[date] = { tanggal: date, kunjungan: 0 };
        acc[date].kunjungan += 1;
      }
      return acc;
    }, {});
    ...
    return Object.values(grouped)
      ...
      .slice(-14); // Ambil 14 hari terakhir agar grafik rapi
  }, [rajalData]);
      ...
      <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 flex-1 space-y-6">
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6 animate-in fade-in">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-md space-y-4 flex flex-col justify-between">
                ...
                <div className="w-full h-56 flex-1">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      {/* Inner radius dibuat 55 agar bolong tengahnya besar (Donut Chart) */}
            ...
            {/* ROW GRAFIK BAWAH (TREN KUNJUNGAN - BARU DITAMBAHKAN) */}
            <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-md space-y-4">
              <h3 className="text-sm font-bold text-slate-800 uppercase flex items-center gap-2">
                <LineIcon className="w-4 h-4 text-blue-500" /> Tren Kunjungan Pasien Harian (Rawat Jalan)
              </h3>
              <div className="w-full h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={trendKunjunganData} margin={{ top: 15, right: 15, left: 0, bottom: 0 }}>
                    <defs>
                    ...
                    <Tooltip formatter={(v: any) => [`${v} Pasien`, 'Kunjungan']} />
                    <Area type="monotone" dataKey="kunjungan" stroke="#3B82F6" strokeWidth={3} fillOpacity={1} fill="url(#colorKunjungan)" name="Kunjungan" />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
