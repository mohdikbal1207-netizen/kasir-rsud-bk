...
export default function DaftarPage() {
  ...
  // Fungsi Memanggil API Internal Fonnte WhatsApp Gateway dengan Payload Lengkap
  const sendWhatsAppNotification = async (payload: {
    nama_lengkap: string;
    unit_kerja: string;
    nip: string;
    email: string;
    ...
    try {
      ...
      if (!res.ok || result.success === false) {
        console.error('Gagal mengirim WhatsApp dari server:', result.message || result.error);
      }
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Unggah Foto ke Supabase Storage (Public Bucket) jika ada
      if (selectedFile) {
      ...
      setLoadingStepText('Memproses pendaftaran akun pegawai...');

      // 2. Kirim seluruh data pendaftaran ke Server API Route (Bypass RLS Client-Side)
      const res = await fetch('/api/daftar', {
        ...
        body: JSON.stringify({
          ...
          no_telepon: formatPhoneNumber(sanitizedData.no_telepon),
      ...
      if (!res.ok || result.error) {
        throw new Error(result.error || 'Gagal memproses pendaftaran akun.');
      }

      // 3. Kirim Notifikasi WhatsApp Otomatis ke Admin IT melalui API Route
      ...
      const detailedMessage = err?.message || 'Terjadi kesalahan sistem.';
          ...
          <div className="bg-white/95 border border-slate-200/90 rounded-3xl p-6 sm:p-8 shadow-xl shadow-slate-200/60 backdrop-blur-xl relative overflow-hidden">
            ...
            <div className="mb-6 p-3.5 bg-sky-50 border border-sky-200 rounded-2xl text-xs text-sky-800 flex items-start space-x-2.5 shadow-sm">
              ...
              <div>
                ...
                Setiap pendaftaran baru memerlukan konfirmasi dari Administrator IT. Setelah pendaftaran berhasil dikirim, Anda dapat berkoordinasi langsung dengan bagian IT untuk pengaktifan hak akses portal kasir/BLUD.
              </div>
            </div>
