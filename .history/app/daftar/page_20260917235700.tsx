...
export default function DaftarPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 3. Cek duplikasi Email di tabel users
      setLoadingStepText('Memeriksa ketersediaan email...');
      ...
      if (existingUserEmail) {
        throw new Error('Alamat email ini sudah terdaftar di sistem. Silakan login atau gunakan email lain.');
      }

      let publicPhotoUrl = '';

      // 4. Unggah Foto ke Supabase Storage jika ada
      if (selectedFile) {
      ...
      if (!authData.user) {
        throw new Error('Gagal mendapatkan ID kredensial pendaftaran.');
      }

      // Jika user sudah pernah terdaftar di Auth tapi belum menyelesaikan profil (identitas user identik tanpa identitas baru)
      if (authData.user.identities && authData.user.identities.length === 0) {
        throw new Error('Email ini sudah terdaftar di Supabase Auth. Silakan hapus akun lama dari Dashboard Supabase atau gunakan email lain.');
      }

      const userId = authData.user.id;

      // 6. Simpan data kredensial / role ke tabel `users`
      ...
      if (userError) throw new Error(`Gagal menyimpan tabel users: ${userError.message}`);

      // 7. Simpan profil lengkap ke tabel `profiles`
      ...
      if (profileError) throw new Error(`Gagal menyimpan tabel profiles: ${profileError.message}`);

      // 8. Kirim Notifikasi WhatsApp Otomatis ke Admin IT (Mohd. Ikbal)
      ...
      if (typeof err === 'string') {
        ...
        detailedMessage = err.error_description;
      } else {
        detailedMessage = JSON.stringify(err);
      }
      
      if (detailedMessage === '{}' || !detailedMessage) {
        detailedMessage = 'Email ini sudah terdaftar di Supabase Auth namun data profil belum lengkap. Silakan hapus user tersebut dari Dashboard Supabase atau gunakan email baru.';
      }
