...
export async function POST(request: Request) {
  try {
    ...
    // (Opsional: Sesuaikan role admin dengan sistem rumah sakit Anda, misal 'Administrator' atau 'IT')
    ...
    // 4. Cabut sesi Auth Supabase secara paksa (Memutus token aktif pengguna)
    const { error: signOutError } = await supabaseAdmin.auth.admin.signOut(userId);
    if (signOutError) throw signOutError;

    // 5. Perbarui status sesi di tabel custom menjadi non-aktif (`is_active = false`)
