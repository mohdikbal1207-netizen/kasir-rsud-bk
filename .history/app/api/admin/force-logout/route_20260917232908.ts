...
export async function POST(request: Request) {
  try {
    ...
    // 5. Validasi Eksistensi: Pastikan pengguna target benar-benar ada di Supabase Auth
    ...
    // 6. Cabut sesi Auth Supabase secara paksa (Memutus token aktif pengguna)
    const { error: signOutError } = await supabaseAdmin.auth.admin.signOut(userId);
    if (signOutError) {
      throw new Error(`Gagal mencabut sesi Auth: ${signOutError.message}`);
    }

    // 7. Perbarui status sesi di tabel custom menjadi non-aktif (`is_active = false`)
    ...
    try {
      ...
      // Abaikan jika tabel audit_logs belum dibuat agar fungsionalitas utama tidak terganggu
