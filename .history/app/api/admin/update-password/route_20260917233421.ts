...
export async function POST(request: Request) {
  try {
    ...
    // 6. Catat ke tabel Jejak Audit Keamanan (Audit Log) dengan informasi yang lebih detail
    try {
      await supabaseAdmin.from('audit_logs').insert([
        {
          ...
          description: `Admin ${callerUser.email} mengubah kata sandi untuk staf ber-email ${targetUser.user.email || userId}`,
