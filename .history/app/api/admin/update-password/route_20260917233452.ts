...
export async function POST(request: Request) {
  try {
    ...
    // 4. Validasi Eksistensi: Pastikan pengguna target benar-benar ada di sistem Auth
    ...
    // 5. Ubah kata sandi pengguna langsung melalui fungsi Admin Supabase Auth
    ...
    // 6. Cabut seluruh sesi aktif perangkat pengguna target secara paksa demi keamanan
    const { error: signOutError } = await supabaseAdmin.auth.admin.signOut(userId);
    if (signOutError) {
      console.warn(`Peringatan: Gagal mencabut sesi aktif setelah reset password untuk ${userId}:`, signOutError.message);
    }

    // 7. Perbarui status sesi di tabel custom (jika ada tabel pelacakan sesi)
    await supabaseAdmin
      .from('user_sessions')
    ...
    // 8. Catat ke tabel Jejak Audit Keamanan (Audit Log)
    try {
    ...
    return NextResponse.json({ 
      success: true, 
      message: 'Kata sandi pengguna berhasil diperbarui dan sesi aktif mereka telah diakhiri.' 
    });
