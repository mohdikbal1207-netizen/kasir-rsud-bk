...
// Fungsi helper untuk membersihkan karakter khusus WhatsApp Markdown dari input pengguna
const sanitizeMarkdown = (text?: string) => {
  if (!text) return '-';
  return text.replace(/[*_~`]/g, ''); // Menghapus karakter *, _, ~, ` agar format pesan tidak rusak
};
...
export async function POST(request: Request) {
  try {
    ...
    // Sanitasi input pengguna dari markdown berbahaya untuk format WhatsApp
    const safeNama = sanitizeMarkdown(nama_lengkap);
    const safeUnit = sanitizeMarkdown(unit_kerja);
    const safeNip = sanitizeMarkdown(nip);
    const safeEmail = sanitizeMarkdown(email);
    ...
    // Mode Mock / Dry-Run untuk pengujian lokal (opsional di .env.local)
    if (process.env.NODE_ENV === 'development' && process.env.WHATSAPP_MOCK_MODE === 'true') {
      console.log('[MOCK WHATSAPP SUCCESS]', { target: adminPhones, message: serverGeneratedMessage });
      return NextResponse.json({ 
        success: true, 
        message: 'Mock WhatsApp berhasil dicetak di konsol server (Development Mode).' 
      });
    }
