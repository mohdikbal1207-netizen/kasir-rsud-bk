...
export default function KasirRawatJalanPage() {
  ...
  // Dynamic Tindakan (3.1 dst)
  const [tindakanList, setTindakanList] = useState<{ nama: string; biaya: number }[]>([
    { nama: '', biaya: 0 }
  ]);

  // Dynamic Penunjang Lainnya (5.6, 5.7 dst)
  const [penunjangLainnyaList, setPenunjangLainnyaList] = useState<{ nama: string; biaya: number }[]>([]);

  // State untuk Data Cetak PDF/Dokumen Pasien Individu
  const [selectedTxPrint, setSelectedTxPrint] = useState<TransaksiRajal | null>(null);

  // FITUR BARU: State untuk Seleksi Multi Pasien & Cetak Rekapitulasi
  const [selectedTxIds, setSelectedTxIds] = useState<string[]>([]);
  const [printMode, setPrintMode] = useState<'single' | 'recap'>('single');

  // Helper Formatting Angka Ribuan dengan Titik (contoh: 15000 -> "15.000")
  ...
  // Hitung Total Form Input Pasien Baru
  const hitungTotal = () => {
  ...
  // FITUR BARU: Logika Kalkulasi Dan Seleksi Pasien
  const filteredTransactions = transactions.filter(t => 
    t.nama_pasien?.toLowerCase().includes(searchTerm.toLowerCase()) || 
    t.no_rm?.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const totalAllFiltered = filteredTransactions.reduce((acc, curr) => acc + (Number(curr.total_biaya) || 0), 0);

  const selectedTransactions = transactions.filter(t => selectedTxIds.includes(t.id));
  const totalSelectedBiaya = selectedTransactions.reduce((acc, curr) => acc + (Number(curr.total_biaya) || 0), 0);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      fetchRiwayat();
      // Opsi untuk Cetak Langsung data individu pasien setelah simpan
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between selection:bg-emerald-500 selection:text-white relative overflow-hidden">
      ...
      <main className="w-full max-w-5xl mx-auto px-4 sm:px-6 py-8 flex-1 space-y-6 relative z-10 pb-24 print:p-0 print:m-0 print:max-w-none">
        ...
        {/* NOTIFIKASI SUCCESS/ERROR & TOMBOL CETAK INSTAN INDIVIDU PASIEN */}
        {message && (
          <div className={`p-4 rounded-2xl text-xs font-medium border shadow-sm print:hidden flex flex-col sm:flex-row sm:items-center justify-between gap-3 ${
            ...
            {message.lastTx && (
              ...
              >
                ...
                <span>Cetak Dokumen Pasien Ini ({message.lastTx.nama_pasien})</span>
              </button>
            )}
          </div>
        )}

        {/* TAB 1: INPUT FORM MODUL (DILIHAT SAAT EDIT/INPUT) */}
        {activeTab === 'form' ? (
          <form onSubmit={handleSubmit} className="bg-white border border-slate-200/90 rounded-3xl p-6 sm:p-8 shadow-xl shadow-slate-200/50 space-y-8 print:hidden">
            ...
            {/* TABEL ITEM RINCIAN BIAYA DENGAN INPUT TITIK (FORMAT RIBUAN) */}
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              >
                ...
                <span>Simpan &amp; Cetak Perincian Pasien Ini</span>
              </button>
            </div>
          </form>
        ) : (
          /* TAB 2: DAFTAR RIWAYAT TAGIHAN DENGAN FITUR CEKLIS & HITUNG TOTAL OTOMATIS */
          <div className="space-y-6 print:hidden">
            
            {/* WIDGET AKUMULASI TOTAL BIAYA PASIEN */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Card 1: Total Seluruh Pasien */}
              <div className="bg-gradient-to-br from-slate-900 to-slate-800 text-white p-5 rounded-3xl shadow-lg border border-slate-700 flex items-center justify-between">
                <div className="space-y-1">
                  <span className="text-[11px] font-bold tracking-wider text-slate-400 uppercase flex items-center space-x-1.5">
                    ...
                    <span>Total Keseluruhan ({filteredTransactions.length} Pasien)</span>
                  </span>
                  <p className="text-2xl font-black font-mono text-emerald-400 tracking-tight">
                    {formatRupiah(totalAllFiltered)}
                  </p>
                  <p className="text-[10px] text-slate-400">
                    {searchTerm ? `Berdasarkan filter pencarian: "${searchTerm}"` : 'Akumulasi seluruh transaksi rawat jalan'}
                  </p>
                </div>
              ...
              {/* Card 2: Total Pasien Terpilih (Diceklis) */}
              <div className="bg-gradient-to-br from-emerald-600 to-teal-700 text-white p-5 rounded-3xl shadow-lg border border-emerald-500 flex items-center justify-between">
                <div className="space-y-1">
                  <span className="text-[11px] font-bold tracking-wider text-emerald-100 uppercase flex items-center space-x-1.5">
                    <ListChecks className="w-4 h-4 text-white" />
                    <span>Total Pasien Terpilih ({selectedTxIds.length} Pasien)</span>
                  </span>
                  <p className="text-2xl font-black font-mono text-white tracking-tight">
                    {formatRupiah(totalSelectedBiaya)}
                  </p>
                  <p className="text-[10px] text-emerald-100">
                    Centang kotak pasien untuk menghitung total gabungan khusus
                  </p>
                </div>
            ...
            {/* CONTROL PANEL: BAR PENCARIAN & SELEKSI MASAL */}
            <div className="bg-white/90 border border-slate-200 rounded-3xl p-4 shadow-sm backdrop-blur-md flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                return (
                  <div 
                    ...
                    className={`bg-white border rounded-3xl p-6 shadow-sm space-y-4 flex flex-col justify-between transition ${
                      isSelected ? 'border-2 border-emerald-500 bg-emerald-50/20 shadow-md' : 'border-slate-200 hover:border-slate-300'
                    }`}
                  >
                    <div className="space-y-3">
                      <div className="flex justify-between items-start gap-2">
                        {/* Checkbox Tombol Pilih Pasien */}
                        <button
                          type="button"
                        ...
                        <span className="text-[11px] font-mono font-bold text-slate-400 shrink-0">{tx.id}</span>
                      </div>
                      ...
                      <div className="flex justify-between items-center pt-2 border-t border-slate-100 text-xs">
                        ...
                        <span className="text-sm font-black text-slate-900 font-mono">{formatRupiah(tx.total_biaya)}</span>
                      </div>
                    </div>
                    ...
                    <div className="pt-2">
                      <button
                        ...
                        className="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold py-2 rounded-xl text-xs flex items-center justify-center space-x-1.5 transition cursor-pointer"
                      >
                        <Printer className="w-3.5 h-3.5 text-emerald-400" />
                        <span>Cetak Dokumen Pasien Ini ({tx.nama_pasien})</span>
                      </button>
                    </div>
                  </div>
                );
            ...
            {/* FLOATING ACTION BAR SAAT PASIEN DICEKLIS */}
            {selectedTxIds.length > 0 && (
      ...
      {/* CETAK DOKUMEN PDF INDIVIDU PASIEN */}
      {printMode === 'single' && (
              ...
              <div className="p-8 max-w-3xl mx-auto space-y-4">
                ...
                <table className="w-full border-collapse border border-black text-xs">
                  ...
                  <tbody className="divide-y divide-black font-sans">
                    ...
                    {/* Dynamic Penunjang Medik (6, 7 dst) */}
                    ...
                    <tr className="font-bold border-t-2 border-black">
                      ...
                      <td className="p-2 text-right font-mono text-sm">{formatRupiahTanpaSimbol(selectedTxPrint.total_biaya)}</td>
                    </tr>
                  </tbody>
                </table>
                  ...
                  <div className="space-y-4">
                    ...
                    {selectedTxPrint.ttd_petugas_url ? (
                      <div className="h-14 flex items-center justify-center">
                        <img src={selectedTxPrint.ttd_petugas_url} alt="TTD" className="max-h-full object-contain" />
                      </div>
                    ) : (
      ...
      {/* CETAK REKAPITULASI BIAYA MULTI-PASIEN TERPILIH */}
      {printMode === 'recap' && (
              ...
              <tbody className="divide-y divide-black font-sans">
                {selectedTransactions.map((tx, idx) => (
                  <tr key={tx.id}>
                    ...
                    <td className="p-2 text-right font-mono font-bold">{formatRupiahTanpaSimbol(tx.total_biaya)}</td>
                  </tr>
                ))}
