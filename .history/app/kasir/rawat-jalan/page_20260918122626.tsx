...
export default function KasirRawatJalanPage() {
  ...
  // Operator Active State (Ditambahkan field role)
  ...
  const totalAllFiltered = filteredTransactions
  ...
  const handleDeleteTx = async (id: string, namaPasien: string) => {
    ...
    if (!confirm(`[OTORISASI ADMIN] Yakin ingin membatalkan (Void) tagihan ${namaPasien} (${id})? Data akan ditandai dibatalkan pada audit trail.`)) return;
    
    try {
      // Soft-delete: Ubah status menjadi 'dibatalkan' agar historis akuntansi RS tetap terjaga
      const { error } = await supabase
        .from('transaksi_pasien')
              ...
              <div className="bg-gradient-to-br from-slate-900 to-slate-800 text-white p-5 rounded-3xl shadow-lg border border-slate-700 flex items-center justify-between">
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] text-slate-400">
                    Akumulasi transaksi aktif (tidak termasuk status dibatalkan)
                  </p>
                </div>
                  ...
                  >
                    <div className="space-y-3">
                      <div className="flex justify-between items-start gap-2">
                        ...
                        <div className="flex items-center space-x-2 shrink-0">
                          ...
                          {/* Tombol Hapus/Void Transaksi Khusus Admin */}
                    ...
                    <div className="pt-2">
                      <button
                        ...
                        className={`w-full font-bold py-2 rounded-xl text-xs flex items-center justify-center space-x-1.5 transition ${
                          isDibatalkan 
