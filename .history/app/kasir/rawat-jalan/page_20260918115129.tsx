...
export default function KasirRawatJalanPage() {
  ...
  // Form Input - Ruang / Poli diisi manual
  ...
  // Fitur Otomatisasi Pasien Berobat Berulang: Cek No. RM di Database
  const handleCekNoRM = async (noRM: string) => {
    ...
    try {
      const { data } = await supabase
        .from('transaksi_pasien')
        .select('nama_pasien, rincian_layanan')
        .eq('no_rm', noRM.trim())
        .order('tanggal_transaksi', { ascending: false })
        .limit(1);
      ...
      if (data && data.length > 0) {
        ...
        try {
          parsedRincian = typeof lastRecord.rincian_layanan === 'string' 
            ? JSON.parse(lastRecord.rincian_layanan) 
            : lastRecord.rincian_layanan;
        } catch {
          parsedRincian = null;
        }
        ...
        setFormData(prev => ({
          ...prev,
          nama_pasien: lastRecord.nama_pasien || prev.nama_pasien,
          umur: parsedRincian?.umur || prev.umur
        }));

        setMessage({ 
          type: 'success', 
          text: `Data pasien lama ditemukan! Nama: ${lastRecord.nama_pasien}. Silakan lengkapi perincian kunjungan baru ini.` 
        });
      }
    } catch (err) {
      console.error('Gagal mengecek No. RM:', err);
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // ID Unik Timestamp-based agar pasien yang sama bisa berobat berkali-kali tanpa terbentrok ID
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 bg-slate-50 border border-slate-200/80 p-4 rounded-2xl text-xs">
              <div className="space-y-1">
                <label className="font-bold text-slate-700 flex items-center justify-between">
                  <span>No. MR</span>
                  {isSearchingRM && <span className="text-[10px] text-emerald-600 font-normal">Mencari...</span>}
                </label>
                <input 
                  type="text" 
                  required
                    ...
                    <div className="space-y-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-emerald-700 font-mono font-semibold">
                          No. RM: {tx.no_rm} | Ruang/Poli: {tx.poli_tujuan}
                        </p>
                      </div>
