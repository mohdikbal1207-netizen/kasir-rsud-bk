...
export default function KasirRawatJalanPage() {
  ...
  const fetchRiwayat = useCallback(async () => {
    ...
    try {
      ...
      if (!error && data) {
        // Filter fleksibel jenis_layanan
  ...
  useEffect(() => {
    async function initOperatorAndData() {
      try {
        ...
        if (userId) {
          // 1. Coba ambil dari tabel `profiles` (nama_lengkap, nip)
          ...
          if (prof) {
            namaLengkap = prof.nama_lengkap || '';
            nipUser = prof.nip || prof.nik || '';
          }

          // 2. Jika tidak ada di profiles, ambil dari tabel `users`
          if (!namaLengkap) {
            ...
            if (uData) {
              namaLengkap = uData.nama_lengkap || '';
              roleUser = uData.role || 'kasir';
            }
          }
        }

        // Fallback jika session belum ada, cari user kasir 'KONIA' langsung dari DB
        if (!namaLengkap) {
          const { data: kasirDefault } = await supabase
            .from('profiles')
            .select('id, nama_lengkap, nip')
            .eq('nama_lengkap', 'KONIA')
            .maybeSingle();

          if (kasirDefault) {
            namaLengkap = kasirDefault.nama_lengkap;
            nipUser = kasirDefault.nip || '15588518525811158';
          }
        }

        setPetugasInfo({
          id: userId || 'ad790732-bcfa-415c-a1b6-c3bec907b109',
          nama: namaLengkap || 'KONIA',
          nip: nipUser || '15588518525811158',
          role: roleUser
        });
  ...
  // Ubah Status Bayar Langsung
  const handleUpdateStatusBayar = async (id: string, statusLama: string) => {
    if (petugasInfo.role !== 'admin' && petugasInfo.role !== 'superadmin' && petugasInfo.role !== 'kepala_kasir' && petugasInfo.role !== 'kasir') {
      alert('Akses Ditolak: Perubahan status verifikasi tagihan hanya dapat dilakukan oleh Admin / Kasir.');
      return;
    }
  ...
  // Hapus / Pembatalan Transaksi (Void)
  const handleDeleteTx = async (id: string, namaPasien: string) => {
    const alasan = prompt(`[OTORISASI KASIR] Masukkan alasan pembatalan (Void) tagihan ${namaPasien} (${id}):`, "Salah input nominal oleh petugas");
    ...
    try {
      const { error } = await supabase
        ...
        .update({ 
          ...
          catatan_admin: alasan.trim() || 'Dibatalkan oleh Kasir'
        })
        .eq('id', id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Simpan/Update Master Pasien di tabel `pasien`
      if (formData.no_rm.trim()) {
        await supabase.from('pasien').upsert([{
          no_rm: formData.no_rm.trim(),
      ...
      // 2. Payload Transaksi yang Kompatibel dengan Skema Database
      const payload = {
        id: txId,
        ...
        ttd_petugas_url: ttdDataUri || null,
  ...
  const isAdmin = petugasInfo.role === 'admin' || petugasInfo.role === 'superadmin' || petugasInfo.role === 'kepala_kasir' || petugasInfo.role === 'kasir';
