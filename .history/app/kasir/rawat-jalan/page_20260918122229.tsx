...
export default function KasirRawatJalanPage() {
  ...
  // FITUR BARU: State Filter Tambahan
  ...
  // FITUR BARU: Reset Form Otomatis
  const resetForm = () => {
  ...
  const handleCekNoRM = async (noRM: string) => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        setMessage({ 
          type: 'success', 
          text: `Data RM ${last.no_rm} (${last.nama_pasien}) ditemukan dan dimuat!` 
        });
      }
    } catch (err) {
  ...
  // FITUR BARU: Filter Multi Kriteria
  const filteredTransactions = transactions.filter(t => {
    ...
    return matchSearch && matchPenjaminan && matchTanggal;
  });
  ...
  // FITUR BARU: Ubah Status Bayar Langsung
  const handleUpdateStatusBayar = async (id: string, statusLama: string) => {
    ...
    try {
      ...
      setMessage({ 
        type: 'success', 
        text: `Status tagihan ${id} diperbarui menjadi ${nextStatus.toUpperCase()}` 
      });
      fetchRiwayat();
    } catch (err: any) {
  ...
  const handleDeleteTx = async (id: string, namaPasien: string) => {
    if (!confirm(`Yakin ingin menghapus data tagihan ${namaPasien} (${id})?`)) return;
    try {
      const { error } = await supabase.from('transaksi_pasien').delete().eq('id', id);
      if (error) throw error;

      setMessage({ type: 'success', text: `Tagihan ${id} berhasil dihapus!` });
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['ID Transaksi', 'No RM', 'Nama Pasien', 'NIK', 'Poli Tujuan', 'Penjaminan', 'Metode Bayar', 'Tanggal', 'Total Biaya (Rp)', 'Status'];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      resetForm(); // Reset otomatis setelah simpan
    } catch (err: unknown) {
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between items-start gap-2">
                        ...
                        >
                          ...
                          {/* Tombol Toggle Status Lunas / Pending */}
                          <button
                            type="button"
                            onClick={(e) => {
                              e.stopPropagation();
                            ...
                            className={`text-[10px] font-bold px-2.5 py-1 rounded-full uppercase flex items-center space-x-1 cursor-pointer transition ${
