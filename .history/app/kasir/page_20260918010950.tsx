...
interface TransactionItem {
  ...
  jenis_layanan?: 'rawat_jalan' | 'rawat_inap' | 'igd';
}
...
export default function KasirDashboardPage() {
  ...
  const [serviceFilter, setServiceFilter] = useState<'all' | 'rawat_jalan' | 'rawat_inap' | 'igd'>('all');
  ...
  const fetchDataKasir = useCallback(async () => {
    ...
    try {
      ...
      if (session?.user) {
        ...
        // 2. Catat / Perbarui Sesi Aktif ke Tabel user_sessions secara Otomatis (Menggunakan .limit(1) agar aman dari duplikasi baris)
        ...
        if (selectError) {
          console.error('Error checking existing session:', selectError.message);
        }
      ...
      if (error || !data || data.length === 0) {
        // Data Dummy fallback lengkap dengan kategori layanan untuk RSUD Bukit Kerman
        const dummyData: TransactionItem[] = [
          ...
          },
          {
            ...
            rincian_layanan: 'Biaya Kamar Inap 3 Hari + Visite Dokter + Keperawatan',
          ...
          },
          {
            ...
            dokter_penanggung_jawab: 'dr. Fikri (Dokter Jaga IGD)',
            total_biaya: 450000,
            status_bayar: 'pending',
            tanggal_transaksi: new Date().toISOString(),
            no_telepon: '085233445566',
            rincian_layanan: 'Penanganan Darurat + Infus + Injeksi Analgesik',
            jenis_layanan: 'igd'
          }
        ];
  ...
  const exportKasirCSV = () => {
    const headers = ['ID Transaksi', 'No RM', 'Nama Pasien', 'Poli', 'Total Biaya', 'Status', 'Metode Bayar', 'Kategori Layanan', 'Tanggal'];
  ...
  const filteredTransactions = transactions.filter((item) => {
    ...
    const matchesService = serviceFilter === 'all' ? true : item.jenis_layanan === serviceFilter;
  ...
  // Hitung jumlah per kategori layanan untuk badge grid card
  const countRawatJalan = transactions.filter(t => t.jenis_layanan === 'rawat_jalan').length;
  const countRawatInap = transactions.filter(t => t.jenis_layanan === 'rawat_inap').length;
  const countIGD = transactions.filter(t => t.jenis_layanan === 'igd').length;
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between selection:bg-emerald-500 selection:text-white relative overflow-hidden">
      ...
      <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 flex-1 space-y-6 relative z-10 pb-24">
        ...
        {/* ================= MODUL GRID CARD RINCIAN BIAYA (RAWAT JALAN, RAWAT INAP, IGD) ================= */}
        <div className="space-y-4">
          <div className="flex items-center justify-between px-1">
            <h2 className="text-sm font-bold text-slate-700 uppercase tracking-wider">
              Modul Kategori Rincian Biaya Layanan
            </h2>
            {serviceFilter !== 'all' && (
              <button 
              ...
              >
                Reset Filter Kategori (Tampilkan Semua)
              </button>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div 
              onClick={() => setServiceFilter(serviceFilter === 'rawat_jalan' ? 'all' : 'rawat_jalan')}
              className={`bg-white border rounded-3xl p-6 shadow-lg shadow-slate-200/50 flex flex-col justify-between space-y-6 transition cursor-pointer relative overflow-hidden group ${
                serviceFilter === 'rawat_jalan' ? 'border-emerald-500 ring-2 ring-emerald-500/20 bg-emerald-50/20' : 'border-slate-200/90 hover:border-emerald-400'
              }`}
            >
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="bg-emerald-100 text-emerald-800 font-bold text-[10px] px-2.5 py-1 rounded-full shadow-sm">
                    {countRawatJalan} Transaksi
                  </span>
                </div>
              ...
              <div className="pt-4 border-t border-slate-100 flex items-center justify-between text-xs font-bold text-emerald-600 group-hover:translate-x-1 transition-transform">
                <span>{serviceFilter === 'rawat_jalan' ? 'Filter Aktif' : 'Pilih Modul Rawat Jalan'}</span>
            ...
            <div 
              onClick={() => setServiceFilter(serviceFilter === 'rawat_inap' ? 'all' : 'rawat_inap')}
              className={`bg-white border rounded-3xl p-6 shadow-lg shadow-slate-200/50 flex flex-col justify-between space-y-6 transition cursor-pointer relative overflow-hidden group ${
                serviceFilter === 'rawat_inap' ? 'border-teal-500 ring-2 ring-teal-500/20 bg-teal-50/20' : 'border-slate-200/90 hover:border-teal-400'
              }`}
            >
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="bg-teal-100 text-teal-800 font-bold text-[10px] px-2.5 py-1 rounded-full shadow-sm">
                    {countRawatInap} Transaksi
                  </span>
                </div>
              ...
              <div className="pt-4 border-t border-slate-100 flex items-center justify-between text-xs font-bold text-teal-600 group-hover:translate-x-1 transition-transform">
                <span>{serviceFilter === 'rawat_inap' ? 'Filter Aktif' : 'Pilih Modul Rawat Inap'}</span>
                <ArrowRight className="w-4 h-4" />
              </div>
            </div>
            ...
            <div 
              onClick={() => setServiceFilter(serviceFilter === 'igd' ? 'all' : 'igd')}
              className={`bg-white border rounded-3xl p-6 shadow-lg shadow-slate-200/50 flex flex-col justify-between space-y-6 transition cursor-pointer relative overflow-hidden group ${
                serviceFilter === 'igd' ? 'border-amber-500 ring-2 ring-amber-500/20 bg-amber-50/20' : 'border-slate-200/90 hover:border-amber-400'
              }`}
            >
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="bg-amber-100 text-amber-800 font-bold text-[10px] px-2.5 py-1 rounded-full shadow-sm">
                    {countIGD} Transaksi
                  </span>
                </div>
              ...
              <div className="pt-4 border-t border-slate-100 flex items-center justify-between text-xs font-bold text-amber-600 group-hover:translate-x-1 transition-transform">
                <span>{serviceFilter === 'igd' ? 'Filter Aktif' : 'Pilih Modul IGD'}</span>
                <ArrowRight className="w-4 h-4" />
              </div>
            </div>
        ...
        <div className="bg-white/90 border border-slate-200/80 rounded-3xl p-6 shadow-xl shadow-slate-200/50 backdrop-blur-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <h1 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight">
              Manajemen Billing &amp; Transaksi Pasien {serviceFilter !== 'all' && `(${serviceFilter.replace('_', ' ').toUpperCase()})`}
            </h1>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="bg-white/90 border border-slate-200/80 rounded-3xl p-12 text-center space-y-4 shadow-sm backdrop-blur-xl">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-500">Semua pasien pada kategori atau filter ini telah melunasi atau belum ada antrean baru.</p>
            </div>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="bg-slate-100 text-slate-700 text-[10px] font-bold px-2 py-0.5 rounded-md uppercase">
                      {tx.jenis_layanan ? tx.jenis_layanan.replace('_', ' ') : 'umum'}
                    </span>
                  </div>
                  ...
                  <div className="bg-slate-50 border border-slate-200/70 rounded-2xl p-3.5 space-y-2 text-xs text-slate-600">
                    <div className="flex items-center space-x-1.5">
                      ...
                      <span className="truncate">Poli/Ruang: <strong className="text-slate-800">{tx.poli_tujuan}</strong></span>
                    </div>
                    <div className="flex items-center space-x-1.5">
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-6 space-y-4 animate-in fade-in zoom-in duration-200 border border-slate-100">
            ...
            <div className="bg-slate-50 p-4 rounded-2xl space-y-2 text-xs border border-slate-200/60 font-mono">
              ...
              <div className="flex justify-between">
                <span className="text-slate-500">Poli/Ruang:</span>
                <span className="font-bold text-slate-800">{completedTx.poli_tujuan}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Kategori:</span>
                <span className="font-bold uppercase text-emerald-700">{completedTx.jenis_layanan || 'Umum'}</span>
              </div>
              <div className="flex justify-between">
