...
export default function InputRawatInapPage() {
  ...
  const [billingItems, setBillingItems] = useState<BillingItem[]>([
    ...
    // III. Visit
    { id: '6', kategori: 'III. VISIT', uraian: '1. Visit Dokter Spesialis / 1', volume: 0, tarif: 0, ditanggung: 0 },
    ...
    { id: '29', kategori: 'VII. LAIN LAIN', uraian: 'e. Tindakan / Biaya Lainnya (Custom)', volume: 0, tarif: 0, ditanggung: 0 }
  ]);
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-slate-200 p-6 sm:p-8 space-y-6">
          ...
          <form onSubmit={handleSubmitToDatabase} className="space-y-6">
            ...
            {/* Tabel Perincian Biaya dengan Pembatas Kategori Rapi */}
                      ...
                      <>
                        {showCategoryHeader && (
                          <tr key={`cat-${item.kategori}`} className="bg-emerald-50/90 border-t-2 border-emerald-600">
                            <td colSpan={6} className="p-2.5 font-black text-emerald-900 tracking-wide text-xs">
                              {item.kategori}
                            </td>
                          </tr>
                        )}
