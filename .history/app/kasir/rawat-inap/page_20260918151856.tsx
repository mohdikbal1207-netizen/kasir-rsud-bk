...
export default function InputRawatInapPage() {
  ...
  // Fungsi Tambah Baris Dinamis Tepat di Dalam Kategori Terkait
  const handleAddRow = (kategoriTarget: string) => {
    ...
    setBillingItems(prev => {
      // Cari indeks terakhir dari kategori yang sama agar baris baru masuk di dalam kelompok kategori tersebut
      const lastIndex = prev.map(item => item.kategori).lastIndexOf(kategoriTarget);
      if (lastIndex !== -1) {
        const copy = [...prev];
  ...
  const handleSubmitToDatabase = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // 1. Simpan Header Pasien Ranap dengan onConflict no_reg
