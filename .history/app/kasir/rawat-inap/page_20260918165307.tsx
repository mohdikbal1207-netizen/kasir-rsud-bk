...
export default function InputRawatInapPage() {
  ...
  // STATE BARU: Mengubah Daftar Tagihan menjadi sistem POP-UP Modal yang elegan
  ...
  // FITUR: Cari Riwayat Pasien Berdasarkan NIK
  ...
  const loadSavedBillings = async () => {
    ...
    try {
      ...
      if (headers) {
        ...
        // Default centang semua data yang dimuat
  ...
  // Logika Hitung Otomatis Berdasarkan Ceklis Pasien di Pop-Up
  ...
  const selectedBillingsList = savedBillings.filter(b => selectedBillingIds.includes(b.no_reg));
  const totalBiayaTerpilih = selectedBillingsList.reduce((acc, curr) => acc + (curr.total_biaya || 0), 0);
  const totalTunaiTerpilih = selectedBillingsList
  ...
  const totalOnlineTerpilih = selectedBillingsList
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-100 font-sans print:bg-white relative">
      ...
      {/* POP-UP MODAL DAFTAR RIWAYAT TAGIHAN */}
      {showListModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in print:hidden">
          <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 max-w-5xl w-full max-h-[90vh] overflow-hidden flex flex-col transform transition-all">
            
            {/* Header Modal */}
            <div className="px-6 py-4 bg-slate-900 text-white flex items-center justify-between">
              <div className="flex items-center space-x-2">
                <FileText className="w-5 h-5 text-emerald-400" />
                <h3 className="text-sm font-black uppercase tracking-wider">Daftar Riwayat Tagihan Rawat Inap & Akumulasi Pembayaran</h3>
              </div>
              <button 
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1 bg-slate-50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-slate-900 text-white p-5 rounded-2xl shadow-md border border-slate-800 flex flex-col justify-between">
                  <div className="flex items-center justify-between text-slate-400 text-xs font-bold uppercase">
                    <span>Total Sesuai Filter ({selectedBillingIds.length} Pasien)</span>
                  ...
                  <div className="mt-3">
                    ...
                    <p className="text-[10px] text-slate-400 mt-1">Akumulasi total tagihan terpilih</p>
                  </div>
                </div>
              ...
              {/* Filter & Pencarian Di Dalam Pop-up */}
              <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-3">
                <div className="relative w-full sm:w-72">
                  ...
                  <input 
                    ...
                    className="w-full pl-9 pr-4 py-2 rounded-xl border border-slate-300 text-xs font-medium bg-slate-50"
                  />
                </div>
              ...
              {/* Tabel Data Pasien Di Dalam Pop-up */}
            ...
            <div className="px-6 py-4 bg-white border-t border-slate-200 flex items-center justify-between">
              <span className="text-xs font-bold text-slate-600">
                Terpilih: <strong className="text-emerald-700">{selectedBillingIds.length}</strong> dari {filteredBillings.length} pasien
              </span>
              <button
                onClick={() => setShowListModal(false)}
                className="bg-slate-900 hover:bg-slate-800 text-white px-6 py-2.5 rounded-xl text-xs font-bold transition shadow-md cursor-pointer"
              >
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        {/* NAVIGASI ATAS YANG LEBIH BERSIH & PROFESIONAL */}
        <div className="flex flex-wrap items-center justify-between gap-3 print:hidden">
          <div className="flex items-center space-x-2 bg-slate-200/80 p-1.5 rounded-2xl">
            ...
            <button
              ...
              onClick={() => {
                setPatientData({
                  ...
                  dokterMerawat: '',
                  kepalaRuangan: '',
                  bendahara: patientData.bendahara,
                  petugasEmail: patientData.petugasEmail,
                  signatureBase64: '',
                  statusVerifikasi: 'NEW',
                  catatanAdmin: ''
                });
            ...
            {/* Tombol Pemicu Pop-up Daftar Tagihan */}
            <button
              type="button"
              onClick={() => { setShowListModal(true); loadSavedBillings(); }}
              className="flex items-center space-x-2 bg-slate-900 hover:bg-slate-800 text-white px-5 py-2 rounded-xl text-xs font-bold transition shadow-sm cursor-pointer"
            >
        ...
        {/* KONTEN UTAMA FORMULIR INPUT RAWAT INAP */}
