...
export default function InputRawatInapPage() {
  ...
  // Ref untuk Canvas Tanda Tangan Digital
  ...
  // Fetch Nama Lengkap Akun Login dari Supabase Auth / Profile Table
  useEffect(() => {
    const fetchCurrentUser = async () => {
      try {
        ...
        if (user) {
          ...
          // Jika metadata belum diset, cek tabel profiles
          if (!fullName) {
            const { data: profile } = await supabase
              .from('profiles')
              .select('full_name, nama_lengkap')
              .eq('id', user.id)
              .single();
            
            if (profile) {
              fullName = profile.nama_lengkap || profile.full_name;
            }
          }

          // Fallback otomatis ke akun penanggung jawab
          if (!fullName) {
            if (user.email === 'satanikbal12@gmail.com') {
              fullName = 'Mohd. Ikbal, S.Tr.Kes';
            } else {
              fullName = user.email ? user.email.split('@')[0].toUpperCase() : 'Petugas Kasir';
            }
          }
  ...
  // --- LOGIKA CANVAS TANDA TANGAN (TTD) ---
  ...
  const draw = (e: React.MouseEvent<HTMLCanvasElement> | React.TouchEvent<HTMLCanvasElement>) => {
    ...
    ctx.strokeStyle = '#0284c7'; // Warna biru tinta pulpen
    ctx.lineWidth = 2.5;
    ctx.lineCap = 'round';
          ...
          <form onSubmit={handleSubmitToDatabase} className="space-y-6">
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-3xl p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-1 pl-6 border-l-2 border-emerald-500">
                    ...
                    <p className="text-xs text-slate-500 font-mono">
                      Email / ID: <span className="font-semibold text-slate-700">{patientData.petugasEmail || 'satanikbal12@gmail.com'}</span>
                    </p>
                ...
                {/* Kanan: Canvas Tanda Tangan Digital */}
