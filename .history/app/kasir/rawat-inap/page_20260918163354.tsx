...
export default function InputRawatInapPage() {
  ...
  const handleSubmitToDatabase = async (e: React.FormEvent) => {
    ...
    if (patientData.jenisPenjaminan === 'BPJS KESEHATAN' && !patientData.noSep) {
      if (!confirm('Nomor SEP BPJS masih kosong. Yakin ingin melanjutkan pengajuan?')) {
        return;
      }
    }
    ...
    try {
      ...
      alert('Rincian Biaya Rawat Inap Berhasil Diajukan!\nMenunggu verifikasi dari Administrator.');
      loadSavedBillings();

      if (confirm('Ingin mencetak Draft Kuitansi / Perincian Biaya?')) {
        window.print();
      }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-100 font-sans print:bg-white">
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        {/* TAB 1: DAFTAR TAGIHAN TERSIMPAN */}
        {activeTab === 'list' && (
        ...
        {activeTab === 'form' && (
          <div className="bg-white rounded-3xl shadow-xl border border-slate-200 p-6 sm:p-8 space-y-6">
            ...
            {/* NOTIFIKASI APABILA DITOLAK ADMIN */}
            {patientData.statusVerifikasi === 'REJECTED' && (
              <div className="bg-rose-50 border-2 border-rose-300 rounded-2xl p-4 flex items-start space-x-3 text-rose-900 shadow-sm animate-pulse">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[11px] text-rose-600">
                    Silakan perbaiki rincian biaya di bawah ini, lalu klik tombol <b>Ajukan Ulang Ke Admin</b>.
                  </p>
                </div>
              </div>
            )}
                ...
                <table className="w-full text-left border-collapse text-xs">
                              ...
                              <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                                ...
                                <td className="p-3 text-center">
                                  <input 
                                    ...
                                    className="w-16 text-center px-2 py-1 rounded-lg border border-slate-300 bg-white font-mono"
                                  />
                                </td>
                                ...
                                <td className="p-3 text-right">
                                  <input 
                                    ...
                                    className="w-32 text-right px-2 py-1 rounded-lg border border-slate-300 bg-white font-mono font-medium"
                                  />
                                </td>
                                ...
                                <td className="p-3 text-center">
                                  <button
                                    ...
                                    className={`inline-flex items-center space-x-1 px-3 py-1 rounded-xl text-xs font-bold transition ${
                                      item.isCovered 
                                        ? 'bg-emerald-100 text-emerald-800 border border-emerald-300' 
                                        : 'bg-slate-100 text-slate-500 border border-slate-200'
                                    }`}
                                  >
                                    {item.isCovered ? <CheckSquare className="w-4 h-4 text-emerald-600" /> : <Square className="w-4 h-4 text-slate-400" />}
                  ...
                  <tfoot>
                    <tr className="bg-slate-900 text-white font-black text-sm border-t-4 border-emerald-500">
                      ...
                      <td className="p-3.5 text-right text-emerald-300 font-mono">Rp {formatNumber(totalJumlah)}</td>
                      <td className="p-3.5 text-right text-emerald-400 font-mono">Rp {formatNumber(totalDitanggung)}</td>
                      <td className="p-3.5 text-right text-rose-300 font-mono">Rp {formatNumber(totalSelisih)}</td>
                      <td></td>
                    </tr>
                  </tfoot>
                </table>
