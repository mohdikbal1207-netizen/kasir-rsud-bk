...
export default function InputRawatInapPage() {
  ...
  const [patientData, setPatientData] = useState({
    ...
    petugasEmail: '',
    signatureBase64: '',
    statusVerifikasi: 'NEW', // 'NEW' berarti data baru yang belum pernah dikirim
    catatanAdmin: ''
  });

  // Data hanya terkunci jika statusnya sudah dikirim (PENDING_VERIFIKASI, VERIFIED, dll)
  ...
  const handleSubmitToDatabase = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Karena ini data baru (NEW), kita gunakan .insert() atau .upsert() murni sebagai data baru
      ...
      // Kunci form setelah berhasil dikirim
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 print:hidden">
          <div className="flex items-center space-x-2 bg-slate-200/80 p-1.5 rounded-2xl">
            ...
            <button
              type="button"
              onClick={() => {
                // RESET TOTAL FORM UNTUK MEMBUAT PENGATURAN/DATA BARU (NO REG BARU)
                setPatientData({
                  noReg: 'REG-RANAP-' + Date.now().toString().slice(-6),
                  jenisPenjaminan: 'UMUM / MANDIRI',
                  metodePembayaran: 'Tunai / Cash',
                          ...
                          <td className="p-3 text-center">
                            <div className="flex items-center justify-center space-x-1">
                              <button
                                ...
                                title="Lihat Detail (Read-Only Terkunci)"
                              >
        ...
        {activeTab === 'form' && (
          <div className="bg-white rounded-3xl shadow-xl border border-slate-200 p-6 sm:p-8 space-y-6">
            ...
            {/* Banner Indikator Status Dokumen */}
            {isFormLocked ? (
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <span className="font-black text-xs uppercase bg-amber-200 text-amber-900 px-2 py-0.5 rounded-md border border-amber-300">
                      DOKUMEN TERKUNCI PERMANEN (TELAH DIKIRIM KE ADMIN)
                    </span>
                    <p className="text-xs font-medium text-amber-800 mt-1">
                      Data ini sudah masuk ke sistem dan tidak dapat diubah kembali. Anda hanya dapat melihat rincian atau mencetaknya.
                    </p>
                  </div>
                </div>
                <span className="text-[11px] font-bold bg-amber-100 text-amber-800 px-3 py-1.5 rounded-xl border border-amber-300">
                  Status: {patientData.statusVerifikasi}
                </span>
              </div>
            ) : (
              <div className="bg-emerald-50 border-2 border-emerald-300 rounded-2xl p-4 flex items-center space-x-3 text-emerald-900 shadow-sm print:hidden">
                ...
                <div>
                  <span className="font-black text-xs uppercase bg-emerald-200 text-emerald-900 px-2 py-0.5 rounded-md border border-emerald-300">
                    MODE INPUT BARU (FORMULIR SIAP DIKIRIM)
                  </span>
                  <p className="text-xs font-medium text-emerald-800 mt-1">
                    Silakan isi rincian tagihan baru. Setelah dikirim, data ini akan terkunci permanen.
                  </p>
                </div>
              </div>
            )}
