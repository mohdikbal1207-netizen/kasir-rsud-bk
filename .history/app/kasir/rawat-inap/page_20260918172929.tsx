...
export default function InputRawatInapPage() {
  ...
  // Mengambil data user yang sedang login langsung dari tabel public.users (Nama & NIP)
  // Mengambil data user yang sedang login otomatis berdasarkan EMAIL dari tabel public.users
  // Mengambil data user yang sedang login otomatis dari Auth Metadata & tabel public.users
  useEffect(() => {
    const fetchCurrentUser = async () => {
      try {
        const { data: { user } } = await supabase.auth.getUser();
        if (user) {
          // 1. Ambil dari user_metadata (sumber yang sama dengan header KasirHeader)
          const meta = user.user_metadata || {};
          let fullName = meta.nama_lengkap || meta.full_name || meta.name || '';
          let nipVal = meta.nip || '';

          // 2. Coba perbarui dari tabel public.users jika email tersedia
          if (user.email) {
            const { data: dbUser } = await supabase
              .from('users')
              .select('nama_lengkap, nip, full_name, name')
              .eq('email', user.email)
              .maybeSingle();
            
            if (dbUser) {
              fullName = dbUser.nama_lengkap || dbUser.full_name || dbUser.name || fullName;
              nipVal = dbUser.nip || nipVal;
            }
          }

          // 3. Fallback terakhir jika data kosong
          if (!fullName) {
            fullName = user.email?.split('@')[0].toUpperCase() || 'PETUGAS KASIR';
          }
          if (!nipVal) {
            nipVal = meta.nip || '-';
          }
