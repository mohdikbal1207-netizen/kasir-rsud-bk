  ...
  useEffect(() => {
    const fetchCurrentUser = async () => {
      try {
        ...
        if (user) {
          // Ambil langsung dari tabel profiles berdasarkan id user yang aktif
          const { data: profile } = await supabase
            .from('profiles')
            .select('nama_lengkap, full_name, name')
            .eq('id', user.id)
            .maybeSingle();
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-100 font-sans print:bg-white relative">
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        {/* SECTION: DAFTAR RIWAYAT & AKUMULASI */}
        <div className="bg-white rounded-3xl shadow-xl border border-slate-200 p-6 sm:p-8 space-y-6 print:hidden">
