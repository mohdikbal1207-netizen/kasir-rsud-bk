...
export default function InputRawatInapPage() {
  ...
  const handleAddRow = (kategoriTarget: string) => {
    const newRow: BillingItem = {
      ...
      uraian: 'Tingkatkan/Ketik Uraian Tambahan...',
      volume: 0,
      tarif: 0,
      ditanggung: 0
    };
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-slate-200 p-6 sm:p-8 space-y-6">
          ...
          <form onSubmit={handleSubmitToDatabase} className="space-y-6">
            ...
            {/* Tabel Perincian Biaya dengan Tombol Tambah Dinamis per Kategori */}
              ...
              <table className="w-full text-left border-collapse text-xs">
                ...
                <tbody className="divide-y divide-slate-200">
                  {billingItems.map((item, index) => {
                    const showCategoryHeader = item.kategori !== lastCategory;
                    ...
                    // Cek apakah item berikutnya berbeda kategori atau ini item terakhir untuk menampilkan tombol tambah di akhir kategori
                    const nextItem = billingItems[index + 1];
                    const isLastInCategory = !nextItem || nextItem.kategori !== item.kategori;

                    return (
                      <>
                        {showCategoryHeader && (
                          <tr key={`cat-${item.kategori}`} className="bg-emerald-50/90 border-t-2 border-emerald-600">
                            <td colSpan={7} className="p-2.5 font-black text-emerald-900 tracking-wide text-xs flex items-center justify-between">
                              <span>{item.kategori}</span>
                              <button
                                type="button"
                                onClick={() => handleAddRow(item.kategori)}
