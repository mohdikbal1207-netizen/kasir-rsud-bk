...
export default function InputRawatInapPage() {
  ...
  const [patientData, setPatientData] = useState({
    ...
    petugasEmail: '',
    signatureBase64: '',
    statusVerifikasi: 'NEW', // Default NEW agar form input baru tidak terkunci
    catatanAdmin: ''
  });

  // Form hanya terkunci jika statusnya benar-benar sudah dikirim dan menunggu verifikasi atau sudah diverifikasi admin
  ...
  const handleSubmitToDatabase = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update status verifikasi di state lokal setelah sukses dikirim agar langsung terkunci
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 print:hidden">
          <div className="flex items-center space-x-2 bg-slate-200/80 p-1.5 rounded-2xl">
            ...
            <button
              type="button"
              onClick={() => {
                // RESET TOTAL FORM KE MODE BARU & TIDAK TERKUNCI
                setPatientData(prev => ({
                  ...prev,
                  ...
                  kepalaRuangan: '',
                  signatureBase64: '',
                  statusVerifikasi: 'NEW', // Status diset NEW agar langsung terbuka/bisa diisi
                  catatanAdmin: ''
                }));
                clearSignature();
                setActiveTab('form');
              ...
              <div className="bg-emerald-50 border-2 border-emerald-300 rounded-2xl p-4 flex items-center space-x-3 text-emerald-900 shadow-sm print:hidden">
                ...
                <div>
                  <span className="font-black text-xs uppercase bg-emerald-200 text-emerald-900 px-2 py-0.5 rounded-md border border-emerald-300">
                    MODE KOREKSI / INPUT BARU (FORMULIR TERBUKA)
                  </span>
                  <p className="text-xs font-medium text-emerald-800 mt-1">
                    Silakan isi rincian biaya rawat inap pasien baru secara lengkap.
                  </p>
                </div>
              </div>
            )}
