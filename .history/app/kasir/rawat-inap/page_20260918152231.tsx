...
export default function InputRawatInapPage() {
  ...
  const [patientData, setPatientData] = useState({
    ...
    dokterMerawat: '',
    kepalaRuangan: '',
    bendahara: 'Memuat data petugas...' // Placeholder saat mengambil sesi
  });

  // Effect untuk Ambil User Session dari Supabase Auth
  useEffect(() => {
    const fetchCurrentUser = async () => {
      try {
        const { data: { user } } = await supabase.auth.getUser();
        if (user) {
          // Mengambil nama dari metadata user atau fallback ke email/full_name
          const displayName = 
          ...
          setPatientData(prev => ({
            ...prev,
            bendahara: 'Petugas Tidak Terautentikasi'
          }));
        }
      } catch (err) {
        console.error('Gagal mengambil sesi petugas:', err);
      }
    };
