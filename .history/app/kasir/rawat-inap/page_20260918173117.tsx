  ...
  export default function InputRawatInapPage() {
    ...
    // Mengambil data user dengan PRIORITAS UTAMA dari tabel public.users
  useEffect(() => {
    const fetchCurrentUser = async () => {
      try {
        ...
        if (user && user.email) {
          ...
          // 1. Ambil langsung dari tabel public.users (Prioritas Utama)
          ...
          if (dbUser) {
            fullName = dbUser.nama_lengkap || dbUser.full_name || dbUser.name || '';
            nipVal = dbUser.nip || '';
          }

          // 2. Fallback ke metadata jika di tabel public.users kosong
          if (!fullName) {
            const meta = user.user_metadata || {};
            fullName = meta.nama_lengkap || meta.full_name || meta.name || user.email.split('@')[0].toUpperCase();
          }

          if (!nipVal) {
            nipVal = user.user_metadata?.nip || '-';
          }
