...
export default function KasirRincianObatPage() {
  ...
  const filteredRecords = records.filter(r => {
    ...
    if (statusFilter === 'pending') return matchSearch && r.status_verifikasi === 'Menunggu Verifikasi';
    if (statusFilter === 'disetujui') return matchSearch && r.status_verifikasi === 'Disetujui';
    return matchSearch;
  });
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full bg-slate-50 border border-slate-300 rounded-xl p-2.5 pr-8 focus:outline-none font-mono"
                    />
                    <History className="w-4 h-4 text-slate-400 absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none" />
                  </div>
                  <span className="text-[10px] text-amber-700 font-semibold mt-0.5 block">
                    {isSearchingRm ? 'Mencari riwayat pasien...' : '💡 Tab keluar kolom untuk auto-fill data lama.'}
                  </span>
                </div>
                <div>
              ...
              <div className="space-y-2 pt-2">
                <div className="flex justify-between items-center">
                  <span className="font-extrabold text-slate-800 uppercase text-[11px] flex items-center gap-1">
                    Daftar Item Obat / OBHP <span className="text-[10px] text-slate-400 font-normal">(Auto-suggest aktif)</span>
                  </span>
                  <button
                    type="button"
