...
export default function KasirRincianObatPage() {
  ...
  // State Tab Navigasi Utama (Transaksi vs Riwayat Pasien)
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between selection:bg-emerald-500 selection:text-white">
      ...
      <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 flex-1 space-y-6 print:p-0 print:max-w-none">
        ...
        {/* TAB SWITCHER UTAMA (TRANSAKSI vs RIWAYAT PASIEN) */}
        <div className="flex items-center gap-2 border-b border-slate-200 pb-2 print:hidden">
          ...
          >
            <FolderClock className="w-4 h-4" /> Tab Riwayat Rekam Medis Pasien ({uniquePatientsList.length})
          </button>
        </div>

        {/* KONTEN TAB 1: DAFTAR TRANSAKSI KASIR */}
        {activeTab === 'transaksi' && (
          <>
        ...
        <div className="bg-white border border-slate-200/80 rounded-3xl p-6 shadow-sm space-y-4 print:hidden">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-2xl text-xs font-bold">
                <button
                  onClick={() => setPenjaminFilter('semua')}
                  className={`px-2.5 py-1.5 rounded-xl transition cursor-pointer ${penjaminFilter === 'semua' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`}
                >
                  Semua Penjamin
                </button>
                <button
                  onClick={() => setPenjaminFilter('Umum')}
                  className={`px-2.5 py-1.5 rounded-xl transition cursor-pointer ${penjaminFilter === 'Umum' ? 'bg-white text-emerald-800 shadow-sm' : 'text-slate-500'}`}
                >
                  Umum
                </button>
                <button
                  onClick={() => setPenjaminFilter('BPJS')}
                  className={`px-2.5 py-1.5 rounded-xl transition cursor-pointer ${penjaminFilter === 'BPJS' ? 'bg-white text-emerald-800 shadow-sm' : 'text-slate-500'}`}
                >
                  BPJS
                </button>
              </div>
        ...
        {/* KONTEN TAB 2: TAB RIWAYAT PASIEN & LOG REKAM MEDIS */}
        {activeTab === 'riwayat_pasien' && (
          <div className="bg-white border border-slate-200/80 rounded-3xl p-6 shadow-sm space-y-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b pb-4">
              <div>
                <h2 className="text-base font-black text-slate-900 flex items-center gap-2">
                  <FolderClock className="w-5 h-5 text-emerald-600" /> Direktori &amp; Riwayat Kunjungan Pasien RSUD Bukit Kerman
                </h2>
                <p className="text-xs text-slate-500">Akumulasi rekam medis pasien berdasarkan No. Rekam Medis (RM) yang tercatat di sistem kasir.</p>
              </div>
            </div>
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-slate-100 text-slate-700 font-black uppercase border-b border-slate-200 text-[10px]">
                    ...
                    <th className="p-3.5">Layanan &amp; Penjamin Terakhir</th>
                    <th className="p-3.5 text-right">Akumulasi Biaya Farmasi</th>
                    <th className="p-3.5 text-center">Aksi Cepat Formulir</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 font-medium">
                  {uniquePatientsList.length === 0 ? (
                    <tr><td colSpan={6} className="text-center py-8 text-slate-400">Belum ada data riwayat rekam medis pasien.</td></tr>
                  ) : (
                      ...
                      <tr key={idx} className="hover:bg-slate-50">
                        ...
                        <td className="p-3.5 text-center">
                          <span className="px-2.5 py-1 bg-emerald-50 text-emerald-700 rounded-full font-extrabold text-[10px]">
                            {p.total_kunjungan} Berkas Kunjungan
                          </span>
                        </td>
                        <td className="p-3.5">
                        ...
                        <td className="p-3.5 text-center">
                          <button
                            ...
                            title="Tarik data pasien ini ke formulir baru"
                          >
                            Tarik ke Form →
                          </button>
                        </td>
