...
export default function KasirRincianObatPage() {
  ...
  // State Tab Navigasi Utama
  const [activeTab, setActiveTab] = useState<'transaksi' | 'riwayat_pasien'>('transaksi');

  // State Filter (Jenis Layanan, Rentang Tanggal, & Penjamin)
  ...
  // State Modal Detail Riwayat Pasien
  const [isPatientDetailModalOpen, setIsPatientDetailModalOpen] = useState<boolean>(false);
  const [selectedPatientHistoryList, setSelectedPatientHistoryList] = useState<RincianObatRecord[]>([]);
  const [selectedPatientName, setSelectedPatientName] = useState<string>('');
  const [selectedPatientRm, setSelectedPatientRm] = useState<string>('');
  ...
  const handleOpenPatientDetail = (rm: string, nama: string) => {
    const patientHistory = records.filter(r => r.no_rm === rm);
    setSelectedPatientHistoryList(patientHistory);
    setSelectedPatientRm(rm);
    setSelectedPatientName(nama);
  ...
  const filteredRecords = records.filter(r => {
    ...
    const matchPenjamin =
      ...
      (r.penjamin || 'Umum / Mandiri').toLowerCase().includes(penjaminFilter.toLowerCase());

    return matchSearch && matchStatus && matchDate && matchService && matchPenjamin;
  });
  ...
  records.forEach(r => {
    if (!uniquePatientsMap.has(r.no_rm)) {
      uniquePatientsMap.set(r.no_rm, {
        no_rm: r.no_rm,
        nama_pasien: r.nama_pasien,
        total_kunjungan: 0,
        layanan_terakhir: r.jenis_layanan,
        penjamin_terakhir: r.penjamin || 'Umum',
        total_akumulasi_biaya: 0,
        tanggal_terakhir: r.created_at,
        status_verifikasi_terakhir: r.status_verifikasi
      });
    }
    const p = uniquePatientsMap.get(r.no_rm);
    p.total_kunjungan += 1;
    p.total_akumulasi_biaya += r.total_biaya || 0;
    if (new Date(r.created_at) > new Date(p.tanggal_terakhir)) {
      p.tanggal_terakhir = r.created_at;
      p.layanan_terakhir = r.jenis_layanan;
      p.penjamin_terakhir = r.penjamin || 'Umum';
      p.status_verifikasi_terakhir = r.status_verifikasi;
    }
  });
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between selection:bg-emerald-500 selection:text-white">
      ...
      <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 flex-1 space-y-6 print:p-0 print:max-w-none">
        ...
        <div className="flex items-center gap-2 border-b border-slate-200 pb-2 print:hidden">
          ...
          >
            <FileText className="w-4 h-4" /> Daftar Transaksi &amp; Kasir ({records.length})
          </button>
          <button
            onClick={() => setActiveTab('riwayat_pasien')}
            className={`px-4 py-2.5 rounded-2xl text-xs font-bold transition flex items-center gap-2 cursor-pointer ${
          ...
          >
            <FolderClock className="w-4 h-4" /> Tab Riwayat &amp; Status Verifikasi Pasien ({uniquePatientsList.length})
          </button>
        </div>

        {/* KONTEN TAB 1: TRANSAKSI */}
        {activeTab === 'transaksi' && (
          <>
              ...
              <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-2xl text-xs font-bold">
                    ...
                    <button onClick={() => setServiceFilter('Rawat Jalan')} className={`px-2.5 py-1.5 rounded-xl transition cursor-pointer ${serviceFilter === 'Rawat Jalan' ? 'bg-white text-emerald-800 shadow-sm' : 'text-slate-500'}`}>Rawat Jalan</button>
                    <button onClick={() => setServiceFilter('Rawat Inap')} className={`px-2.5 py-1.5 rounded-xl transition cursor-pointer ${serviceFilter === 'Rawat Inap' ? 'bg-white text-emerald-800 shadow-sm' : 'text-slate-500'}`}>Rawat Inap</button>
                    <button onClick={() => setServiceFilter('IGD')} className={`px-2.5 py-1.5 rounded-xl transition cursor-pointer ${serviceFilter === 'IGD' ? 'bg-white text-emerald-800 shadow-sm' : 'text-slate-500'}`}>IGD</button>
                  </div>

                  <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-2xl text-xs font-bold">
                    <button onClick={() => setPenjaminFilter('semua')} className={`px-2.5 py-1.5 rounded-xl transition cursor-pointer ${penjaminFilter === 'semua' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`}>Semua Penjamin</button>
                    <button onClick={() => setPenjaminFilter('Umum')} className={`px-2.5 py-1.5 rounded-xl transition cursor-pointer ${penjaminFilter === 'Umum' ? 'bg-white text-emerald-800 shadow-sm' : 'text-slate-500'}`}>Umum</button>
                    <button onClick={() => setPenjaminFilter('BPJS')} className={`px-2.5 py-1.5 rounded-xl transition cursor-pointer ${penjaminFilter === 'BPJS' ? 'bg-white text-emerald-800 shadow-sm' : 'text-slate-500'}`}>BPJS</button>
                  </div>

                  <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-2xl text-xs font-bold">
                    <button onClick={() => setDateFilter('semua')} className={`px-2.5 py-1.5 rounded-xl transition cursor-pointer ${dateFilter === 'semua' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`}>Semua Tgl</button>
                    <button onClick={() => setDateFilter('hari_ini')} className={`px-2.5 py-1.5 rounded-xl transition cursor-pointer flex items-center gap-1 ${dateFilter === 'hari_ini' ? 'bg-white text-emerald-800 shadow-sm' : 'text-slate-500'}`}><Calendar className="w-3 h-3 text-emerald-600" /> Hari Ini</button>
                    <button onClick={() => setDateFilter('custom')} className={`px-2.5 py-1.5 rounded-xl transition cursor-pointer ${dateFilter === 'custom' ? 'bg-white text-emerald-800 shadow-sm' : 'text-slate-500'}`}>Kustom</button>
                  </div>

                  <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-2xl text-xs font-bold">
                    <button onClick={() => setStatusFilter('semua')} className={`px-3 py-1.5 rounded-xl transition cursor-pointer ${statusFilter === 'semua' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`}>Status</button>
            ...
            {dateFilter === 'custom' && (
              <div className="bg-emerald-50/70 border border-emerald-200 p-4 rounded-2xl flex flex-wrap items-center gap-4 text-xs font-bold print:hidden">
                <span className="text-emerald-900 flex items-center gap-1.5"><Filter className="w-4 h-4 text-emerald-600" /> Saring Rentang Tanggal:</span>
                <div className="flex items-center gap-2">
          ...
          <div className="bg-white border border-slate-200/80 rounded-3xl p-6 shadow-sm space-y-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b pb-4">
              <div>
                <h2 className="text-base font-black text-slate-900 flex items-center gap-2">
                  <FolderClock className="w-5 h-5 text-emerald-600" /> Direktori Riwayat Kunjungan &amp; Status Verifikasi Admin
                </h2>
                <p className="text-xs text-slate-500">Memantau status validasi admin/apototek beserta rekam jejak kunjungan pasien di RSUD Bukit Kerman.</p>
              </div>
            ...
            <div className="border border-slate-200 rounded-2xl overflow-hidden bg-white">
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-slate-100 text-slate-700 font-black uppercase border-b border-slate-200 text-[10px]">
                    <th className="p-3.5">No. Rekam Medis (RM)</th>
                    <th className="p-3.5">Nama Lengkap Pasien</th>
                    <th className="p-3.5 text-center">Total Kunjungan</th>
                    <th className="p-3.5">Layanan &amp; Penjamin</th>
                    <th className="p-3.5 text-center">Status Verifikasi Admin</th>
                    <th className="p-3.5 text-right">Akumulasi Biaya</th>
                    <th className="p-3.5 text-center">Aksi Formulir</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 font-medium">
                  {filteredUniquePatients.length === 0 ? (
                    <tr><td colSpan={7} className="text-center py-8 text-slate-400">Belum ada data riwayat pasien yang cocok.</td></tr>
                  ) : (
                    filteredUniquePatients.map((p, idx) => (
                      <tr key={idx} className="hover:bg-slate-50">
                        <td className="p-3.5 font-mono font-bold text-emerald-800">{p.no_rm}</td>
                        <td className="p-3.5 font-bold text-slate-900">{p.nama_pasien}</td>
                        <td className="p-3.5 text-center">
                          <span className="px-2.5 py-1 bg-emerald-50 text-emerald-700 rounded-full font-extrabold text-[10px]">
                            {p.total_kunjungan} Berkas
                          </span>
                        </td>
                        <td className="p-3.5">
                          <span className="block font-bold text-slate-700">{p.layanan_terakhir}</span>
                          <span className="text-[10px] text-slate-500">{p.penjamin_terakhir}</span>
                        </td>
                        <td className="p-3.5 text-center">
                          <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full uppercase inline-block ${
                            p.status_verifikasi_terakhir === 'Disetujui' ? 'bg-emerald-100 text-emerald-800' :
                            p.status_verifikasi_terakhir === 'Ditolak' ? 'bg-rose-100 text-rose-800' : 'bg-amber-100 text-amber-800'
                          }`}>
                            {p.status_verifikasi_terakhir || 'Menunggu Verifikasi'}
                          </span>
                        </td>
                        <td className="p-3.5 text-right font-mono font-black text-slate-900">{formatRupiah(p.total_akumulasi_biaya)}</td>
                        <td className="p-3.5 text-center">
                          <div className="flex items-center justify-center gap-1.5">
                            <button
                              onClick={() => handleOpenPatientDetail(p.no_rm, p.nama_pasien)}
                              className="px-2.5 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-300 rounded-xl font-bold transition cursor-pointer"
                              title="Lihat riwayat lengkap transaksi"
                            >
                              <Info className="w-3.5 h-3.5 inline mr-1 text-emerald-600" /> Detail
                            </button>
                            <button
                              onClick={() => {
                                handleCheckNoRm(p.no_rm);
                                setIsFormOpen(true);
                              }}
                              className="px-3 py-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-300 rounded-xl font-bold transition cursor-pointer"
                            >
                              Tarik ke Form →
                            </button>
                          </div>
            ...
            <div className="flex justify-between items-center border-b pb-3">
              <div>
                <h3 className="text-sm font-black uppercase text-slate-900 flex items-center gap-1.5">
                  <Users className="w-4 h-4 text-emerald-600" /> Rekam Jejak Transaksi: {selectedPatientName}
                </h3>
                <span className="text-xs text-slate-500 font-mono">No. RM: {selectedPatientRm}</span>
              </div>
              <button onClick={() => setIsPatientDetailModalOpen(false)} className="p-1 hover:bg-slate-100 rounded-full cursor-pointer">
                <X className="w-5 h-5 text-slate-500" />
              </button>
            </div>
            
            <div className="overflow-y-auto space-y-3 pr-1 flex-1 text-xs">
              {selectedPatientHistoryList.length === 0 ? (
                <p className="text-center py-6 text-slate-400">Tidak ada rincian transaksi ditemukan untuk pasien ini.</p>
              ) : (
                selectedPatientHistoryList.map((trx) => (
                  <div key={trx.id} className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-2">
                    <div className="flex justify-between items-center font-mono text-[11px] border-b pb-2">
                      <span className="font-bold text-slate-900">{trx.no_transaksi}</span>
                      <span className="text-slate-500">{new Date(trx.created_at).toLocaleDateString('id-ID')} &bull; {trx.jenis_layanan}</span>
                    </div>
                    <div className="flex justify-between items-center text-xs">
                      <span className="text-slate-600">Status Verifikasi: <strong className={`uppercase ${trx.status_verifikasi === 'Disetujui' ? 'text-emerald-700' : trx.status_verifikasi === 'Ditolak' ? 'text-rose-700' : 'text-amber-700'}`}>{trx.status_verifikasi}</strong></span>
                      <span className="font-mono font-black text-slate-900">{formatRupiah(trx.total_biaya)}</span>
                    </div>
                  </div>
                ))
              )}
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-3xl w-full p-6 sm:p-8 shadow-2xl border border-slate-200 space-y-4 max-h-[92vh] flex flex-col justify-between">
            <div className="flex items-center justify-between border-b pb-3">
              ...
              <div className="flex items-center gap-2">
                <button type="button" onClick={handleResetForm} className="bg-slate-100 hover:bg-slate-200 text-slate-600 text-[11px] font-bold px-2.5 py-1.5 rounded-xl transition flex items-center gap-1 cursor-pointer">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 overflow-y-auto pr-1 flex-1 text-xs">
              <div className="bg-emerald-50 border border-emerald-200 p-3 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-2">
                  <button type="button" onClick={() => handleApplyTemplate('umum')} className="px-2.5 py-1 bg-white hover:bg-emerald-100 text-emerald-800 rounded-xl font-bold border border-emerald-300 transition cursor-pointer">+ Paket Umum</button>
                  <button type="button" onClick={() => handleApplyTemplate('igd')} className="px-2.5 py-1 bg-white hover:bg-emerald-100 text-emerald-800 rounded-xl font-bold border border-emerald-300 transition cursor-pointer">+ Paket IGD / Infus</button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                <div>
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <button type="button" onClick={() => setIsHistoryModalOpen(true)} className="text-[10px] text-emerald-700 font-bold hover:underline flex items-center gap-0.5 cursor-pointer">
                      <History className="w-3 h-3" /> Pilih Riwayat
                    </button>
                  </div>
                  <div className="relative flex items-center">
                    <input type="text" required placeholder="RM-2026-001" value={noRm} onChange={(e) => setNoRm(e.target.value)} onBlur={() => handleCheckNoRm()} className="w-full bg-slate-50 border border-slate-300 rounded-xl p-2.5 pr-16 focus:outline-none font-mono" />
                    <button type="button" onClick={() => handleCheckNoRm()} className="absolute right-1 px-2 py-1.5 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 text-[10px] font-bold flex items-center gap-0.5 cursor-pointer">
                ...
                <div>
                  ...
                  <input type="text" required placeholder="Nama Pasien" value={namaPasien} onChange={(e) => setNamaPasien(e.target.value)} className="w-full bg-slate-50 border border-slate-300 rounded-xl p-2.5 focus:outline-none font-bold" />
                </div>
                <div>
                  <label className="font-bold text-slate-700 block mb-1">Jenis Layanan</label>
                  <select value={jenisLayanan} onChange={(e) => setJenisLayanan(e.target.value)} className="w-full bg-slate-50 border border-slate-300 rounded-xl p-2.5 focus:outline-none font-bold">
                ...
                <div>
                  ...
                  <select value={penjamin} onChange={(e) => setPenjamin(e.target.value)} className="w-full bg-slate-50 border border-slate-300 rounded-xl p-2.5 focus:outline-none font-bold text-emerald-800">
              ...
              <div className="space-y-2 pt-2">
                <div className="flex justify-between items-center">
                  <span className="font-extrabold text-slate-800 uppercase text-[11px] flex items-center gap-1">
                    Daftar Item Obat / OBHP <span className="text-[10px] text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full font-bold">({items.length} Item)</span>
                  </span>
                  <button type="button" onClick={handleAddItem} className="bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-300 px-3 py-1 rounded-xl font-bold flex items-center gap-1 cursor-pointer">
                  ...
                  <table className="w-full text-left text-xs border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {items.map((item, idx) => (
                        <tr key={idx}>
                          <td className="p-2">
                            <input type="text" list="list-obat-rsud" required placeholder="Ketik / pilih obat..." value={item.nama_obat_obhp} onChange={(e) => handleItemChange(idx, 'nama_obat_obhp', e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-2 focus:outline-none font-bold" />
                          </td>
                          <td className="p-2">
                            <input type="number" min={1} required value={item.jumlah} onChange={(e) => handleItemChange(idx, 'jumlah', Number(e.target.value))} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-2 text-center font-mono font-bold focus:outline-none" />
                          </td>
                          <td className="p-2">
                            <input type="number" min={0} required value={item.harga_satuan} onChange={(e) => handleItemChange(idx, 'harga_satuan', Number(e.target.value))} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-2 text-right font-mono focus:outline-none" />
                          </td>
                          <td className="p-2 text-right font-mono font-bold text-slate-900">{formatRupiah(item.subtotal)}</td>
                          <td className="p-2 text-center">
                            <button type="button" onClick={() => handleRemoveItem(idx)} className="text-rose-500 hover:text-rose-700 p-1 cursor-pointer">
              ...
              <div className="bg-slate-50 border border-slate-200 p-3 rounded-2xl space-y-2">
                ...
                <div className="flex justify-between items-center text-xs">
                  ...
                  <input type="number" min={0} value={diskonRupiah} onChange={(e) => setDiskonRupiah(Number(e.target.value))} className="w-36 bg-white border border-slate-300 rounded-xl p-1.5 text-right font-mono text-xs font-bold focus:outline-none" />
                </div>
              ...
              <div>
                ...
                <input type="text" value={penanggungJawab} onChange={(e) => setPenanggungJawab(e.target.value)} className="w-full bg-slate-50 border border-slate-300 rounded-xl p-2.5 focus:outline-none font-medium" />
              </div>

              <div className="flex justify-end gap-2 pt-2 border-t">
                <button type="button" onClick={() => setIsFormOpen(false)} className="px-4 py-2 bg-slate-200 text-slate-700 rounded-xl font-bold cursor-pointer">Batal</button>
                <button type="submit" disabled={isSubmitting} className="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold shadow flex items-center gap-1.5 cursor-pointer disabled:opacity-50">
                  <Send className="w-4 h-4" /> {isSubmitting ? 'Mengirim Data...' : 'Kirim & Kunci Data'}
                </button>
              </div>
            </form>
          </div>
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-4 print:p-0 print:bg-white print:static">
          <div className="bg-white rounded-3xl max-w-2xl w-full p-8 shadow-2xl border border-slate-200 space-y-6 print:border-0 print:shadow-none print:max-w-none print:p-0">
                ...
                <tbody className="divide-y border-black font-medium">
                  ...
                  <tr className="border-t-2 border-black font-black">
                    <td className="p-2 border-r-2 border-black uppercase font-bold text-left flex justify-between items-center">
                      ...
                      <span className="text-[10px] font-mono font-normal">({printRecord.rincian_obat_detail?.length || 0} Item)</span>
                    </td>
            ...
            <div className="flex justify-end gap-2 pt-4 border-t border-slate-100 print:hidden">
              <button onClick={() => setPrintRecord(null)} className="px-4 py-2 bg-slate-200 text-slate-700 rounded-xl text-xs font-bold cursor-pointer">Tutup</button>
