...
export default function KasirAbsensiPage() {
  ...
  const [currentUser, setCurrentUser] = useState<{ id: string; email: string; nama_lengkap: string; jabatan?: string } | null>(null);
  ...
  useEffect(() => {
    const loadSessionAndSettings = async () => {
      ...
      // Periksa berbagai kemungkinan kunci sesi di localStorage
      ...
      for (const k of keys) {
        ...
        if (val) {
          try {
            ...
            if (parsed.user) {
              userData = { id: parsed.user.id, email: parsed.user.email, nama_lengkap: parsed.user.user_metadata?.nama_lengkap || parsed.user.email.split('@')[0] };
              break;
      ...
      // Jika tidak ada di localStorage, ambil via Supabase Auth
      if (!userData) {
        const { data: { user } } = await supabase.auth.getUser();
        if (user) {
          // Ambil detail dari tabel public.users
          const { data: profile } = await supabase.from('users').select('*').eq('email', user.email).single();
          userData = {
            id: profile?.id || user.id,
            email: user.email,
            nama_lengkap: profile?.nama_lengkap || user.email.split('@')[0],
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between selection:bg-sky-500 selection:text-white">
      ...
      <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 flex-1 space-y-6">
        ...
        {/* INFO PENGATURAN HARI INI DARI TABEL SETTING */}
        {activeSetting && (
          <div className="bg-sky-50 border border-sky-200 rounded-3xl p-4 px-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-sky-900 shadow-sm">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-sky-100 rounded-2xl flex items-center justify-center text-sky-700 shrink-0">
                <CalendarDays className="w-5 h-5" />
              </div>
              <div>
                <h4 className="text-xs font-black uppercase tracking-wide">Jadwal Shift Aktif Hari Ini ({activeSetting.hari})</h4>
                <p className="text-xs text-sky-700">Jam Masuk: <strong>{activeSetting.jam_masuk}</strong> | Jam Pulang: <strong>{activeSetting.jam_pulang}</strong> | Radius Maksimal: <strong>{activeSetting.radius_meter} Meter</strong></p>
              </div>
            </div>
            <span className="bg-sky-600 text-white font-bold text-[10px] px-3 py-1 rounded-full uppercase">{activeSetting.nama_shift}</span>
          </div>
        )}
        ...
        <div className="bg-white border border-slate-200/80 rounded-3xl p-6 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">Akun login aktif: <strong className="text-sky-700 font-mono">{currentUser?.email || 'Memuat akun...'}</strong></p>
            </div>
          </div>
          <button
