...
// Koordinat Resmi RSUD Bukit Kerman, Kerinci, Jambi (Contoh Titik Sentral Rumah Sakit)
const HOSPITAL_LAT = -2.155500; // Sesuaikan dengan titik lat RSUD
const HOSPITAL_LNG = 101.455000; // Sesuaikan dengan titik lng RSUD
...
export default function KasirAbsensiPage() {
  ...
  useEffect(() => {
    fetchRecords();
    addLog('Modul Smart Absensi Kasir terhubung.');
  ...
  // Fungsi Menghitung Jarak Haversine (Meter)
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        if (dist <= MAX_RADIUS_METERS) {
          setLocationStatus(`Lokasi Valid! Berada di dalam area RSUD (${Math.round(dist)} meter dari titik pusat).`);
        } else {
          setLocationStatus(`Di Luar Radius! Anda berada ${Math.round(dist)} meter dari RSUD Bukit Kerman (Maksimal ${MAX_RADIUS_METERS}m).`);
        }
      },
      (error) => {
        setIsLocating(false);
        setLocationStatus('Gagal mendeteksi GPS. Pastikan izin lokasi (GPS) diaktifkan.');
      },
      { enableHighAccuracy: true, timeout: 10000, maximumAge: 0 }
    );
  };

  // Nyalakan Kamera untuk Live Selfie
  ...
  const handleSaveAbsensi = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast('Smart Absensi berhasil dikirim dengan Validasi GPS & Selfie!', 'success');
      addLog(`Mengirim absensi terverifikasi GPS untuk ${namaPegawai}`);
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between selection:bg-sky-500 selection:text-white">
      ...
      <KasirHeader title="RSUD BUKIT KERMAN" subtitle="Modul Kasir — Smart Absensi (GPS &amp; Live Selfie)" />
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <h1 className="text-xl font-black text-slate-900 tracking-tight flex items-center gap-2">
                <UserCheck className="w-6 h-6 text-sky-600" /> Smart Absensi Shift &amp; Lokasi
              </h1>
              <p className="text-xs text-slate-500 mt-0.5">Absensi dilindungi validasi radius GPS rumah sakit dan foto live selfie otomatis.</p>
            </div>
          </div>
          <button
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-slate-100 text-slate-700 font-black uppercase border-b border-slate-200 text-[10px]">
                  <th className="p-3.5">Selfie &amp; Pegawai</th>
                    ...
                    <tr key={r.id} className="hover:bg-slate-50">
                      <td className="p-3.5 flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <span className="text-[10px] text-slate-400 font-mono">{new Date(r.created_at).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })} WIB</span>
                        </div>
                      </td>
                      <td className="p-3.5">
      ...
      {/* MODAL FORM SMART ABSENSI (KAMERA & GPS) */}
      {showFormModal && (
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-4 animate-in fade-in max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b pb-3">
              <h3 className="text-sm font-black uppercase text-slate-900 flex items-center gap-2">
                <Camera className="w-5 h-5 text-sky-600" /> Smart Absensi RSUD Bukit Kerman
              </h3>
            ...
            <form onSubmit={handleSaveAbsensi} className="space-y-4 text-xs">
              ...
              <div>
                <label className="font-bold text-slate-700 block mb-1">Nama Lengkap Petugas *</label>
                <input
                  type="text"
