...
export default function KasirKwitansiPage() {
  ...
  const handleDuplicateRecord = (r: KwitansiRecord) => {
    ...
    setJumlah(r.jumlah || 0);
    setBanyaknyaUang(r.banyaknya_uang || terbilangIndo(r.jumlah));
  ...
  const handleSaveKwitansi = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data, error } = await supabase.from('kwitansi_header').insert([payload]);
      ...
      // Menangani error yang terkadang berupa objek kosong {}
      const errorMsg = err?.message || err?.error_description || (Object.keys(err || {}).length === 0 ? 'Gagal terhubung ke server Supabase (Periksa koneksi internet atau URL/Anon Key Anda).' : JSON.stringify(err));
      
      showToast(`Gagal menyimpan: ${errorMsg}`, 'error');
      console.error('Full caught error object:', err);
    }
  };
