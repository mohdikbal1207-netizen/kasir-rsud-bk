  ...
  const handleSaveKwitansi = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        banyaknya_uang: banyaknyaUang || `${jumlah.toLocaleString('id-ID')} Rupiah`,
      ...
      <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 flex-1 space-y-6">
        ...
        <div className="bg-white border border-slate-200/80 rounded-3xl p-6 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <button
            ...
            title="Pintasan: Alt + N"
          >
            <Plus className="w-4 h-4" /> Buat Kwitansi Baru <span className="opacity-75 text-[10px] font-mono bg-sky-700 px-1.5 py-0.5 rounded">Alt+N</span>
          </button>
        </div>

        {/* KARTU STATISTIK SHIFT HARI INI */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-white border border-slate-200/80 p-4 rounded-2xl shadow-sm flex items-center justify-between">
            <div>
              <span className="text-[11px] font-bold text-slate-400 uppercase">Input Hari Ini</span>
          ...
          <div className="bg-white border border-slate-200/80 p-4 rounded-2xl shadow-sm flex items-center justify-between">
            <div>
              ...
              <h3 className="text-sm font-black font-mono text-slate-900">{formatRupiah(todayTodayNominal)}</h3>
            </div>
            ...
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 font-medium">
                {isLoading ? (
                  ...
                  <tr><td colSpan={7} className="text-center py-12 text-slate-400">Belum ada data kwitansi ditemukan.</td></tr>
                ) : (
