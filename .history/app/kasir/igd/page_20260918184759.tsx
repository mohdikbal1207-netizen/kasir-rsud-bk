...
import { Printer, Save, Calculator, Stethoscope, Search, User, Calendar, MapPin, FileText, CheckCircle2 } from 'lucide-react';
...
export default function FormKasirIGD() {
  ...
  // State Pencarian Tindakan
  const [searchTerm, setSearchTerm] = useState('');

  // State Pilihan Tindakan (ID item -> Quantity)
  ...
  // Filter daftar tindakan berdasarkan pencarian
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white text-slate-800 shadow-2xl rounded-3xl my-6 border border-slate-200 font-sans">
      ...
      <div className="text-center border-b-2 border-slate-900 pb-4 mb-6">
        ...
        <h1 className="text-xl font-black uppercase tracking-wide text-slate-900 mt-1">RSUD BUKIT KERMAN[cite: 12]</h1>
        <p className="text-[11px] text-slate-500">Desa Pondok, Kecamatan Bukit Kerman, Kode Pos: 37176[cite: 12]</p>
        <p className="text-[11px] text-slate-500">Website: https://rsudbukitkerman.kerincikab.go.id | e-mail: rsubukitkerman@gmail.com[cite: 12]</p>
        ...
        <div className="inline-block bg-slate-900 text-white text-xs font-extrabold px-6 py-1.5 rounded-full mt-3 uppercase tracking-widest shadow">
          FORMULIR PEMERIKSAAN &amp; RINCIAN BIAYA IGD[cite: 12]
        </div>
      </div>

      {/* FORM INPUT DATA PASIEN */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-200 mb-6">
        <div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mb-3">
        <h3 className="text-xs font-black uppercase text-slate-900 tracking-wider flex items-center gap-2">
          <Stethoscope className="w-4 h-4 text-emerald-600" /> Daftar Tindakan &amp; Pemeriksaan IGD[cite: 12]
        </h3>
        <div className="relative w-full sm:w-72">
          ...
          <input 
            ...
            className="w-full pl-9 pr-3 py-1.5 bg-slate-50 border border-slate-300 rounded-xl text-xs focus:ring-2 focus:ring-emerald-500 font-medium"
          />
        </div>
      </div>
        ...
        <table className="w-full text-left border-collapse text-xs">
          <thead className="sticky top-0 bg-slate-900 text-white uppercase text-[10px] z-10">
            <tr>
              ...
              <th className="p-3">Jenis Pemeriksaan / Tindakan Medis IGD[cite: 12]</th>
              <th className="p-3 text-right">Tarif (Rp)[cite: 12]</th>
              ...
              filteredTindakan.map((item) => {
                ...
                return (
                  <tr key={item.id} className="hover:bg-slate-50 transition">
                    <td className="p-3 font-bold text-[10px] text-emerald-700 uppercase">{item.kategori}</td>
                    ...
                    <td className="p-3 text-center">
                      <input 
                        ...
                        placeholder="0"
                        className="w-16 p-1.5 text-center border border-slate-300 rounded-lg text-xs font-bold bg-white focus:ring-2 focus:ring-emerald-500"
                      />
                    </td>
      ...
      <div className="bg-slate-900 text-white p-5 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-4 mb-8 shadow-lg">
        <div className="flex items-center space-x-2">
          ...
          <div>
            ...
            <p className="text-lg font-black text-white">Akumulasi Seluruh Pemeriksaan[cite: 12]</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-8 text-center text-xs font-medium text-slate-700 mb-8 pt-4">
        <div>
          <p className="mb-16">Pasien / PJ Pasien / Keluarga[cite: 12]</p>
        ...
        <div>
          ...
          <p className="mb-14">Petugas Kasir / Yang Menerima[cite: 12]</p>
          <p className="font-bold border-b border-slate-400 pb-1 inline-block min-w-[180px] uppercase">
            ( Petugas Kasir IGD )
          </p>
        </div>
      ...
      {/* TOMBOL AKSI CETAK / SIMPAN (Sembunyikan saat dicetak) */}
      <div className="flex items-center justify-end gap-3 print:hidden border-t border-slate-200 pt-4">
        ...
        >
          <Printer className="w-4 h-4 text-emerald-400" /> Cetak Formulir
        </button>
        <button 
          onClick={() => alert('Data rincian biaya IGD berhasil disimpan untuk verifikasi admin!')}
          className="px-6 py-2.5 bg-emerald-700 hover:bg-emerald-600 text-white rounded-xl text-xs font-bold transition flex items-center gap-2 shadow cursor-pointer"
        >
