...
import { 
  ...
  History, FileText, Eye, AlertCircle, RefreshCw, ArrowLeft, X, FileSpreadsheet, Calendar, User, Banknote, Zap, CheckCircle2, ShieldCheck, Info, Download, Upload, Plus, Minus, Trash2, Copy, Receipt, Flame, ShieldAlert, CheckCircle, Tag, MessageSquare
} from 'lucide-react';
...
export default function FormKasirIGD() {
  ...
  const grossTarif = Object.entries(selectedItems).reduce((sum, [id, qty]) => {
    const item = daftarTindakanIGD.find((t) => t.id === id);
    return sum + (item ? item.tarif * qty : 0);
  }, 0);

  const nominalDiskon = typeof formData.diskon === 'number' ? formData.diskon : 0;
  const totalTarif = Math.max(0, grossTarif - nominalDiskon);
  ...
  const handleSaveAndSend = () => {
    ...
    const newRecord: TagihanRecord = {
      ...
      catatanKlinis: formData.catatanKlinis,
    ...
    showToast(`Data rincian biaya IGD untuk pasien ${formData.nama} berhasil disimpan & dikirim!`, 'success');
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-100 font-sans relative pb-16 lg:pb-0">
      ...
      <main className="flex-grow max-w-6xl mx-auto px-4 py-6 sm:py-8 w-full print:p-0 print:m-0 print:max-w-none">
        ...
        {/* NAVIGASI UTAMA & KEYBOARD SHORTCUT BADGES */}
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-6 print:hidden">
          ...
          <div className="flex items-center gap-2">
            <div className="hidden lg:flex items-center gap-1.5 text-[10px] bg-sky-50 text-sky-800 px-3 py-1.5 rounded-full border border-sky-200 font-bold">
              <Zap className="w-3 h-3 text-amber-500" /> Shortcut: Ctrl+S (Simpan) | Ctrl+P (Cetak)
            </div>
            <div className="hidden lg:flex items-center gap-2 text-[11px] bg-emerald-50 text-emerald-800 px-3 py-1.5 rounded-full border border-emerald-200 font-bold">
              <CheckCircle className="w-3.5 h-3.5 text-emerald-600" /> Storage ({riwayatTagihan.length})
            </div>
          </div>
        </div>
        ...
        {activeTab === 'form' && (
            ...
            <div className="bg-white text-slate-800 shadow-2xl rounded-3xl p-6 sm:p-10 border border-slate-200 print:hidden">
              ...
              <div className="bg-slate-50/80 p-6 rounded-3xl border border-slate-200 mb-8 space-y-4 shadow-sm relative">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 relative">
                  ...
                  <div>
                    <label className="block text-[11px] font-bold uppercase text-slate-700 mb-1">Diskon / Potongan Biaya (Rp)</label>
                    <input 
                      type="number"
                      name="diskon"
                      min="0"
                      ...
                      onChange={handleInputChange}
                      placeholder="0"
                      className="w-full p-2.5 bg-white border border-slate-300 rounded-xl text-xs font-semibold focus:ring-2 focus:ring-sky-500 shadow-sm font-mono"
                    />
                  </div>

                  <div>
                    <label className="block text-[11px] font-bold uppercase text-slate-700 mb-1">Catatan Klinis / Instruksi Dokter</label>
                    <input 
                      type="text"
                      name="catatanKlinis"
                      value={formData.catatanKlinis}
                      onChange={handleInputChange}
                      placeholder="Contoh: Kontrol ulang 3 hari lagi / Anjuran rawat"
                      className="w-full p-2.5 bg-white border border-slate-300 rounded-xl text-xs font-semibold focus:ring-2 focus:ring-sky-500 shadow-sm"
                    />
                  </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-8">
              <div className="bg-slate-900 text-white p-5 rounded-2xl flex flex-col justify-between shadow-lg">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    <p className="text-xs uppercase tracking-wider text-slate-400 font-bold">Total Biaya Setelah Diskon</p>
                    <p className="text-sm font-semibold text-slate-200">Gross: {formatRupiah(grossTarif)} | Diskon: {formatRupiah(nominalDiskon)}</p>
                  </div>
                </div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between border-t border-slate-200 pt-4 gap-3">
              ...
              <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto justify-end">
                ...
                <button 
                  ...
                  className="w-full sm:w-auto px-6 py-2.5 bg-sky-700 hover:bg-sky-600 text-white rounded-xl text-xs font-bold transition flex items-center justify-center gap-2 shadow cursor-pointer"
                >
                  <Save className="w-4 h-4" /> Simpan &amp; Kirim (Ctrl+S)
                </button>
              </div>
            </div>
          ...
          {Object.keys(selectedItems).length > 0 && activeTab === 'form' && (
            <div className="fixed bottom-0 left-0 right-0 z-40 bg-slate-900/95 backdrop-blur text-white p-3 px-4 flex items-center justify-between shadow-2xl lg:hidden border-t border-slate-700">
              ...
              <button
                onClick={handleSaveAndSend}
                className="px-4 py-2 bg-sky-600 hover:bg-sky-500 text-white rounded-xl text-xs font-bold shadow cursor-pointer"
              >
                Simpan Tagihan
              </button>
            </div>
          ...
          <div className="hidden print:block bg-white text-black p-6 font-sans">
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-xs mb-6 border border-black p-4">
              ...
              {formData.catatanKlinis && (
                <div className="col-span-2 border-t border-black/30 pt-1 mt-1">
                  <span className="font-bold">Catatan Klinis / Pesan Dokter:</span> {formData.catatanKlinis}
                </div>
              )}
            </div>
            ...
            {/* TOTAL & DISKON PRINT FORMAT */}
            <div className="border-t-2 border-black pt-3 mb-8 space-y-1 text-xs">
              <div className="flex justify-between font-bold">
                <span>Subtotal Biaya Tindakan:</span>
                <span>{formatRupiah(grossTarif)}</span>
              </div>
              {nominalDiskon > 0 && (
                <div className="flex justify-between font-bold text-red-700">
                  <span>Potongan Diskon / Keringanan:</span>
                  <span>- {formatRupiah(nominalDiskon)}</span>
                </div>
              )}
              <div className="flex justify-between items-center border-t border-black pt-2 text-sm font-black">
                <span>TOTAL KESELURUHAN BIAYA IGD:</span>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 print:hidden">
        <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-slate-200">
          ...
          <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 text-xs space-y-2 mb-4 font-mono">
            ...
            {nominalDiskon > 0 && <p><span className="font-bold">Diskon:</span> {formatRupiah(nominalDiskon)}</p>}
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 print:hidden">
        <div className="bg-white rounded-3xl max-w-sm w-full p-6 shadow-2xl border border-slate-200">
          ...
          <div className="bg-amber-50/50 p-4 rounded-2xl border border-amber-200 font-mono text-[11px] text-slate-900 space-y-1 mb-4 shadow-inner">
            ...
            {formData.catatanKlinis && (
              <p className="text-[9px] pt-1 border-t border-dashed border-slate-400 mt-1">Catatan: {formData.catatanKlinis}</p>
            )}
      ...
      <div className="bg-white rounded-3xl max-w-2xl w-full p-6 sm:p-8 shadow-2xl border border-slate-200 max-h-[90vh] overflow-y-auto">
        ...
        <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 mb-5 space-y-2 text-xs">
          <div className="grid grid-cols-2 gap-2">
            ...
            {selectedDetailRecord.catatanKlinis && (
              <div className="col-span-2 pt-1 border-t border-slate-200"><span className="text-slate-500 font-medium">Catatan Klinis:</span> <span className="font-bold text-sky-900">{selectedDetailRecord.catatanKlinis}</span></div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-1 mb-6">
          {selectedDetailRecord.diskon > 0 && (
            <div className="flex items-center justify-between bg-red-50 text-red-800 px-4 py-2 rounded-xl text-xs font-bold border border-red-200">
              <span>Potongan Diskon / Keringanan</span>
              <span>- {formatRupiah(selectedDetailRecord.diskon)}</span>
            </div>
          )}
          ...
          <button
            onClick={() => {
              setFormData(prev => ({
                ...
                diskon: selectedDetailRecord.diskon,
                catatanKlinis: selectedDetailRecord.catatanKlinis || ''
              }));
