...
import { 
  ...
  History, CheckCircle2, Clock, FileText, Eye, Trash2, AlertCircle, RefreshCw, ArrowLeft, X, FileSpreadsheet, Calendar, User, MapPin, Phone, CreditCard 
} from 'lucide-react';
...
export default function FormKasirIGD() {
  ...
  const filteredRiwayat = useMemo(() => {
    return riwayatTagihan.filter((item) => {
      ...
      return matchSearch && matchStatus && matchPenjaminan && matchDate;
    });
  }, [riwayatTagihan, riwayatSearchTerm, filterStatusBayar, filterPenjaminan, filterTanggal]);
  ...
  const handleSaveAndSend = () => {
    ...
    alert(`Data rincian biaya IGD untuk pasien ${formData.nama} (RM: ${formData.noRm}) berhasil disimpan & dikirim ke Admin!`);
  ...
  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.checked) {
      setSelectedRecordIds(filteredRiwayat.map(r => r.id));
    } else {
  ...
  const handleExportExcel = () => {
    ...
    alert(`Berhasil mengexport ${filteredRiwayat.length} data transaksi ke format Excel (.xlsx).`);
  };
          ...
          <div>
            ...
            <div className="bg-white text-slate-800 shadow-2xl rounded-3xl p-6 sm:p-10 border border-slate-200 print:hidden">
              ...
              <div className="bg-gradient-to-r from-sky-50 to-indigo-50 border border-sky-200 p-4 rounded-2xl mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-sm">
                ...
                <span className="bg-emerald-100 text-emerald-800 text-[10px] font-black px-3 py-1.5 rounded-full uppercase border border-emerald-300">
              ...
              {/* FORM INPUT DATA PASIEN PROFESSIONAL STYLING */}
              <div className="bg-slate-50/80 p-6 rounded-3xl border border-slate-200 mb-8 space-y-4 shadow-sm">
            ...
            <div className="hidden print:block bg-white text-black p-6 font-sans">
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-xs mb-6 border border-black p-4">
                ...
                <div><span className="font-bold">Metode / Penjaminan:</span> {formData.metodeBayar} ({formData.penjaminan})</div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-slate-900 text-white p-5 rounded-3xl shadow-xl flex flex-col justify-between">
                <div>
                  <p className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400">Total Sesuai Filter ({filteredRiwayat.length} Pasien)</p>
                  <p className="text-2xl sm:text-3xl font-black font-mono text-emerald-400 mt-1">{formatRupiah(totalNominalFilter)}</p>
                </div>
                <p className="text-[10px] text-slate-400 mt-4">Rincian aktif kasir sesi ini</p>
              </div>

              <div className="bg-emerald-800 text-white p-5 rounded-3xl shadow-xl flex flex-col justify-between">
                <div>
                  <p className="text-[10px] font-extrabold uppercase tracking-widest text-emerald-200">Penerimaan Tunai / Cash</p>
                  <p className="text-2xl sm:text-3xl font-black font-mono text-white mt-1">{formatRupiah(totalTunaiFilter)}</p>
                </div>
                <p className="text-[10px] text-emerald-200 mt-4">Fisik kasir di laci</p>
              </div>

              <div className="bg-teal-800 text-white p-5 rounded-3xl shadow-xl flex flex-col justify-between">
                <div>
                  <p className="text-[10px] font-extrabold uppercase tracking-widest text-teal-200">Penerimaan Online / QRIS</p>
                  <p className="text-2xl sm:text-3xl font-black font-mono text-white mt-1">{formatRupiah(totalOnlineFilter)}</p>
                </div>
                <p className="text-[10px] text-teal-200 mt-4">Transfer / Rekening RSUD</p>
              </div>
            </div>
              ...
              <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 border-b border-slate-200 pb-4">
                <div className="flex items-center gap-3">
                  <div className="flex items-center space-x-2 bg-slate-100 px-3 py-2 rounded-xl border border-slate-200">
                    <input 
                      ...
                      checked={filteredRiwayat.length > 0 && selectedRecordIds.length === filteredRiwayat.length}
                      className="w-4 h-4 rounded text-emerald-600 focus:ring-emerald-500 cursor-pointer"
                    />
                    <span className="text-xs font-bold text-slate-700">Pilih Semua ({selectedRecordIds.length})</span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    ...
                    <span>Ekspor Excel ({filteredRiwayat.length})</span>
                  </button>
                  <button
                    onClick={() => setActiveTab('form')}
                ...
                <table className="w-full text-left border-collapse text-xs">
                  <thead className="bg-slate-900 text-white uppercase text-[10px]">
                    <tr>
                      ...
                      <th className="p-3">Metode / Penjaminan</th>
                      ...
                      filteredRiwayat.map((item) => (
                        <tr key={item.id} className="hover:bg-slate-50 transition">
                          ...
                          <td className="p-3">
                            ...
                            <span className="inline-block mt-0.5 px-2 py-0.5 bg-sky-100 text-sky-800 text-[9px] font-bold rounded uppercase">
                          ...
                          <td className="p-3 text-center">
                            <div className="flex items-center justify-center space-x-1.5">
                              ...
                              <button 
                                onClick={() => {
                                  const updated = riwayatTagihan.filter(r => r.id !== item.id);
          ...
          <div className="bg-white rounded-3xl max-w-2xl w-full p-6 sm:p-8 shadow-2xl border border-slate-200 max-h-[90vh] overflow-y-auto">
            ...
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 mb-5 space-y-2 text-xs">
              <div className="grid grid-cols-2 gap-2">
                ...
                <div><span className="text-slate-500 font-medium">Metode Bayar:</span> <span className="font-bold text-slate-900">{selectedDetailRecord.metodeBayar}</span></div>
                <div><span className="text-slate-500 font-medium">Penjaminan:</span> <span className="font-bold text-slate-900">{selectedDetailRecord.penjaminan}</span></div>
