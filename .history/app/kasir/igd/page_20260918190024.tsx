...
import { 
  ...
  History, CheckCircle2, Clock, FileText, Eye, Trash2 
} from 'lucide-react';
...
export default function FormKasirIGD() {
  // State Tab Aktif ('form' atau 'riwayat')
  ...
  // Data Contoh Riwayat & Akumulasi Tagihan
  const [riwayatTagihan, setRiwayatTagihan] = useState<TagihanRecord[]>([
    { id: 'IGD-2026-001', noRm: '00-45-12', namaPasien: 'Ahmad Zulfikar', tanggal: '18/09/2026', dokter: 'Dr. Rian Pratama', totalTarif: 350000, status: 'Disetujui' },
    { id: 'IGD-2026-002', noRm: '00-45-13', namaPasien: 'Siti Aminah', tanggal: '18/09/2026', dokter: 'Dr. Melati Suci', totalTarif: 200000, status: 'Menunggu Verifikasi' }
  ]);
  ...
  const handleSaveAndSend = () => {
    if (!formData.nama || !formData.noRm) {
      alert('Mohon lengkapi Nama Pasien dan No RM terlebih dahulu!');
      return;
    }
    const newRecord: TagihanRecord = {
      id: `IGD-2026-00${riwayatTagihan.length + 1}`,
      noRm: formData.noRm,
    ...
    alert('Data rincian biaya IGD berhasil disimpan & dikirim ke Admin!');
                  ...
                  {riwayatTagihan.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50 transition">
                      ...
                      <td className="p-3">
                        ...
                        <p className="text-[10px] text-slate-500">RM: {item.noRm}</p>
                      </td>
                      ...
                      <td className="p-3 text-center">
                        <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase ${
                          item.status === 'Disetujui' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'
                        }`}>
                          {item.status}
                        </span>
                      </td>
                      <td className="p-3 text-center">
                        <div className="flex items-center justify-center space-x-1.5">
                          <button 
                            onClick={() => alert(`Melihat detail transaksi ${item.id}`)}
                            className="p-1.5 bg-slate-100 hover:bg-sky-100 hover:text-sky-600 rounded-lg transition"
                            title="Lihat Detail"
                          >
                          ...
                          <button 
                            ...
                            className="p-1.5 bg-slate-100 hover:bg-rose-100 hover:text-rose-600 rounded-lg transition"
                            title="Hapus"
                          >
