...
export default function AdminIGDPage() {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          if (payload.new && (payload.new as TransactionItem).triase === 'Merah') {
            playEmergencyBeep();
          }
          fetchData();
        }
  ...
  const analyticsData = useMemo(() => {
    ...
    filtered.forEach(t => {
      const doc = t.dokter_penanggung_jawab || 'Dokter Jaga IGD';
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col justify-between selection:bg-amber-500 selection:text-white relative overflow-hidden">
              ...
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="bg-slate-100 text-slate-600 uppercase tracking-wider font-extrabold text-[10px] border-b border-slate-200">
                    ...
                    <th className="p-4">Layanan & Dokter</th>
                  ...
                  {paginatedData.map((tx, idx) => {
                    ...
                    return (
                      <tr key={tx.id} className="hover:bg-slate-50/80 transition">
                        ...
                        <td className="p-4">
                          <span className="block text-slate-900">{tx.rincian_layanan || 'Pemeriksaan Darurat'}</span>
                          <span className="text-[10px] text-slate-400">{tx.dokter_penanggung_jawab || 'dr. Jaga IGD'}</span>
                        </td>
      ...
      {/* MODAL ENTRI BARU KASIR (QUICK MANUAL ENTRY) */}
      {showNewEntryModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 print:hidden">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 sm:p-8 shadow-2xl border border-slate-200 space-y-4">
            ...
            <form onSubmit={handleCreateNewTransaction} className="space-y-3 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="font-bold text-slate-700 block mb-1">Dokter Penanggung Jawab</label>
                  <input
                    type="text"
                    placeholder="dr. Jaga IGD"
                    value={newTx.dokter_penanggung_jawab}
                    onChange={(e) => setNewTx({ ...newTx, dokter_penanggung_jawab: e.target.value })}
                    className="w-full bg-slate-50 border border-slate-300 rounded-xl p-2.5 focus:outline-none"
                  />
                </div>
                <div>
              ...
              <div>
                <label className="font-bold text-slate-700 block mb-1">Rincian Layanan Medis</label>
                <textarea
                  rows={2}
                  placeholder="Tindakan IGD, obat-obatan, resep, laboratorium..."
                  value={newTx.rincian_layanan}
                  onChange={(e) => setNewTx({ ...newTx, rincian_layanan: e.target.value })}
                  className="w-full bg-slate-50 border border-slate-300 rounded-xl p-2.5 focus:outline-none"
                />
              </div>

              <div>
                <label className="font-bold text-slate-700 block mb-1">Catatan Klinis / Keterangan</label>
                <input
                  type="text"
                  placeholder="Catatan tambahan..."
                  value={newTx.catatan_klinis}
                  onChange={(e) => setNewTx({ ...newTx, catatan_klinis: e.target.value })}
                  className="w-full bg-slate-50 border border-slate-300 rounded-xl p-2.5 focus:outline-none"
                />
              </div>
      ...
      {/* MODAL CETAK MASSAL / BATCH PRINT */}
      {showBatchPrintModal && (
          ...
          <div className="bg-white rounded-3xl max-w-xl w-full p-6 sm:p-8 shadow-2xl border border-slate-200 space-y-5">
            ...
            <div className="space-y-2">
              <h4 className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                <UserCheck className="w-4 h-4 text-amber-600" /> Beban Kunjungan Berdasarkan Dokter Jaga:
              </h4>
            ...
            <div className="bg-amber-50/40 border border-amber-200/80 rounded-2xl p-5 space-y-3 font-mono text-xs text-slate-800">
              ...
              <div className="space-y-1 text-[11px] font-sans">
                ...
                <div className="flex justify-between">
                  ...
                  <strong className="text-slate-900">{receiptRecord.dokter_penanggung_jawab || 'Dokter Jaga IGD'}</strong>
                </div>
                <div className="flex justify-between">
              ...
              <div className="border-t border-dashed border-amber-300 pt-3 space-y-1 text-[11px] font-sans">
                <p className="text-slate-500 font-bold">Rincian Layanan:</p>
                <p className="text-slate-800 bg-white/80 p-2 rounded-xl border border-amber-100">{receiptRecord.rincian_layanan || 'Pemeriksaan Medis Darurat IGD'}</p>
              </div>
