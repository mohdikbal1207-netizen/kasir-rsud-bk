...
export default function FormKasirIGD() {
  ...
  // MURNI BERDASARKAN INPUT KASIR (Disinkronkan dengan localStorage agar data tidak hilang saat refresh)
  ...
  // DETEKSI OTOMATIS JIKA PASIEN PERNAH BEROBAT SEBELUMNYA
  ...
  }, [riwayatTagihan, riwayatSearchTerm, filterStatusBayar, filterTanggal]);

  // Kalkulasi Metrik Keuangan Ringkasan Sesuai Filter
  ...
  const handleExportExcel = () => {
    ...
    alert(`Mengexport ${filteredRiwayat.length} data transaksi ke format Excel (.xlsx)...`);
  };
      ...
      <main className="flex-grow max-w-6xl mx-auto px-4 py-6 sm:py-8 w-full">
        ...
        {activeTab === 'riwayat' && (
          <div className="space-y-6">
            ...
            {/* KARTU STATISTIK KEUANGAN (SESUAI LAMPIRAN ANDA) */}
            ...
            <div className="bg-white text-slate-800 shadow-2xl rounded-3xl p-6 sm:p-8 border border-slate-200 space-y-6">
              ...
              {/* FILTER BARU SESUAI LAMPIRAN GAMBAR 2 */}
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                <div>
                  <select
                    ...
                    className="w-full p-2 bg-slate-50 border border-slate-300 rounded-xl text-xs font-semibold focus:ring-2 focus:ring-emerald-500"
                  >
                    <option value="Semua Penjaminan">Semua Penjaminan (Umum/BPJS)</option>
                    ...
                    {filteredRiwayat.length > 0 ? (
                      filteredRiwayat.map((item, index) => (
                        <tr key={item.id} className="hover:bg-slate-50 transition">
                          ...
                          <td className="p-3 font-semibold text-slate-700">{item.metodeBayar}</td>
