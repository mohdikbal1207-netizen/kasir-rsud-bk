...
export default function LupaPasswordPage() {
  ...
  const getIdentifierType = (val: string) => {
    if (!val) return '';
    if (val.includes('@')) return 'Email Kedinasan';
    if (/^\d+$/.test(val)) return 'Nomor NIP / ID Pegawai';
    return 'Nama / Username Akun';
  };
    ...
    try {
      await fetch('/api/whatsapp', {
        ...
        body: JSON.stringify({
          ...
          no_telepon: '6282281155614', // Nomor IT Coordinator (Mohd. Ikbal)
        }),
      });
    } catch (err) {
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-50 border border-slate-300 rounded-2xl pl-10 pr-3.5 py-2.5 text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition font-medium shadow-inner"
                    />
                  </div>
                </div>
