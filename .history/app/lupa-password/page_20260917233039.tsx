...
export default function LupaPasswordPage() {
  ...
  // Fungsi memanggil API Internal WhatsApp Gateway untuk Notifikasi Admin IT
    ...
    try {
      await fetch('/api/whatsapp', {
        ...
        body: JSON.stringify({
          ...
          unit_kerja: 'Manajemen / Kasir BLUD',
          no_telepon: '6282281155614', // Placeholder atau nomor dinamis admin
        }),
      });
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoadingStepText('Memeriksa data akun di database...');
  ...
  const handleDirectWhatsApp = () => {
    const message = encodeURIComponent(
      `Halo Admin IT RSUD Bukit Kerman, saya lupa kata sandi akun Kasir BLUD. Berikut ID/NIP/Email saya: ${identifier || '-'}. Mohon bantuan verifikasi dan reset password.`
    );
      ...
      <main className="w-full max-w-6xl mx-auto px-3 sm:px-4 py-6 sm:py-10 flex-1 flex items-center justify-center relative z-10">
        <div className="w-full max-w-md space-y-4">
          ...
          <div className="bg-white/90 border border-slate-200/80 rounded-3xl p-6 sm:p-8 shadow-xl shadow-slate-200/60 backdrop-blur-xl relative overflow-hidden">
            ...
            {errorMessage && (
              <div className="mb-4 p-3.5 bg-rose-50 border border-rose-200 rounded-2xl text-xs text-rose-700 flex items-start space-x-2.5 animate-in fade-in" role="alert">
            ...
            {!isSubmitted ? (
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="Masukkan NIP atau Email aktif..."
              ...
              <div className="space-y-4 text-center animate-in fade-in">
                <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-2xl text-xs text-emerald-800 space-y-2 shadow-sm">
                  <CheckCircle2 className="w-8 h-8 text-emerald-600 mx-auto" />
                  <p className="font-bold text-sm">Permintaan Berhasil Dikirim!</p>
                  <p className="text-[11px] text-slate-600 leading-relaxed">
                    Instruksi pemulihan kata sandi untuk ID <span className="font-semibold text-emerald-700">{identifier}</span> telah dikirim ke antrean sistem dan notifikasi otomatis telah diteruskan ke WhatsApp Admin IT (Mohd. Ikbal).
                  </p>
                </div>
