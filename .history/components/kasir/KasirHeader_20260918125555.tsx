...
export default function KasirHeader({
  ...
  // Ambil Data Profil Asli dari Supabase (Sinkron dengan Halaman Utama)
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        ...
        if (session?.user) {
          ...
          let foundAvatar = meta?.avatar_url || meta?.picture || meta?.foto_url || meta?.foto;

          // Query ke tabel database `users`
          if (email) {
            ...
            if (dbUser) {
              ...
              if (dbUser.foto_url || dbUser.avatar_url || dbUser.foto) {
                foundAvatar = dbUser.foto_url || dbUser.avatar_url || dbUser.foto;
              }
            } else {
              // Cek tabel alternatif `pegawai`
              ...
              if (dbPegawai) {
                ...
                if (dbPegawai.foto_url || dbPegawai.avatar_url || dbPegawai.foto) {
                  foundAvatar = dbPegawai.foto_url || dbPegawai.avatar_url || dbPegawai.foto;
                }
              }
            }
          }
    ...
    <header className="w-full bg-white/85 border-b border-slate-200/80 backdrop-blur-xl sticky top-0 z-40 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex flex-col lg:flex-row items-center justify-between gap-3">
        ...
        <div className="flex items-center flex-wrap gap-2 sm:gap-3 justify-end w-full lg:w-auto">
          ...
          {/* Badge Nama Lengkap & Foto Profil Asli Supabase (Tampil di semua layar md+) */}
          {userName && (
            <div className="hidden md:flex items-center space-x-2 bg-slate-100 border border-slate-200 px-3 py-1.5 rounded-xl text-xs font-medium text-slate-700 shadow-sm">
              {userAvatar ? (
                <img 
                  src={userAvatar} 
                  alt={userName} 
