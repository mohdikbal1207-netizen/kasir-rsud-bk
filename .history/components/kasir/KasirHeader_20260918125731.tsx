...
export default function KasirHeader({
  ...
  const [userName, setUserName] = useState<string>('SAMPEL KASIR');
  ...
  // Ambil Data Profil & Resolusi Foto dari Supabase Storage Bucket ('avatars' / 'profiles')
  useEffect(() => {
    const fetchUserProfileFromSupabase = async () => {
      try {
        ...
        if (session?.user) {
          ...
          let rawAvatar = meta?.avatar_url || meta?.picture || meta?.foto_url || meta?.foto;

          // Query ke tabel database Supabase (`users` atau `pegawai`)
          if (email) {
            const { data: dbUser } = await supabase
              .from('users')
              .select('nama, foto_url, avatar_url, foto, role, unit_kerja')
              .eq('email', email)
              .maybeSingle();

            if (dbUser) {
              if (dbUser.nama) foundName = dbUser.nama;
              if (dbUser.foto_url || dbUser.avatar_url || dbUser.foto) {
              ...
              if (dbPegawai) {
                if (dbPegawai.nama) foundName = dbPegawai.nama;
                if (dbPegawai.foto_url || dbPegawai.avatar_url || dbPegawai.foto) {
                  rawAvatar = dbPegawai.foto_url || dbPegawai.avatar_url || dbPegawai.foto;
                }
              }
            }
          }

          // Resolusi URL Foto dari Supabase Storage Buckets ('avatars' & 'profiles')
          let finalAvatarUrl = null;
          if (rawAvatar) {
            if (rawAvatar.startsWith('http') || rawAvatar.startsWith('data:')) {
              finalAvatarUrl = rawAvatar;
            } else {
              // Cek ke bucket 'avatars' terlebih dahulu
              const { data: avatarBucketData } = supabase.storage
                .from('avatars')
                .getPublicUrl(rawAvatar);

              if (avatarBucketData?.publicUrl && !avatarBucketData.publicUrl.includes('empty')) {
                finalAvatarUrl = avatarBucketData.publicUrl;
              } else {
                // Jika tidak ditemukan di 'avatars', cek ke bucket 'profiles'
                const { data: profileBucketData } = supabase.storage
                  .from('profiles')
                  .getPublicUrl(rawAvatar);

                finalAvatarUrl = profileBucketData?.publicUrl || rawAvatar;
              }
            }
          }
    ...
    <header className="w-full bg-white/85 border-b border-slate-200/80 backdrop-blur-xl sticky top-0 z-40 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex flex-col lg:flex-row items-center justify-between gap-3">
        ...
        <div className="flex items-center flex-wrap gap-2 sm:gap-3 justify-end w-full lg:w-auto">
          ...
          {/* Badge Nama Lengkap & Foto Profil Bucket Supabase di Header */}
          {userName && (
            <div className="flex items-center space-x-2 bg-slate-100 border border-slate-200 px-3 py-1.5 rounded-xl text-xs font-medium text-slate-700 shadow-sm">
              {userAvatar ? (
                <img 
                  ...
                  onError={(e) => {
                    // Fallback otomatis ke inisial huruf jika gambar bucket gagal dimuat
