...
export default function KasirFooter({
  ...
  developerAvatar = "/koordinator.jpeg" // Mengarah langsung ke file foto di folder public
}: KasirFooterProps) {
  return (
    <footer className="w-full bg-white/90 border-t border-slate-200/85 backdrop-blur-xl py-4 px-4 sm:px-6 mt-auto shadow-sm">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4 text-xs text-slate-500">
        ...
        <div className="flex items-center flex-wrap justify-center gap-3">
          ...
          {/* Badge Pengembang (Mohd Ikbal) dengan Foto */}
          <div className="flex items-center space-x-2 bg-slate-100 border border-slate-200 px-3 py-1 rounded-xl text-slate-700 shadow-xs">
            {developerAvatar ? (
              <img 
                src={developerAvatar} 
                alt={developerName} 
                className="w-6 h-6 rounded-full object-cover border border-emerald-500 shadow-xs flex-shrink-0"
                onError={(e) => {
