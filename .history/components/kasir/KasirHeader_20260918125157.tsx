...
import { ArrowLeft, LogOut, Wallet, ShieldCheck, User, Clock, Activity, Wifi } from 'lucide-react';
...
export default function KasirHeader({
  ...
  // FITUR BARU: Jam Real-time untuk validasi waktu transaksi kasir
  useEffect(() => {
    const updateClock = () => {
      const now = new Date();
    ...
    <header className="w-full bg-white/85 border-b border-slate-200/80 backdrop-blur-xl sticky top-0 z-40 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-3">
        ...
        <div className="flex items-center flex-wrap gap-2 sm:gap-3 justify-end w-full sm:w-auto">
          ...
          {currentTime && (
            <div className="hidden lg:flex items-center space-x-1.5 bg-slate-100 border border-slate-200 px-3 py-1.5 rounded-xl text-xs font-mono font-bold text-slate-700">
