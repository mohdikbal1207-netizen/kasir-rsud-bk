...
import { 
  ...
  MessageSquare, Bug, CheckCircle2, UserCheck, Copy, Check, Timer 
} from 'lucide-react';
...
export default function KasirFooter({
  ...
  // FITUR BARU: Penghitung Durasi Sesi Aktif Staf
  const [sessionSeconds, setSessionSeconds] = useState<number>(0);

  // Jam & Tanggal Real-time + Sesi Aktif
  useEffect(() => {
  ...
  const checkDatabaseStatus = () => {
    setDbStatusMessage("Koneksi Supabase Cloud Stabil & Terenkripsi SSL.");
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-400">
          ...
          <div className="flex items-center flex-wrap justify-center gap-2.5 relative">
            ...
            {dbStatusMessage && (
              <div className="absolute -top-10 right-0 bg-emerald-600 text-white text-[11px] font-bold px-3 py-1.5 rounded-xl shadow-xl animate-in fade-in slide-in-from-bottom-1 z-20">
                {dbStatusMessage}
              </div>
            )}
            ...
            {/* FITUR BARU: Indikator Durasi Sesi Aktif Staf */}
            ...
            <button 
              ...
              title="Klik untuk cek status database"
            >
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-[#0B132B] border border-slate-700/80 w-full max-w-md rounded-3xl shadow-2xl overflow-hidden p-6 text-slate-200 relative animate-in zoom-in-95 duration-200">
            ...
            {/* Status Info & Kontak Cepat dengan Salin Ganda */}
            <div className="space-y-2 py-1 text-xs">
              ...
              <div className="flex items-center justify-between bg-slate-900/80 border border-slate-800 px-3 py-2 rounded-xl">
                ...
                <button 
                  ...
                  className="flex items-center space-x-1 text-emerald-400 hover:text-emerald-300 text-[11px] font-bold transition cursor-pointer"
                >
                  {copiedField === 'phone' ? (
                    <>
              ...
              <div className="flex items-center justify-between bg-slate-900/80 border border-slate-800 px-3 py-2 rounded-xl">
                ...
                <button 
                  ...
                  className="flex items-center space-x-1 text-emerald-400 hover:text-emerald-300 text-[11px] font-bold transition cursor-pointer flex-shrink-0"
                >
                  {copiedField === 'email' ? (
                    <>
                      <Check className="w-3.5 h-3.5 text-emerald-400" />
            ...
            {/* Tombol Lapor Kendala dengan Animasi Halus */}
            <div className="pt-3">
              <a 
                ...
                className="w-full flex items-center justify-center space-x-2 bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 border border-amber-500/30 py-2.5 rounded-2xl text-xs font-bold transition shadow-sm mb-3 animate-pulse"
              >
