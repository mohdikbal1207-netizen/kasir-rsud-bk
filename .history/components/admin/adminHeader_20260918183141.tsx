...
import { ArrowLeft, ShieldCheck, Clock, UserCheck, Activity, Calendar, Bell, Globe, CheckCircle2, AlertCircle, Wifi, WifiOff, Cpu, Sparkles, ChevronDown, User, Shield, LogOut } from 'lucide-react';
...
export default function AdminHeader({
  ...
  // State Tambahan UX Baru: Status Performa Server & Dropdown Profil Admin
  const [serverLoad, setServerLoad] = useState<string>('Normal');
    ...
    <header className="w-full bg-slate-900 text-white border-b border-slate-800 sticky top-0 z-50 backdrop-blur-md shadow-xl">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 relative">
        ...
        <div className="hidden xl:flex items-center space-x-3 bg-slate-800/70 border border-slate-700/60 px-4 py-2 rounded-2xl text-xs shadow-inner">
          ...
          {/* Indikator Beban Performa Server */}
          <div className="flex items-center space-x-1 text-slate-300" title="Kapasitas CPU & Database RSUD">
            <Cpu className="w-3.5 h-3.5 text-emerald-400" />
            <span className="text-[11px] font-medium">Load: <strong className="text-white">{serverLoad}</strong></span>
          </div>
          ...
          {/* Sisi Kanan Tengah: Sesi Admin, Dropdown Profil & Dropdown Notifikasi */}
          <div className="flex items-center space-x-3 text-slate-200 relative">
            
            {/* Tombol Profil Admin Interaktif (Fitur Baru) */}
            <div className="relative">
              ...
              {/* Popup Dropdown Profil */}
              {showProfileDropdown && (
                <div className="absolute right-0 mt-3 w-64 bg-white rounded-2xl shadow-2xl border border-slate-200 text-slate-800 p-3 z-50 animate-fade-in">
                  <div className="px-3 py-2 border-b border-slate-100 mb-2">
                    <p className="text-xs font-black uppercase text-slate-900">{adminName}</p>
                    <p className="text-[10px] text-emerald-700 font-bold">Administrator Terverifikasi</p>
                  </div>
                  <div className="space-y-1 text-xs font-medium">
                    <div className="flex items-center space-x-2 px-3 py-2 text-slate-600 rounded-xl bg-slate-50">
                      <Shield className="w-3.5 h-3.5 text-emerald-600" />
                      <span>Akses Penuh RSUD Bukit Kerman</span>
                    </div>
                  </div>
                  <button 
            ...
            <div className="relative">
              ...
              {showNotifDropdown && (
                <div className="absolute right-0 mt-3 w-80 bg-white rounded-2xl shadow-2xl border border-slate-200 text-slate-800 p-4 z-50 animate-fade-in">
