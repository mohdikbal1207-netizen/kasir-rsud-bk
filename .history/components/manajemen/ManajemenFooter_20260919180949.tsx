...
import { ShieldCheck, Server, Lock, HeartHandshake, Code2, Cpu, Activity, Mail, CheckCircle2, Clock, Headphones, Printer, Info } from 'lucide-react';
...
export default function ManajemenFooter() {
  ...
  // TAMBAHAN: State untuk pop-up detail kesehatan server (Uptime Info)
  ...
  // TAMBAHAN: Fungsi helper untuk memicu cetak halaman langsung dari footer
  ...
  return (
    <footer className="w-full bg-white/95 border-t border-slate-200 backdrop-blur-xl mt-auto pt-8 pb-6 shadow-sm print:hidden relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center border-b border-slate-200 pb-6">
          ...
          <div className="lg:col-span-4 flex flex-col items-center lg:items-end space-y-2 text-center lg:text-right">
            ...
            <div className="flex items-center gap-2 text-[10px] text-slate-500 font-mono">
              ...
              {/* TAMBAHAN: Badge Uptime yang dapat diklik untuk info server */}
              <button 
                onClick={() => setShowServerInfoModal(true)}
                className="inline-flex items-center gap-1 text-emerald-600 font-bold hover:underline cursor-pointer bg-emerald-50/80 px-2 py-0.5 rounded-md border border-emerald-100"
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-500">
          ...
          <div className="flex items-center space-x-3 sm:space-x-4 font-semibold text-[11px] flex-wrap justify-center sm:justify-end">
            ...
            {/* TAMBAHAN: Tombol Pintasan Cetak Cepat di Footer */}
            <button 
              onClick={handleQuickPrint}
              className="hover:text-emerald-600 transition flex items-center gap-1 text-slate-700 font-bold bg-slate-100 px-2.5 py-1 rounded-xl border border-slate-200 cursor-pointer active:scale-95"
              title="Cetak Haporan Cepat"
            >
              <Printer className="w-3 h-3 text-slate-600" /> Cetak
            </button>
            <span>&bull;</span>
      ...
      {/* TAMBAHAN: Modal Detail Kesehatan Server & Uptime */}
      {showServerInfoModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 p-6 rounded-3xl shadow-2xl w-full max-w-sm animate-in zoom-in-95 space-y-4">
